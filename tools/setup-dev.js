...
const findAllBlockFolders = (folderPath) => {
  const results = [];
  for (const entry of fs.readdirSync(folderPath)) {
    ...
    if (fs.existsSync(path.join(full, 'package.json'))) {
      ...
      results.push(...findAllBlockFolders(full));
    }
  }
  return results;
};
...
const devPieces = process.env.IB_DEV_BLOCKS || envConfig.IB_DEV_BLOCKS;

if (devPieces) {
  const pieceNames = [...new Set(devPieces.split(',').map(n => n.trim()))];
  const allFolders = findAllBlockFolders(path.resolve('packages', 'blocks'));
