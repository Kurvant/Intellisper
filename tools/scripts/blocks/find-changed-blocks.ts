...
import { findAllPiecesDirectoryInSource, NON_PIECES_PACKAGES, IB_CLOUD_API_BASE } from '../utils/block-script-utils'
...
async function fetchRegistry(release: string): Promise<Set<string>> {
  const url = `${IB_CLOUD_API_BASE}/pieces/registry?release=${release}&edition=ee`
