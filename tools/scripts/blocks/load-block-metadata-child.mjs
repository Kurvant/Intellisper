 ...
 * `block-script-utils.ts:loadPieceFromFolder` so that the piece's
 * `require('@intelblocks/blocks-framework')` resolves via standard
 ...
 *
 * Usage: node load-block-metadata-child.mjs <pieceDistFolder>
 */
...
if (!folderPath) {
    console.error('[load-block-metadata-child] missing folder path argv')
    process.exit(2)
}
...
if (!piece) {
    console.error(`[load-block-metadata-child] no Piece export found in ${entryPath}`)
    process.exit(3)
}
