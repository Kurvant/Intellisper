...
function hasChangesComparedToMain(blockPath: string): boolean {
  try {
    // Check if there are any changes in the piece directory compared to main
    execSync(`git diff --quiet origin/main -- ${blockPath}`, { encoding: 'utf8' });
...
function hasI18nDirectory(blockPath: string): boolean {
  const i18nPath = path.join(blockPath, 'src', 'i18n');
...
function hasTranslationChanges(blockPath: string): boolean {
  try {
    // Check if there are changes in the i18n directory
    const i18nChanges = execSync(`git diff --name-only origin/main -- ${blockPath}/src/i18n`, { encoding: 'utf8' }).trim();
...
function hasVersionBeenBumped(blockPath: string): boolean {
  try {
...
function main(): void {
  ...
  const blocksWithChanges: string[] = [];
  ...
  if (blocksWithChanges.length === 0) {
    console.log('\nNo pieces with translation changes found.');
    return;
  }
  
  console.log(`\nFound ${blocksWithChanges.length} pieces with translation changes:`);
  blocksWithChanges.forEach(piece => console.log(`  - ${piece}`));
  ...
  for (const piece of blocksWithChanges) {
