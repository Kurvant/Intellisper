 ...
 *
 * If no path is provided, it scans packages/blocks/custom/ for all pieces.
 */

import * as fs from 'fs';
import * as path from 'path';

const CUSTOM_PIECES_DIR = path.resolve(__dirname, '../../packages/blocks/custom');

function getRelativeRoot(blockDir: string): string {
  const piecesIndex = blockDir.indexOf('/packages/blocks/');
  if (piecesIndex === -1) {
    throw new Error(`Unexpected piece directory structure: ${blockDir}`);
  }
  const relative = path.relative(blockDir, blockDir.substring(0, piecesIndex));
  return relative;
}

function migratePiece(blockDir: string): void {
  const blockName = path.basename(blockDir);
  const relativeRoot = getRelativeRoot(blockDir);

  console.log(`\nMigrating piece: ${blockName}`);
  ...
  const tsconfigLibPath = path.join(blockDir, 'tsconfig.lib.json');
  ...
  const tsconfigPath = path.join(blockDir, 'tsconfig.json');
  ...
  const projectJsonPath = path.join(blockDir, 'project.json');
  ...
  const workspaceJsonPath = path.join(blockDir, 'workspace.json');
...
if (args.length > 0) {
  ...
  const blockPath = path.resolve(args[0]);
  if (!fs.existsSync(blockPath)) {
    console.error(`Error: Path not found: ${blockPath}`);
    process.exit(1);
  }
  migratePiece(blockPath);
} else {
  // Migrate all custom pieces
  console.log(`Scanning ${CUSTOM_PIECES_DIR} for pieces...`);
  const blockDirs = findPieceDirs(CUSTOM_PIECES_DIR);
  ...
  console.log(`Found ${blockDirs.length} piece(s) to check.\n`);
