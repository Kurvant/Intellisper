...
const main = async () => {
  const blocksMetadata = await findAllPiecesDirectoryInSource()
  const sharedDeps = ['packages/blocks/framework', 'packages/blocks/common']
  
  const sharedResults = await Promise.all(sharedDeps.map(packagePrePublishChecks))
  const validationResults = await processBatches(
    blocksMetadata.filter(p => !sharedDeps.includes(p)),
    10,
