...
const { createToolName, createBlockToolName } = mcpToolNameUtils
...
describe('createBlockToolName', () => {
    it('strips @scope/block- prefix', () => {
        expect(createBlockToolName('@intelblocks/block-slack', 'send_message')).toBe('slack-send_message_pqyv3q_mcp')
    })

    it('strips plain block- prefix', () => {
        expect(createBlockToolName('block-github', 'create_issue')).toBe('github-create_issue_gmsjqn_mcp')
    })

    it('handles names without block- prefix', () => {
        expect(createBlockToolName('slack', 'send_message')).toBe('slack-send_message_pqyv3q_mcp')
    })

    it('normalizes the combined name correctly', () => {
        expect(createBlockToolName('@intelblocks/block-google-sheets', 'insert_row')).toBe('google-sheets-insert_row_q388b6_mcp')
    })
})
