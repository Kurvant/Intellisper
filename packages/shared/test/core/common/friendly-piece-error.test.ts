import { formatBlockError, tryParseFriendlyBlockError } from '../../../src/lib/core/common/friendly-piece-error'
...
describe('formatBlockError', () => {
    it('extracts status, request, response, and apiMessage from a Jira-style 403 HttpError', () => {
        ...
        const result = formatBlockError(error)
...
<Error><Code>NoSuchKey</Code><Message>The specified key does not exist.</Message></Error>`
    ...
    it('returns a default message for nil input', () => {
        expect(formatBlockError(null).message).toBe('Unknown error')
        expect(formatBlockError(undefined).message).toBe('Unknown error')
    })
    ...
    it('treats a string input as the message, stripping any stack frames', () => {
        ...
        const result = formatBlockError(raw)

        expect(result.message).toBe('HttpError: something went wrong')
        expect(result.message).not.toContain('at ')
    })

    it('returns the input verbatim if it is already a FriendlyBlockError', () => {
        const original = formatBlockError(new TestHttpError({ status: 404, body: { message: 'not found' } }, { body: {} }))
        const second = formatBlockError(original)
        expect(second).toBe(original)
    })

    it('fills in raw on an already-formatted error that has none', () => {
        const original = formatBlockError(new Error('Boom'))
        expect(original.raw).toBeUndefined()

        const second = formatBlockError(original, { raw: 'Error: Boom\n    at x' })
    ...
    it('does not overwrite an existing raw on an already-formatted error', () => {
        const original = formatBlockError(new Error('Boom'), { raw: 'original raw' })

        const second = formatBlockError(original, { raw: 'new raw' })
    ...
    it('attaches the raw dump when provided in options', () => {
        ...
        const result = formatBlockError(error, { raw: 'Error: Boom\n    at somewhere' })
    ...
    it('does not set raw when no raw option is provided', () => {
        const result = formatBlockError(new Error('Boom'))

        expect(result.raw).toBeUndefined()
    })

    it('truncates a very long raw dump', () => {
        const result = formatBlockError(new Error('Boom'), { raw: 'y'.repeat(20000) })
...
describe('tryParseFriendlyBlockError', () => {
    it('parses a JSON-stringified FriendlyBlockError', () => {
        const original = formatBlockError(new TestHttpError({ status: 429, body: { message: 'Too many requests' } }, { body: {} }))
        const json = JSON.stringify(original)
        const parsed = tryParseFriendlyBlockError(json)
        expect(parsed).not.toBeNull()
        expect(parsed?.status).toBe(429)
        expect(parsed?.apiMessage).toBe('Too many requests')
    })

    it('returns the value if already a FriendlyBlockError object', () => {
        const original = formatBlockError(new Error('x'))
        expect(tryParseFriendlyBlockError(original)).toEqual(original)
    })

    it('returns null for non-structured strings', () => {
        expect(tryParseFriendlyBlockError('not a friendly error')).toBeNull()
        expect(tryParseFriendlyBlockError('{"foo":"bar"}')).toBeNull()
    })

    it('returns null for nil values', () => {
        expect(tryParseFriendlyBlockError(null)).toBeNull()
        expect(tryParseFriendlyBlockError(undefined)).toBeNull()
    })
})
