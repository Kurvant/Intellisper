...
function buildBlockAction({
    ...
    return {
        ...
        lastUpdatedDate: '2026-05-02T00:00:00.000Z',
        settings: {
            blockName: '@intelblocks/block-store',
            blockVersion: '0.0.1',
            actionName: 'get',
            input: {},
            propertySettings: {},
...
describe('Continue-on-Failure branches', () => {
    ...
    describe('UPDATE_ACTION', () => {
        ...
        it('preserves CoF branches when updating a Block action', () => {
            const head = buildBlockAction({
                name: 'step_1',
                cof: true,
                onSuccess: buildBlockAction({ name: 'success_head' }),
                onFailure: buildBlockAction({ name: 'failure_head' }),
            })
            const flow = buildFlow(head)
            const op: FlowOperationRequest = {
                type: FlowOperationType.UPDATE_ACTION,
            ...
            const after = flowOperations.apply(flow, op)
            const updatedHead = after.trigger.nextAction as BlockAction
            const branches = updatedHead.continueOnFailureBranches
            expect(branches?.onSuccess?.name).toBe('success_head')
            expect(branches?.onFailure?.name).toBe('failure_head')
        })
        ...
        it('preserves CoF branches when changing a step from Code to Block', () => {
            const head = buildCodeAction({
                name: 'step_1',
                cof: true,
            ...
            const after = flowOperations.apply(flow, op)
            const updatedHead = after.trigger.nextAction as BlockAction
            expect(updatedHead.type).toBe(FlowActionType.BLOCK)
            const branches = updatedHead.continueOnFailureBranches
            expect(branches?.onSuccess?.name).toBe('success_head')
            expect(branches?.onFailure?.name).toBe('failure_head')
        })
