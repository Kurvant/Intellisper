...
export enum ApplicationEventName {
    ...
    SECRET_MANAGER_CONNECTED = 'secret.manager.connected',
    SECRET_MANAGER_ROTATED = 'secret.manager.rotated',
    SECRET_MANAGER_DISCONNECTED = 'secret.manager.disconnected',
}
...
// External secret-store configuration is a security-relevant event (spec E.6 / K.1):
// connecting, rotating (re-configuring), or disconnecting a store is audit-logged. The stored
// provider credentials are NEVER included — only non-sensitive identity (id, provider, name).
const SecretManagerEventData = z.object({
...
export const SecretManagerEvent = z.object({
    ...BaseAuditEventProps,
    action: z.union([
        z.literal(ApplicationEventName.SECRET_MANAGER_CONNECTED),
        z.literal(ApplicationEventName.SECRET_MANAGER_ROTATED),
        z.literal(ApplicationEventName.SECRET_MANAGER_DISCONNECTED),
    ]),
    data: SecretManagerEventData,
})
export type SecretManagerEvent = z.infer<typeof SecretManagerEvent>
...
export function summarizeApplicationEvent(event: ApplicationEvent) {
    switch (event.action) {
        ...
        case ApplicationEventName.SECRET_MANAGER_CONNECTED:
            return `Secret manager "${event.data.secretManager.name}" (${event.data.secretManager.providerId}) was connected`
        case ApplicationEventName.SECRET_MANAGER_ROTATED:
            return `Secret manager "${event.data.secretManager.name}" (${event.data.secretManager.providerId}) was reconfigured`
        case ApplicationEventName.SECRET_MANAGER_DISCONNECTED:
            return `Secret manager "${event.data.secretManager.name}" (${event.data.secretManager.providerId}) was disconnected`
    }
}
