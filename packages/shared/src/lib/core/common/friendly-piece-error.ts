...
const FRIENDLY_BLOCK_ERROR_VERSION = 1
...
const isFriendlyBlockError = (value: unknown): value is FriendlyBlockError => {
    if (!isObjectRecord(value)) {
        return false
    }
    return value['__apErrorVersion'] === FRIENDLY_BLOCK_ERROR_VERSION && typeof value['message'] === 'string'
}

export const formatBlockError = (error: unknown, options?: FormatBlockErrorOptions): FriendlyBlockError => {
    const raw = options?.raw
    const withRaw = (base: FriendlyBlockError): FriendlyBlockError => {
    ...
    if (isNil(error)) {
        return withRaw({
            __apErrorVersion: FRIENDLY_BLOCK_ERROR_VERSION,
...
export const tryParseFriendlyBlockError = (value: unknown): FriendlyBlockError | null => {
    if (isNil(value)) {
        return null
    }
    const candidate = isString(value) ? safeJsonParse(value) : value
    return isFriendlyBlockError(candidate) ? candidate : null
}
...
export type FriendlyBlockError = {
    __apErrorVersion: 1
    message: string
    errorName?: string
    status?: number
