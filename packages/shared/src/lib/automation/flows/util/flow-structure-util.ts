...
export const AI_BLOCK_NAME = '@intelblocks/block-ai'
...
function transferStep<T extends Step>(
    ...
    if (updatedStep.type === FlowActionType.CODE || updatedStep.type === FlowActionType.BLOCK) {
...
function extractAgentIds(flowVersion: FlowVersion): string[] {
    const getExternalAgentId = (action: Step) => {
        if (isAgentBlock(action) && 'agentId' in action.settings.input) {
...
function isAgentBlock(action: Step) {
    return (
        action.type === FlowActionType.BLOCK && action.settings.blockName === AI_BLOCK_NAME
    )
}
