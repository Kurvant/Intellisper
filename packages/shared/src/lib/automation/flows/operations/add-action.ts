...
function handleContinueOnFailureBranches(parentStep: Step, request: AddActionRequest): Step {
    if (parentStep.type !== FlowActionType.CODE && parentStep.type !== FlowActionType.BLOCK) {
        throw new IntellisperError({
            code: ErrorCode.FLOW_OPERATION_INVALID,
            params: {
                message: `Continue-on-failure branches are only available on Code and Block actions, got ${parentStep.type}`,
            },
        })
    }
