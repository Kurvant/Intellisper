    ...
    const next = flowStructureUtil.transferFlow(flowVersion, (stepToUpdate) => {
        ...
        switch (request.type) {
            case FlowActionType.CODE: {
                const existingContinueOnFailureBranches = stepToUpdate.type === FlowActionType.CODE || stepToUpdate.type === FlowActionType.BLOCK ? stepToUpdate.continueOnFailureBranches : undefined
            ...
            case FlowActionType.BLOCK: {
                const existingContinueOnFailureBranches = stepToUpdate.type === FlowActionType.CODE || stepToUpdate.type === FlowActionType.BLOCK ? stepToUpdate.continueOnFailureBranches : undefined
                const existingSampleData = stepToUpdate.type === FlowActionType.BLOCK ? stepToUpdate.settings.sampleData : undefined
                updatedAction = {
                    ...baseProps,
                    settings: { ...request.settings, sampleData: existingSampleData },
