...
import { BranchCondition, CodeActionSchema, CodeActionSettings, FlowActionType, LoopOnItemsActionSchema, LoopOnItemsActionSettings, BlockActionSchema, BlockActionSettings, RouterActionSchema, RouterActionSettings } from '../actions/action'
import { FlowStatus } from '../flow'
import { FlowVersion, FlowVersionState } from '../flow-version'
import { Note } from '../note'
import { SampleDataSetting, SaveSampleDataRequest } from '../sample-data'
import { EmptyTrigger, FlowTrigger, FlowTriggerType, BlockTrigger, BlockTriggerSettings } from '../triggers/trigger'
import { flowBlockUtil } from '../util/flow-piece-util'
...
export const UpdateActionRequest = z.union([
    ...
    BlockActionSchema.omit({ lastUpdatedDate: true, settings: true }).and(z.object({ settings: BlockActionSettings.omit({ sampleData: true }) })),
...
export const UpdateTriggerRequest = z.union([
    ...
    BlockTrigger.omit({ lastUpdatedDate: true, settings: true }).and(z.object({ settings: BlockTriggerSettings.omit({ sampleData: true }) })),
])
    ...
    apply(flowVersion: FlowVersion, operation: FlowOperationRequest): FlowVersion {
        ...
        switch (operation.type) {
            ...
            case FlowOperationType.ADD_ACTION: {
                if (operation.request.action.type === FlowActionType.BLOCK) {
                    operation.request.action.settings.blockVersion = flowBlockUtil.getExactVersion(operation.request.action.settings.blockVersion)
                }
            ...
            case FlowOperationType.UPDATE_TRIGGER: {
                if (operation.request.type === FlowTriggerType.BLOCK) {
                    operation.request.settings.blockVersion = flowBlockUtil.getExactVersion(operation.request.settings.blockVersion)
                }
            ...
            case FlowOperationType.UPDATE_ACTION: {
                if (operation.request.type === FlowActionType.BLOCK) {
                    operation.request.settings.blockVersion = flowBlockUtil.getExactVersion(operation.request.settings.blockVersion)
                }
        ...
        clonedVersion.valid = flowStructureUtil.getAllSteps(clonedVersion.trigger).every((step) => {
            const isSkipped = step.type != FlowTriggerType.EMPTY && step.type != FlowTriggerType.BLOCK && step.skip
