...
export type WorkerToApiContract = {
    ...
    getBlock(input: GetBlockRequest): Promise<unknown>
    getBlockArchive(input: { archiveId: string }): Promise<Buffer>
    extendLock(input: { jobId: string, token: string, queueName: string }): Promise<void>
    getUsedBlocks(input: Record<string, never>): Promise<BlockPackage[]>
    markBlockAsUsed(input: { blocks: BlockPackage[] }): Promise<void>
