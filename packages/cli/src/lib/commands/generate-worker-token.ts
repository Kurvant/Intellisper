...
export const generateWorkerTokenCommand = new Command('token')
    ...
    .action(async () => {
        const answers = await prompt([
            {
                type: 'input',
                name: 'jwtSecret',
                message: 'Enter your JWT secret (should be the same as IB_JWT_SECRET used for the app server):',
                validate: (input) => {
                    if (!input) {
        ...
        try {
            ...
            console.log(chalk.green('\nGenerated Worker Token, Please use it in IB_WORKER_TOKEN environment variable:'));
