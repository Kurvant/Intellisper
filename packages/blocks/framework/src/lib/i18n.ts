import { I18nForPiece, BlockMetadataModel, BlockMetadataModelSummary } from "./piece-metadata"
import { LocalesEnum, MAX_KEY_LENGTH_FOR_CORWDIN } from "@intelblocks/shared"
import path from 'path';
import fs from 'fs/promises';

export const blockTranslation = {
  translateBlock: <T extends BlockMetadataModelSummary | BlockMetadataModel>(params: TranslatePieceParams<T>): T => {
    ...
    try {
      ...
      const translatedBlock: T = mutate ? piece : JSON.parse(JSON.stringify(piece))
      blockTranslation.pathsToValuesToTranslate.forEach(key => {
        translateProperty(translatedBlock, key, target)
      })
...
type TranslatePieceParams<T extends BlockMetadataModelSummary | BlockMetadataModel> = {
  piece: T
  locale?: LocalesEnum
  mutate?: boolean
}
