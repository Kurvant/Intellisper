import { PauseType } from '@intelblocks/shared';
import { ActionContext, PauseHook } from ".";
import { InputPropertyMap, BlockAuthProperty } from "../property";
...
export const backwardCompatabilityContextUtils = {
    makeActionContextBackwardCompatible({ context, contextVersion }: MakeActionContextBackwardCompatibleParams): ActionContext<BlockAuthProperty, InputPropertyMap> {
...
function addLegacyMethods({ context }: { context: ActionContext<BlockAuthProperty, InputPropertyMap> }): ActionContext<BlockAuthProperty, InputPropertyMap> {
    return {
        ...context,
...
function addLegacyMethodsAndServerUrl({ context }: { context: ActionContext<BlockAuthProperty, InputPropertyMap> }): ActionContext<BlockAuthProperty, InputPropertyMap> {
    return {
        ...addLegacyMethods({ context }),
        serverUrl: context.server.publicUrl,
    } as unknown as ActionContext<BlockAuthProperty, InputPropertyMap>
}

/**
 * @deprecated Since 2026-04-12. Remove after 2026-10-12 once all pieces migrate to createWaitpoint/waitForWaitpoint.
 */
function buildLegacyPauseHook({ context }: { context: ActionContext<BlockAuthProperty, InputPropertyMap> }): PauseHook {
    return (req) => {
 ...
 * @deprecated Since 2026-04-12. Remove after 2026-10-12 once all pieces migrate to createWaitpoint/waitForWaitpoint.
 */
function buildLegacyGenerateResumeUrl({ context }: { context: ActionContext<BlockAuthProperty, InputPropertyMap> }): (params: { queryParams: Record<string, string>, sync?: boolean }) => string {
    return (params) => {
...
type MakeActionContextBackwardCompatibleParams = {
    context: ActionContext<BlockAuthProperty, InputPropertyMap>;
