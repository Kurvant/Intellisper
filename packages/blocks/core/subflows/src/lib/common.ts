import { FlowTriggerType, isNil, PopulatedFlow } from "@intelblocks/shared";
import { FlowsContext, ListFlowsContextParams } from "@intelblocks/blocks-framework";
...
export async function listFlowsWithSubflowTrigger({
    ...
    const flows = allFlows.filter(
        (flow) =>
            flow.version.trigger.type === FlowTriggerType.BLOCK &&
            flow.version.trigger.settings.blockName ==
            '@intelblocks/block-subflows'
    );
    return flows;
}
