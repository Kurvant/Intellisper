...
export const descriptPublishProjectAction = createAction({
  ...
  props: {
    ...
    media_type: Property.StaticDropdown({
      ...
      description:
        'Choose whether to export the project as a video or audio file.',
      required: false,
      defaultValue: 'Video',
      options: {
        options: [
    ...
    resolution: Property.StaticDropdown({
      ...
      description:
        'Resolution for the exported video. Only applies when Output Type is Video.',
      required: false,
      defaultValue: '1080p',
      options: {
        options: [
    ...
    access_level: Property.StaticDropdown({
      ...
      description:
        "Who can view the published share page. Leave blank to use your Drive's default. Note: requesting a level your Drive disallows (e.g. Public when search-engine indexing is off) will return an error.",
      required: false,
      options: {
        options: [
    ...
    callback_url: Property.ShortText({
      ...
      description:
        'Optional webhook URL. When the publish job finishes or fails, Descript will POST the job status to this URL (same payload as Get Job Status). The result includes share_url and download_url once complete.',
      required: false,
    }),
  },
  async run(context) {
