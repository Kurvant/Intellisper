...
import { IbFile, DynamicPropsValue, Property, createAction } from '@intelblocks/blocks-framework';
import { defaultLLM, getGeminiModelOptions } from '../common/common';
import { GenerateContentParameters, GoogleGenAI } from '@google/genai';
import { isEmpty, MarkdownVariant } from '@intelblocks/shared';
...
export const generateContentAction = createAction({
  audience: 'human',
	...
	async run({ auth, propsValue }) {
		...
		switch (toolType) {
			case 'file-search': {
				const { file, fileStoreName } = toolProperties as { file: IbFile; fileStoreName: string };
