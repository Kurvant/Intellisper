...
> = {
  ...
  items: async ({ auth, propsValue, lastFetchEpochMS }) => {
    ...
    if (lastFetchEpochMS && lastFetchEpochMS > 0) {
      queryParams['CompletedSince'] = new Date(lastFetchEpochMS).toISOString();
    }
    ...
    return items
      ...
      .map((item) => ({
        epochMilliSeconds: new Date(item.completedOn as string).getTime(),
        data: item,
      }));
  },
};

export const completedItemTrigger = createTrigger({
  auth: teamhoodAuth,
  name: 'completed_item',
  ...
  sampleData: {
    ...
    completed_on: '2026-01-02T00:00:00Z',
    description: null,
    budget: null,
    estimation: null,
