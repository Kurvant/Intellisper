...
export const listOrders = createAction({
  ...
  props: {
    ...
    returnAll: Property.Checkbox({ displayName: 'Return All', description: 'Retrieve all orders using pagination.', required: false, defaultValue: false }),
  ...
  async run(context) {
    ...
    if (context.propsValue.returnAll) {
      ...
      let hasMore = true;
      while (hasMore) {
        const response = await ninjapipeApiCall<{ data?: unknown[] }>({ auth, method: HttpMethod.GET, path: '/orders', queryParams: { ...qs, limit: String(limit), page: String(page) } });
    ...
    const response = await ninjapipeApiCall<{ data?: unknown[] }>({ auth, method: HttpMethod.GET, path: '/orders', queryParams: qs });
    const items = extractItems(response.body);
