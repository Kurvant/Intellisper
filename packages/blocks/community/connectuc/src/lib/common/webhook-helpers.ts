...
import { BaseContext, InputPropertyMap, BlockAuthProperty } from '@intelblocks/blocks-framework';
...
> {
    ...
    event?: string;
    events?: string[];
    context: BaseContext<BlockAuth, Props>;
}
...
export async function registerConnectUCWebhook<
    BlockAuth extends BlockAuthProperty | BlockAuthProperty[] | undefined,
    Props extends InputPropertyMap
>(params: RegisterWebhookParams<BlockAuth, Props>): Promise<WebhookResponse> {
...
export async function unregisterConnectUCWebhook<
    BlockAuth extends BlockAuthProperty | BlockAuthProperty[] | undefined,
    Props extends InputPropertyMap
>(params: UnregisterWebhookParams<BlockAuth, Props>): Promise<void> {
