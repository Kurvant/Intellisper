...
export const subscriptionPaymentSuccessTrigger = createTrigger({
  ...
  async onEnable(context) {
    ...
    await context.store?.put(createWebhookTriggerKey('subscription_payment_success'), {
  ...
  async onDisable(context) {
    const response: { webhookId: string } | null = await context.store?.get(createWebhookTriggerKey('subscription_payment_success'));
