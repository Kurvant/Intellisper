...
export const subscriptionExpiredTrigger = createTrigger({
  ...
  async onEnable(context) {
    ...
    await context.store?.put(createWebhookTriggerKey('subscription_expired'), {
  ...
  async onDisable(context) {
    const response: { webhookId: string } | null = await context.store?.get(createWebhookTriggerKey('subscription_expired'));
  ...
  sampleData: {
    ...
    data: {
      ...
      attributes: {
        ...
        updated_at: '2021-09-17T09:45:53.000000Z',
        test_mode: false,
        pause_mode: null,
