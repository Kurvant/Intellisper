...
export const subscriptionPaymentRecoveredTrigger = createTrigger({
  ...
  async onEnable(context) {
    ...
    await context.store?.put(createWebhookTriggerKey('subscription_payment_recovered'), {
  ...
  async onDisable(context) {
    const response: { webhookId: string } | null = await context.store?.get(createWebhookTriggerKey('subscription_payment_recovered'));
  ...
  sampleData: {
    ...
    data: {
      ...
      attributes: {
        ...
        paid_at: '2021-09-17T10:45:53.000000Z'
      }
    }
  }
});
