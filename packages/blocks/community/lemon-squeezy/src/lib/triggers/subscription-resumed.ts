...
export const subscriptionResumedTrigger = createTrigger({
  ...
  async onEnable(context) {
    ...
    await context.store?.put(createWebhookTriggerKey('subscription_resumed'), {
  ...
  async onDisable(context) {
    const response: { webhookId: string } | null = await context.store?.get(createWebhookTriggerKey('subscription_resumed'));
