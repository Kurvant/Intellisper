...
export const licenseKeyUpdatedTrigger = createTrigger({
  ...
  async onEnable(context) {
    ...
    await context.store?.put(createWebhookTriggerKey('license_key_updated'), {
  ...
  async onDisable(context) {
    const response: { webhookId: string } | null = await context.store?.get(createWebhookTriggerKey('license_key_updated'));
