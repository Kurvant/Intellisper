...
export const snowflakeOAuth2Auth = BlockAuth.OAuth2({
  displayName: 'OAuth',
...
export const snowflakeCustomAuth = BlockAuth.CustomAuth({
...
**3. Paste the private key** — copy the full contents of \`rsa_key.p8\` (including the \`-----BEGIN PRIVATE KEY-----\` header and footer) into the **Private Key** field below.`,
  props: {
    ...
    privateKey: BlockAuth.SecretText({
    ...
    privateKeyPassphrase: BlockAuth.SecretText({
