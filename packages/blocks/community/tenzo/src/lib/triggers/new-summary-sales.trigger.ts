...
interface SummarySalesTriggerProps {
  location_id?: string;
  area_id?: string;
}
...
const polling: Polling<TenzoAuthValue, SummarySalesTriggerProps> = {
  strategy: DedupeStrategy.TIMEBASED,
  items: async ({ auth, propsValue, lastFetchEpochMS }) => {
    const { location_id, area_id } = propsValue;

    console.debug('Polling for new summary sales', { location_id, area_id, lastFetchEpochMS });
    ...
    try {
      ...
      console.error('Error polling for summary sales:', error);
      return [];
    }
  },
};

export const newSummarySalesTrigger = createTrigger({
  auth: tenzoAuth,
