...
import { PartnerCreateResponse } from '../../../models/responses';
...
'RO': ['AB', 'AR', 'AG', 'BC', 'BH', 'BN', 'BT', 'BV', 'BR', 'B', 'BZ', 'CL', 'CS', 'CT', 'CV', 'DB', 'DJ', 'GL', 'GR', 'GJ', 'HR', 'HD', 'IL', 'IS', 'IF', 'MM', 'MH', 'MS', 'NT', 'OT', 'PH', 'SM', 'SJ', 'SB', 'SV', 'TR', 'TM', 'TL', 'VS', 'VL', 'VN']
  ...
  async run(context): Promise<PartnerCreateResponse> {
    try {
      const url = processPathParameters('/public-api/resources/partners/v2.0', context.propsValue);
      
      const queryParams = prepareQueryParams(context.propsValue, []);
      
      const body = prepareRequestBody(context.propsValue,
        ['businessName', 'name', 'regNo', 'vatNo', 'address', 'postcode', 'city', 'country', 'region', 'state', 'contactDetails', 'notifications', 'monthlyPlatformFee', 'options', 'corporateBilling', 'externalId', 'bankDetails']
      );
