...
import { ChargePointCreateResponse } from '../../../models/responses';
...
export const chargePointCreateAction = createAction({
  ...
  props: {
    ...
    subscription__required: Property.StaticDropdown({
      ...
      description:
        'Only for personal charge points. Require an active subscription to use the charge point.',
      required: false,
      options: {
        options: [
    ...
    subscription__planIds: Property.Array({
      ...
      description:
        'Only for personal charge points. List of subscription plans for any of which the user should have an active subscription, to be able to use the charge point.',
      required: false,
    }),
    ...
    partner__corporateBillingAsDefault: Property.StaticDropdown({
      ...
      description:
        "The partner's corporate billing would be used as a payment method by default, when a user who is invited to use the partner's corporate billing starts a session on the charge point.",
      required: false,
      options: {
        options: [
          { label: 'true', value: 'true' },
    ...
    partner__accessType: Property.StaticDropdown({
      ...
      description:
        '`private_view_private_use` - only users invited by the partner with the option to access private charge points could view this charge point on the map and use it.',
      required: false,
      options: {
        options: [
          {
    ...
    ocppConnectedChargePointId: Property.Number({
    ...
    communicationMode: Property.StaticDropdown({
  ...
  async run(context): Promise<ChargePointCreateResponse> {
    try {
