...
export const tariffUpdateAction = createAction({
  ...
  type: Property.StaticDropdown({
    ...
    options: {
      ...
      { label: 'duration+energy time of day', value: 'duration+energy time of day' },
      ...
      { label: 'optimised dynamic pricing', value: 'optimised dynamic pricing' }
      ],
    },
  }),
  ...
  async run(context): Promise<TariffUpdateResponse> {
    try {
      ...
      const body = prepareRequestBody(context.propsValue,
        ['name', 'type', 'dayTariffStart', 'nightTariffStart', 'pricing', 'stopSession', 'restrictions', 'partner', 'display', 'integrationId']
      );
