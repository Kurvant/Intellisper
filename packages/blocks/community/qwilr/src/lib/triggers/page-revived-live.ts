...
export const pageRevivedLiveTrigger = createTrigger({
  auth: qwilrAuth,
  ...
  async onEnable(context) {
    ...
    await context.store?.put('_page_revived_live_trigger', {
      subscriptionId: webhookData.id,
    });
  },
  async onDisable(context) {
    const response: { subscriptionId: string } | null = await context.store?.get('_page_revived_live_trigger');
