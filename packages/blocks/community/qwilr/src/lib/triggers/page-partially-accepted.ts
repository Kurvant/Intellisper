...
export const pagePartiallyAcceptedTrigger = createTrigger({
  auth: qwilrAuth,
  ...
  async onEnable(context) {
    ...
    await context.store?.put('_page_partially_accepted_trigger', {
      subscriptionId: webhookData.id,
    });
  },
  async onDisable(context) {
    const response: { subscriptionId: string } | null = await context.store?.get('_page_partially_accepted_trigger');
