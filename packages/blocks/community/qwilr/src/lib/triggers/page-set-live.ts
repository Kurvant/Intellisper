...
export const pageSetLiveTrigger = createTrigger({
  auth: qwilrAuth,
  name: 'page_set_live',
  ...
  async onEnable(context) {
    ...
    await context.store?.put('_page_set_live_trigger', {
      subscriptionId: webhookData.id,
    });
  },
  async onDisable(context) {
    const response: { subscriptionId: string } | null = await context.store?.get('_page_set_live_trigger');
