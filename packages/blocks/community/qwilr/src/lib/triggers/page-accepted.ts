...
export const pageAcceptedTrigger = createTrigger({
  auth: qwilrAuth,
  name: 'page_accepted',
  ...
  async onEnable(context) {
    ...
    await context.store?.put('_page_accepted_trigger', {
      subscriptionId: webhookData.id,
    });
  },
  async onDisable(context) {
    const response: { subscriptionId: string } | null = await context.store?.get('_page_accepted_trigger');
