...
import { AgentKnowledgeBaseTool, AgentMcpTool, AgentOutputField, AgentTaskStatus, AgentTool, AgentToolType, buildAuthHeaders, isNil, isString, KnowledgeBaseSourceType, McpProtocol, mcpToolNameUtils, TASK_COMPLETION_TOOL_NAME } from "@intelblocks/shared";
...
export async function constructAgentTools(
  params: ConstructAgentToolParams
    ...
    const agentBlockTools = await context.agent.tools({
      tools: agentTools.filter(tool => tool.type === AgentToolType.BLOCK),
      model: model,
    });
