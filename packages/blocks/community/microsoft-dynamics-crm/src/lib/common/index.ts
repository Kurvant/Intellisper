...
export function makeClient(auth: BlockPropValueSchema<typeof dynamicsCRMAuth>) {
...
export const DynamicsCRMCommon = {
  entityType: (description: string) =>
    ...
    options: async ({ auth, entityType }) => {
      ...
      const client = makeClient(
        auth as BlockPropValueSchema<typeof dynamicsCRMAuth>
      );
      ...
      const authValue = auth as BlockPropValueSchema<typeof dynamicsCRMAuth>;
