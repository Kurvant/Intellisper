import { INTELLISPER_CHAT_TIERS } from '@intelblocks/shared';
...
export function ChatModelSelector({
  ...
  useEffect(() => {
    if (!open) return;
    const idx = INTELLISPER_CHAT_TIERS.findIndex(
    ...
    (e: React.KeyboardEvent) => {
      if (e.key === 'ArrowDown') {
        ...
        setFocusedIndex((prev) =>
          prev < INTELLISPER_CHAT_TIERS.length - 1 ? prev + 1 : 0,
        );
      } else if (e.key === 'ArrowUp') {
        e.preventDefault();
        setFocusedIndex((prev) =>
          prev > 0 ? prev - 1 : INTELLISPER_CHAT_TIERS.length - 1,
        );
      } else if (e.key === 'Enter') {
        e.preventDefault();
        const tier = INTELLISPER_CHAT_TIERS[focusedIndex];
        if (tier) {
  ...
  return (
    <Popover open={open} onOpenChange={setOpen}>
      ...
      >
        ...
        >
          <div className="py-1">
            {INTELLISPER_CHAT_TIERS.map((tier, index) => {
