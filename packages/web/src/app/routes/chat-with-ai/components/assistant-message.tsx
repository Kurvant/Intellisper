import { BatchProgressData } from '@intelblocks/shared';
  ...
  const {
    ...
    for (let i = 0; i < message.parts.length; i++) {
      ...
      if (p.type === 'text' && p.text.length > 0) {
        ...
        } else if (toolName === 'ib_execute_action') {
    ...
    for (const block of result) {
      ...
      for (const step of block.steps) {
        ...
        if (toolName !== 'ib_execute_action') continue;
...
function DisplayToolCard({
  ...
  switch (toolName) {
    case 'ib_show_connection_required': {
    ...
    case 'ib_show_connection_picker': {
      const selectedLabel =
