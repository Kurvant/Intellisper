...
interface FlowChatProps {
  ...
  onError?: (error: IbErrorParams | null) => void;
...
export function FlowChat({
  ...
  const [sendingError, setSendingError] = useState<IbErrorParams | null>(null);
  const [selectedImage, setSelectedImage] = useState<string | null>(null);
  const [imageDialogOpen, setImageDialogOpen] = useState(false);

  const botName =
    chatUI?.props.botName ?? `${chatUI?.platformName ?? 'Intellisper'} Bot`;
  ...
  const { mutate: sendMessage, isPending: isSending } = useMutation({
    ...
    onError: (error: AxiosError) => {
      const errorData = error.response?.data as IbErrorParams;
