...
type ApplyTagsProps = {
  selectedBlocks: BlockMetadataModelSummary[];
  onApplyTags: () => void;
};

const ApplyTags = ({ selectedBlocks, onApplyTags }: ApplyTagsProps) => {
  const { data: tags = [] } = blocksTagQueries.useTags();
    ...
    setSelectedTags(
      new Set(
        tags
          .map((tag) => tag.name)
          .filter((tag) =>
            selectedBlocks.every((block) => block.tags?.includes(tag)),
          ),
      ),
    );
  }, [selectedBlocks, tags]);

  const { mutate: applyTags } = blocksTagMutations.useApplyTags({
    onSuccess: () => onApplyTags(),
  });

  const { mutate: deleteTag } = blocksTagMutations.useDeleteTag({
    onSuccess: () => onApplyTags(),
  });
                  ...
                  {tagOptions.map((option) => {
                    ...
                    const isIndeterminate =
                      selectedBlocks.some((block) =>
                        block.tags?.includes(option.value),
                      ) &&
                    ...
                    return (
                      ...
                      >
                        ...
                        <ConfirmationDeleteDialog
                          title={t('Delete Tag')}
                          message={t(
                            'Are you sure you want to delete the tag "{tagName}"? It will be removed from all blocks.',
            ...
            <CommandGroup>
              <CommandItem
                ...
                onSelect={() => {
                  ...
                  applyTags({
                    blocksName: selectedBlocks.map((block) => block.name),
