...
const notifyVendorPostAuthentication = () => {
  ...
  const configurationFinishedEvent: IntellisperClientConfigurationFinished = {
...
const handleVendorNavigation = ({ projectId }: { projectId: string }) => {
  const handleVendorRouteChange = (
    event: MessageEvent<IntellisperVendorRouteChanged>,
  ) => {
    if (
      event.source === parentWindow &&
      event.data.type === IntellisperVendorEventName.VENDOR_ROUTE_CHANGED
    ) {
...
const EmbedPage = React.memo(() => {
  ...
  const initState = (event: MessageEvent<IntellisperVendorInit>) => {
    if (
      event.source === parentWindow &&
      event.data.type === IntellisperVendorEventName.VENDOR_INIT
    ) {
      if (event.data.data.jwtToken) {
        ...
        mutateAsync(
          ...
          },
          {
            ...
            onError: (error) => {
              const errorEvent: IntellisperClientAuthenticationFailed = {
  ...
  useEffectOnce(() => {
    const event: IntellisperClientInit = {
