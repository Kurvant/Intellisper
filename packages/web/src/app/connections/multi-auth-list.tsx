...
export function MutliAuthList({
  ...
  const authItems: RadioGroupListItem<AuthListItem>[] = blockAuth.flatMap(
    (auth) => {
...
const getDisplayName = (auth: BlockAuthProperty): string => {
  if (
...
export type AuthListItem =
  | {
      authProperty: Exclude<BlockAuthProperty, OAuth2Property<any>>;
      grantType: null;
      oauth2App: null;
    }
  | {
