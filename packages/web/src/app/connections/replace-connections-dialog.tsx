import { AppConnectionScope, PopulatedFlow } from '@intelblocks/shared';
...
const ReplaceConnectionsDialog = ({
  ...
  const { blocks, isLoading: blocksLoading } = blocksHooks.useBlocks({});
  ...
  const connectionBlockNames = new Set(
    connections?.data.map((conn) => conn.blockName),
  );
  ...
  const blocksOptions =
    blocks
      ?.filter(
        (block) =>
          block.name !== '@intelblocks/block-mcp' &&
          block.name !== '@intelblocks/block-webhook' &&
          connectionBlockNames.has(block.name),
      )
      .map((block) => ({
  ...
  const filteredConnections =
    connections?.data.filter((conn) => conn.blockName === selectedBlock) ?? [];
                  ...
                  <div className="flex flex-col gap-2">
                    ...
                    <SearchableSelect
                      ...
                      valuesRendering={(value) => {
                        const block = blocks?.find((p) => p.name === value);
                        return (
