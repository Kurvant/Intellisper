...
const shortcutHandler = (
  ...
  // `key` is absent on keydown events the browser synthesises without a physical
  // key — IME composition, autofill, and focus changes across a popup window.
  if (isNil(event.key)) {
    return;
  }
  ...
  const shortcutActivated = Object.entries(CanvasShortcuts).find(
    ([_, shortcut]) =>
      shortcut.shortcutKey?.toLowerCase() === eventKey &&
      !!(
