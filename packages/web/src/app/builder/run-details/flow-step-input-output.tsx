...
export const FlowStepInputOutput = () => {
  ...
  const slicedOutputRef = isSlicedOutput
    ...
    : undefined;
  const friendlyError = tryParseFriendlyBlockError(
    selectedStepOutput?.errorMessage,
  );
  const stepBlockName =
    selectedStep?.type === FlowActionType.BLOCK ||
    selectedStep?.type === FlowTriggerType.BLOCK
      ? selectedStep.settings.blockName
      : undefined;
  const stepBlockVersion =
    selectedStep?.type === FlowActionType.BLOCK ||
    selectedStep?.type === FlowTriggerType.BLOCK
      ? selectedStep.settings.blockVersion
      : undefined;
  const { blockModel } = blocksHooks.useBlock({
    name: stepBlockName ?? '',
    version: stepBlockVersion,
    enabled: !isNil(stepBlockName),
  });
  ...
  const stepKind: 'action' | 'trigger' =
    selectedStep.type === FlowTriggerType.BLOCK ? 'trigger' : 'action';
  const stepName =
    selectedStep.type === FlowActionType.BLOCK
  ...
  const stepInput =
    ...
    selectedStep.type === FlowTriggerType.BLOCK
      ...
      : undefined;
  const blockSchema =
    blockModel?.actions[stepName ?? '']?.outputSchema ??
    blockModel?.triggers[stepName ?? '']?.outputSchema ??
    null;
  const explanationContext: ErrorExplanationContext = {
    blockName: stepBlockName,
    blockVersion: stepBlockVersion,
    blockDisplayName: blockModel?.displayName,
    blockAuthType: stepPropertiesSnapshotUtils.findAuthType(blockModel),
    stepKind,
    stepName,
