...
const TestTriggerSection = React.memo(
  ({ isSaving, flowVersionId, flowId }: TestTriggerSectionProps) => {
    ...
    if (!runner || !currentStep || currentStep.type !== FlowTriggerType.BLOCK) {
      return null;
    }
    ...
    if (isBlockLoading || isNil(testType)) {
      return (
    ...
    const explanationContext: ErrorExplanationContext = {
      blockName: currentStep.settings.blockName,
      blockVersion: currentStep.settings.blockVersion,
        ...
        case 'simulation':
          return t('testPieceWebhookTriggerNote', {
            ...
            triggerName: triggerName
              ? blockModel?.triggers[triggerName]?.displayName
              : undefined,
          });
        case 'webhook':
          return (
          ...
          <>
            {showSampleDataViewer && (
              <TestSampleDataViewer
                ...
                isSaving={isSaving}
                blockSchema={
                  blockModel?.triggers[triggerName ?? '']?.outputSchema ?? null
                }
