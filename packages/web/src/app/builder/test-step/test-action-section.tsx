...
const TestStepSectionImplementation = React.memo(
  ({
    ...
    const blockName =
      currentStep.type === FlowActionType.BLOCK
        ? currentStep.settings.blockName
        : undefined;
    const blockVersion =
      currentStep.type === FlowActionType.BLOCK
        ? currentStep.settings.blockVersion
        : undefined;
    const { blockModel } = blocksHooks.useBlock({
      name: blockName ?? '',
      version: blockVersion,
      enabled: !isNil(blockName),
    });
      ...
      <>
        ...
        {(sampleDataExists || isTesting) && (
          <TestSampleDataViewer
            ...
            blockDisplayName={blockModel?.displayName}
            blockSchema={
              blockModel?.actions[stepName ?? '']?.outputSchema ?? null
            }
            onCancelTesting={() => {
