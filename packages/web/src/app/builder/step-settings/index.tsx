...
import { BlockSettings } from './piece-settings';
import { RouterSettings } from './router-settings';
import { StepNavigationButtons } from './step-navigation-buttons';
import { useStepSettingsContext } from './step-settings-context';
import { UpdateBlockVersionDialog } from './update-piece-version-dialog/update-piece-version-dialog';

const StepSettingsContainer = () => {
  const { selectedStep, blockModel, formSchema } = useStepSettingsContext();
  ...
  const form = useForm<FlowAction | FlowTrigger>({
    ...
    resolver: async (values, context, options) => {
      ...
      if (
        ...
        (isNil(blockModel) &&
          (cleanedNewValues.type === FlowActionType.BLOCK ||
            cleanedNewValues.type === FlowTriggerType.BLOCK))
      ) {
        return result;
      }
      if (
      ...
      if (cleanedNewValues.type === FlowTriggerType.BLOCK) {
        applyOperation({
  ...
  const isManualTrigger =
    modifiedStep.type === FlowTriggerType.BLOCK &&
    blockSelectorUtils.isManualTrigger({
      blockName: modifiedStep.settings.blockName,
  ...
  const showActionErrorHandlingForm =
    [FlowActionType.CODE, FlowActionType.BLOCK].includes(
      modifiedStep.type as FlowActionType,
    ) && !isNil(stepMetadata);

  const runAgentStep =
    modifiedStep.settings.blockName === '@intelblocks/block-ai' &&
  ...
  const settingsForm = (
    <ScrollArea className="h-full">
      ...
      >
        ...
        {modifiedStep.type === FlowActionType.BLOCK &&
          runAgentStep &&
          modifiedStep && (
            <AgentSettings
              step={modifiedStep}
        ...
        {modifiedStep.type === FlowTriggerType.BLOCK && modifiedStep && (
          <BlockSettings
            step={modifiedStep}
            flowId={flowVersion.flowId}
            readonly={readonly}
          ...
          <SidebarHeader
            ...
            actions={
              <div className="flex items-center gap-1">
                {isBlockMetadata(stepMetadata) &&
                  stepMetadata.blockVersion &&
                  (modifiedStep.type === FlowActionType.BLOCK ||
                    modifiedStep.type === FlowTriggerType.BLOCK) && (
                    <BlockVersionInHeader
                      step={modifiedStep}
                      blockVersion={stepMetadata.blockVersion}
                      readonly={readonly}
                    />
                  )}
...
type BlockVersionInHeaderProps = {
  step: FlowAction | FlowTrigger;
  blockVersion: string;
  readonly: boolean;
};

const BlockVersionInHeader = ({
  step,
  blockVersion,
  readonly,
}: BlockVersionInHeaderProps) => {
  const exactVersion = flowBlockUtil.getExactVersion(blockVersion);
  const showSwitcher =
    !readonly &&
  ...
  return (
    <div className="flex items-center gap-1 shrink-0">
      ...
      {showSwitcher && (
        <UpdateBlockVersionDialog step={step} currentVersion={exactVersion} />
      )}
    </div>
  );
};
