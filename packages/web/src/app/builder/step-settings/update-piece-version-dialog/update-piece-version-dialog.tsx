...
import { UpgradeBlockVersionContent } from './upgrade-piece-version-dialog';

type DialogView = 'upgrade' | 'advanced';

const UpdateBlockVersionDialog: React.FC<UpdateBlockVersionDialogProps> = ({
  step,
  currentVersion,
}) => {
  const [view, setView] = useState<DialogView | null>(null);
  const blockName = step.settings.blockName;
  const { blockVersions, isLoading } = blocksHooks.useBlockVersions(blockName);
          ...
          <DialogHeader className="mb-0">
            <DialogTitle>
              {view === 'upgrade'
                ...
                : t('Update Block Version')}
            </DialogTitle>
          </DialogHeader>
          {view === 'upgrade' && latestVersion && (
            <UpgradeBlockVersionContent
              key="upgrade"
              step={step}
...
export { UpdateBlockVersionDialog };

type UpdateBlockVersionDialogProps = {
...
const AdvancedForm: React.FC<AdvancedFormProps> = ({
  ...
  const patchVersions = (blockVersions ?? []).filter((p) => {
  ...
  const visibleVersions = showAllVersions ? blockVersions ?? [] : patchVersions;
  ...
  const { mutate: applyVersionChange, isPending: isApplyPending } = useMutation(
    {
      mutationFn: async ({ version }: FormSchema) => {
        await changeVersionUtils.applyBlockVersionChange({
          step,
