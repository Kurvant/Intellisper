...
function ConnectionSelect(params: ConnectionSelectProps) {
  ...
  const { blockModel: blockWithCorrectVersion, isLoading: isLoadingBlock } =
    blocksHooks.useBlock({
      name: params.block.name,
      version: reconnectConnection?.blockVersion ?? params.block.version,
    });
  ...
  const {
    ...
    blockAuth: params.block.auth,
    extraKeys: [params.block.name, authenticationSession.getProjectId()!],
    staleTime: 0,
  });
  ...
  return (
    <FormField
      ...
      render={({ field }) => (
        <>
          {(isLoadingConnections || !blockWithCorrectVersion) && (
