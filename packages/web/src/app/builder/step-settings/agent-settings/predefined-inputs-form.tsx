...
export const PredefinedInputsForm = () => {
  ...
  const selectedBlock = blocks?.find((p) => p.name === block?.blockName);
  ...
  useEffect(() => {
    const subscription = form.watch((values, { name }) => {
      ...
      const currentPredefined =
        useBlockToolsDialogStore.getState().predefinedInputs;
  ...
  const blockHasAuth = requireAuth && selectedBlock?.auth;
  return (
    <Form {...form}>
      <ScrollArea className="h-full">
        ...
        <div className="space-y-6 p-4">
          {blockHasAuth && !isNil(selectedBlock) && (
            <ConnectionDropdown
              block={selectedBlock}
