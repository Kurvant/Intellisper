import { AgentToolType, AIProviderName } from '@intelblocks/shared';
import type {
...
import {
  ...
  AgentBlockToolComponent,
  AgentFlowToolDialog,
  AgentMcpDialog,
  KnowledgeBaseSection,
} from '@/features/agents';

import { AgentBlockDialog } from './piece-tool-dialog';
...
export const AgentTools = ({
  ...
  const blockToToolMap = tools
    .filter((tool) => tool.type === AgentToolType.BLOCK)
    .reduce<Record<string, AgentBlockTool[]>>((acc, tool) => {
      const key = tool.blockMetadata?.blockName;
  ...
  return (
    <div>
      ...
      <div className="mt-2">
        {flowTools.length +
          mcpTools.length +
          Object.keys(blockToToolMap).length >
        0 ? (
          <>
            <Accordion
              type="single"
            ...
            >
              {Object.entries(blockToToolMap).map(([blockName, tools]) => (
                <AgentBlockToolComponent
                  key={blockName}
                  disabled={disabled}
                  tools={tools}
      ...
      <AgentBlockDialog tools={tools} onToolsUpdate={onToolsUpdate} />
