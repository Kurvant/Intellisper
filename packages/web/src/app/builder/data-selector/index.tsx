...
const DataSelector = ({ parentHeight, parentWidth }: DataSelectorProps) => {
  ...
  const blockQueries = useQueries({
    queries: blockPairs.map(({ blockName, blockVersion }) => ({
      queryKey: ['piece', blockName, blockVersion],
      queryFn: () =>
        blocksApi.get({
          name: blockName,
  ...
  const schemaMap = useMemo<Record<string, OutputSchema | null>>(() => {
    ...
    blockPairs.forEach(({ stepName, stepKey }, idx) => {
      const block = blockQueries[idx]?.data as BlockMetadataModel | undefined;
      result[stepName] =
        block?.triggers?.[stepKey]?.outputSchema ??
        block?.actions?.[stepKey]?.outputSchema ??
        null;
    });
    return result;
  }, [blockPairs, blockQueries]);
