import { FriendlyBlockError, isNil } from '@intelblocks/shared';
...
const buildContextBlock = ({
  ...
  const lines: string[] = [];
  if (context.blockDisplayName || context.blockName) {
    const version = context.blockVersion ? ` v${context.blockVersion}` : '';
    lines.push(
      `Block: ${context.blockDisplayName ?? context.blockName}${version}`,
    );
  }
  if (context.blockAuthType) {
    lines.push(`Block auth type: ${context.blockAuthType}`);
  }
