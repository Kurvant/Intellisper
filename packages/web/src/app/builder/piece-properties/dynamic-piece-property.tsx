...
import { blocksHooks, formUtils } from '@/features/pieces';
...
const DynamicPropertiesImplementation = React.memo(
  (props: DynamicPropertiesProps) => {
    ...
    const [propertyMap, setPropertyMap] = useState<
      BlockPropertyMap | undefined
    >(undefined);
    const propertyPrefix =
      props.placedInside === 'stepSettings' ? 'settings.input' : '';
    const { mutate, isPending } =
      blocksHooks.useBlockOptions<PropertyType.DYNAMIC>({
        onMutate: () => {
...
type DynamicPropertiesProps = {
  ...
  updateFormSchema:
    | ((key: string, newFieldSchema: BlockPropertyMap) => void)
    | null;
