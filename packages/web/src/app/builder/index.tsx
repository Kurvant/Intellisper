...
const BuilderPage = () => {
  ...
  useEffect(() => {
    if (!isSplitForBlock || !isDraggingHandle) return;
  ...
  const { blockModel, refetch: refetchBlock } =
    blocksHooks.useBlockModelForStepSettings({
      name: selectedStep?.settings.blockName,
      version: selectedStep?.settings.blockVersion,
      enabled:
        selectedStep?.type === FlowActionType.BLOCK ||
        selectedStep?.type === FlowTriggerType.BLOCK,
    });
  flowCanvasHooks.useSetSocketListener(refetchBlock);
              ...
              selectedStep && (
                <StepSettingsProvider
                  ...
                  key={constructContainerKey({
                    ...
                    step: selectedStep,
                    hasBlockModelLoaded: !!blockModel,
                  })}
                >
...
function constructContainerKey({
  ...
  const blockName =
    step?.type === FlowTriggerType.BLOCK || step?.type === FlowActionType.BLOCK
      ? step?.settings.blockName
      : undefined;
  const blockVersion =
    step?.type === FlowTriggerType.BLOCK || step?.type === FlowActionType.BLOCK
      ? step?.settings.blockVersion
      : undefined;
  ...
  return `${flowVersionId}-${stepName ?? ''}-${triggerOrActionName ?? ''}-${
    blockName ?? ''
  }-${blockVersion ?? ''}-${'skipped-' + !!isSkipped}-${
    hasBlockModelLoaded ? 'loaded' : 'not-loaded'
  }`;
}
