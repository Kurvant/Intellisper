...
import { BlockSelector } from '@/app/builder/pieces-selector';
...
import { IbStepNode } from '../../utils/types';
...
import { IbStepNodeSkippedStatus } from './step-node-skipped-status';
import { IbStepNodeStatusInDraft } from './step-node-status-in-draft';
import { IbStepNodeStatusInRun } from './step-node-status-in-run';
import { TriggerWidget } from './trigger-widget';

const IbStepCanvasNode = React.memo(
  ({ data: { step } }: NodeProps & Omit<IbStepNode, 'position'>) => {
    const [
      selectStepByName,
      isSelected,
      isDragging,
      ...
      state.openedBlockSelectorStepNameOrAddButtonId === step.name,
    ...
    const handleStepClick = (
      ...
      if (step.type === FlowTriggerType.EMPTY) {
        setOpenedBlockSelectorStepNameOrAddButtonId(step.name);
      }
      if (preventDefault) {
        e.stopPropagation();
        e.preventDefault();
    ...
    const stepNodeDivAttributes = isBlockSelectorOpened ? {} : attributes;
    const stepNodeDivListeners = isBlockSelectorOpened ? {} : listeners;
    ...
    return (
      <div
        ...
        key={step.name}
        ref={isBlockSelectorOpened ? null : setNodeRef}
      ...
      >
        ...
        <IbStepNodeStatusInRun stepName={step.name} />
        <IbStepNodeSkippedStatus stepName={step.name} />
        <IbStepNodeStatusInDraft stepName={step.name} />
        <div
        ...
        >
          {!isDragging && (
            <BlockSelector
              operation={{
                type: getBlockSelectorOperationType(step),
                stepName: step.name,
              }}
              id={step.name}
              openSelectorOnClick={false}
              stepToReplaceBlockDisplayName={stepMetadata?.displayName}
            >
              {isHorizontal ? (
                <div
                  className="flex items-center justify-center h-full w-full"
                  onClick={(e) => {
                    if (!isBlockSelectorOpened) {
                      handleStepClick(e);
                    }
                  }}
                >
...
IbStepCanvasNode.displayName = 'IbStepCanvasNode';
export { IbStepCanvasNode };

function getBlockSelectorOperationType(step: Step) {
