import { OAuth2Property, OAuth2Props } from '@intelblocks/blocks-framework';
import {
  AppConnectionType,
...
function getPredefinedOAuth2App(
  ...
  const blockOAuth2Apps = blocksOAuth2AppsMap[blockName];
  if (isNil(blockOAuth2Apps)) {
    return null;
  }
  if (blockOAuth2Apps.platformOAuth2App) {
    return blockOAuth2Apps.platformOAuth2App;
  }
  if (blockOAuth2Apps.cloudOAuth2App) {
    return blockOAuth2Apps.cloudOAuth2App;
  }
  return null;
}
...
export type BlocksOAuth2AppsMap = Record<
  string,
  | {
