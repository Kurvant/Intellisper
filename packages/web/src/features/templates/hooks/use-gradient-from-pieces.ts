...
import { extractBlockNamesAndCoreMetadata } from '@/features/pieces/utils/step-utils';
...
export const useGradientFromBlocks = (
  ...
  const uniqueMetadata: StepMetadata[] = useMemo(() => {
    ...
    const allMetadata = [...coreMetadata, ...blockMetadata];
  ...
  return { gradient, blocksMetadata: uniqueMetadata };
};
