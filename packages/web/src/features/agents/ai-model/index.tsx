...
const INTELLISPER_PROVIDER_CONFIG = {
...
const ALL_PROVIDERS = [...SUPPORTED_AI_PROVIDERS, INTELLISPER_PROVIDER_CONFIG];
...
export function AIModelSelector({
  ...
  const intellisperProvider = React.useMemo(
    () => providers.find((p) => p.provider === AIProviderName.INTELLISPER),
    [providers],
  );

  const sortedProviders = React.useMemo(() => {
    return [...providers].sort((a, b) => {
      if (a.provider === AIProviderName.INTELLISPER) return -1;
      if (b.provider === AIProviderName.INTELLISPER) return 1;
      return 0;
    });
  }, [providers]);
  ...
  }, [providers, providersLoading, selectedProvider, intellisperProvider]);
