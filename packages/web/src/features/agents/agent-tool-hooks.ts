import { ToolCallType, type ToolCallContentBlock } from '@intelblocks/shared';
import { useQuery } from '@tanstack/react-query';

import { blocksApi } from '../pieces/api/pieces-api';
    ...
    return useQuery<ToolMetadata, Error>({
      ...
      queryFn: async () => {
        switch (contentBlock.toolCallType) {
          case ToolCallType.BLOCK: {
            const block = await blocksApi.get({
              name: contentBlock.blockName,
              version: contentBlock.blockVersion,
            });
            const actionMetadata = block.actions[contentBlock.actionName];
            return {
              displayName:
