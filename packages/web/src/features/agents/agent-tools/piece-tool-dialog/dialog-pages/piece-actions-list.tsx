...
export const BlockActionsList: React.FC<BlockActionsDialogProps> = ({
  tools,
}) => {
  const [searchQuery, setSearchQuery] = useState('');
  const [debouncedQuery] = useDebounce(searchQuery, 200);
  const { handleActionSelect, selectedBlock } = useBlockToolsDialogStore();
  ...
  const fuse = useMemo(() => {
    if (isNil(selectedBlock) || isNil(selectedBlock.suggestedActions))
      return null;

    return new Fuse(selectedBlock.suggestedActions, {
      keys: [
  ...
  }, [selectedBlock?.suggestedActions]);

  const filteredActions = useMemo(() => {
    if (!debouncedQuery.trim() || isNil(fuse))
      return selectedBlock?.suggestedActions || [];

    return fuse.search(debouncedQuery).map((r) => r.item);
  }, [debouncedQuery, fuse, selectedBlock?.suggestedActions]);
