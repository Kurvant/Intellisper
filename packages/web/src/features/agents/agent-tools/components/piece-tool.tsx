import { AgentBlockTool, mcpToolNameUtils } from '@intelblocks/shared';
...
type AgentBlockToolProps = {
  disabled?: boolean;
  tools: AgentBlockTool[];
  removeTool: (toolName: string) => void;
};

export const AgentBlockToolComponent = ({
  disabled,
  tools,
  removeTool,
}: AgentBlockToolProps) => {
  const { openAddBlockToolDialog } = useBlockToolsDialogStore();
  ...
  const blockMetadata = blocksMetadata?.find(
    (p) => p.blockName === tools[0].blockMetadata.blockName,
  );
  ...
  const handleEditTool = (tool: AgentBlockTool) => {
    openAddBlockToolDialog({ page: 'action-inputs', tool });
  };

  return (
    <AccordionItem
      value={blockMetadata.blockName}
    ...
    >
        ...
        <div className="flex w-full items-center justify-between">
          <div className="flex items-center gap-3">
            <div className="h-8 w-8 rounded-md bg-muted flex items-center justify-center">
              {blockMetadata.logoUrl ? (
                <img
                  src={blockMetadata.logoUrl}
                  alt={blockMetadata.displayName}
      ...
      <AccordionContent className="px-4 py-2">
        <div className="flex flex-wrap gap-2">
          {tools.map((tool) => {
            const toolName = blockMetadata.suggestedActions?.find(
              (action) =>
                mcpToolNameUtils.createBlockToolName(
