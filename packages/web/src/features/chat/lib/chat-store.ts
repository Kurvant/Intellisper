...
function selectActiveQuestions({
  ...
  if (
    activeTool &&
    chatPartUtils.getToolPartName(activeTool) === 'ib_show_questions'
  ) {
  ...
  const historyPart = chatPartUtils.findLastToolPart({
    message: lastAssistantMessage,
    predicate: (name, p) =>
      name === 'ib_show_questions' && p.state !== 'output-available',
  });
...
function selectHasBlockingCard({
  ...
  const part = chatPartUtils.findLastToolPart({
    ...
    predicate: (name, p) => {
      ...
      if (chatPartUtils.isDisplayTool(name) && name !== 'ib_show_quick_replies')
        return true;
