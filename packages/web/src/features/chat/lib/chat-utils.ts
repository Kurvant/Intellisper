...
function stripBlockPrefix(name: string): string {
  return name.replace(/^@intelblocks\/block-/, '');
}

function humanizeBlockName(raw: string): string {
  return formatUtils.convertEnumToHumanReadable(
    stripBlockPrefix(raw).replace(/-/g, '_'),
  );
}

const TOOL_LABELS: Record<string, { active: string; done: string }> = {
  ib_execute_action: { active: 'Running action', done: 'Ran action' },
  ib_discover_action_auth: {
    active: 'Checking connections',
    done: 'Checked connections',
  },
  ib_list_across_projects: {
  ...
  ib_build_flow: { active: 'Building automation', done: 'Built automation' },
  ib_create_flow: { active: 'Creating automation', done: 'Created automation' },
  ...
  ib_test_flow: { active: 'Testing automation', done: 'Tested automation' },
  ib_add_step: { active: 'Adding step', done: 'Added step' },
  ib_update_step: { active: 'Updating step', done: 'Updated step' },
  ib_update_trigger: {
    active: 'Updating starting event',
    done: 'Updated starting event',
  },
  ib_manage_notes: { active: 'Adding notes', done: 'Added notes' },
  ib_list_connections: {
    active: 'Checking connections',
    done: 'Checked connections',
  },
  ib_get_block_props: { active: 'Loading settings', done: 'Loaded settings' },
...
function extractToolContext({
  ...
  if (typeof input.blockName === 'string') {
    parts.push(humanizeBlockName(input.blockName));
  }
  ...
  if (
    ...
    parts.push(humanizeBlockName(input.settings.blockName));
  }
  ...
  switch (part.type) {
    ...
    case PersistedChatPartType.THINKING_STATUS:
      return {
        ...
        toolName: 'ib_update_thinking_status',
        title: 'ib_update_thinking_status',
...
function extractQuickRepliesFromHistory(messages: ChatUIMessage[]): string[] {
  ...
  for (let i = lastMessage.parts.length - 1; i >= 0; i--) {
    ...
    if (
      ...
      chatPartUtils.getToolPartName(p) === 'ib_show_quick_replies'
    ) {
