  ...
  switch (type) {
    case FlowActionType.BLOCK: {
      if (
        ...
        return blockPropertiesUtils.buildSchema(
    ...
    case FlowTriggerType.BLOCK: {
      if (
        ...
        return blockPropertiesUtils.buildSchema(
          ...
          block.auth,
          block.triggers[actionNameOrTriggerName].requireAuth,
        );
      }
      return z.object({});
    }
...
function buildOAuth2ValueSchema(
  ...
  const propsSchema = blockPropertiesUtils.buildSchema(
    auth.props ?? {},
    undefined,
  );
...
function buildCustomAuthValueSchema(auth: BlockAuthProperty) {
  ...
  return z.object({
    value: z.object({
      ...
      props: blockPropertiesUtils.buildSchema(auth.props, undefined),
    }),
  });
}

function buildConnectionSchema(
  auth: BlockAuthProperty | null,
  options: {
    isGlobalConnection: boolean;
    showConnectionNameField: boolean;
  },
...
export const formUtils = {
  ...
  removeUndefinedFromInput: (step: FlowAction | FlowTrigger) => {
    const copiedStep = JSON.parse(JSON.stringify(step)) as
      ...
      | FlowTrigger;
    if (
      copiedStep.type !== FlowTriggerType.BLOCK &&
      copiedStep.type !== FlowActionType.BLOCK
    ) {
      return step;
    }
  ...
  buildBlockSchema: (
    ...
    switch (type) {
      ...
      case FlowActionType.BLOCK: {
        return BlockActionSchema.omit({ settings: true }).extend(
          z.object({
            settings: BlockActionSettings.omit({
              input: true,
              actionName: true,
            }).extend(
              z.object({
      ...
      case FlowTriggerType.BLOCK: {
        return BlockTrigger.omit({ settings: true }).extend(
          z.object({
            settings: BlockTriggerSettings.omit({
              input: true,
              triggerName: true,
            }).extend(
              z.object({
