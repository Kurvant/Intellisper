...
import IbFieldActionMenuItemRenderer, {
  FieldActionType,
} from './field-action-menu-item-renderer';

type IbFieldHeaderProps = {
  field: ClientField & { index: number };
};

export function IbFieldHeader({ field }: IbFieldHeaderProps) {
        ...
        {actions && actions.length > 0 && (
          ...
          >
            {actions.map((action, index) => (
              <div key={index}>
                {<IbFieldActionMenuItemRenderer action={action} />}
              </div>
            ))}
          </DropdownMenuContent>
        )}
