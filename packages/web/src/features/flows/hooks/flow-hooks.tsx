...
import { blocksApi } from '@/features/pieces/api/pieces-api';
import { blockSelectorUtils } from '@/features/pieces/utils/piece-selector-utils';
...
export const flowHooks = {
  ...
  useChangeFlowStatus: ({
    ...
    const { data: enableFlowOnPublish } = flagsHooks.useFlag<boolean>(
      IbFlagId.ENABLE_FLOW_ON_PUBLISH,
    );
    const { data: triggerTimeout } = flagsHooks.useFlag<number>(
      IbFlagId.TRIGGER_TIMEOUT_SECONDS,
    );
    ...
    return useMutation({
      ...
      onError: (error: unknown) => {
        ...
        const apError = error.response.data as IbErrorParams;
        if (apError.code === ErrorCode.TRIGGER_UPDATE_STATUS) {
          ...
          openDialog({
            ...
            description: (
              <p>
                {t(
                  'An error occurred while changing the flow status. This may be due to an issue in the trigger block or its settings.',
                )}
              </p>
            ),
            error: {
  ...
  useCreateMcpFlow: () => {
    ...
    return useMutation({
      mutationFn: async () => {
        ...
        const mcpBlock = await blocksApi.get({
          name: '@intelblocks/block-mcp',
        });
        const trigger = mcpBlock.triggers['mcp_tool'];
        if (!trigger) {
          throw new Error('MCP trigger not found');
        }
        const stepData = blockSelectorUtils.getDefaultStepValues({
          stepName: 'trigger',
          blockSelectorItem: {
            actionOrTrigger: trigger,
            type: FlowTriggerType.BLOCK,
            blockMetadata: stepUtils.mapBlockToMetadata({
              block: mcpBlock,
              type: 'trigger',
            }),
          },
