...
function SmartOutputViewer({
  ...
  const blockDefinedSchema = blockSchema ?? null;
  ...
  if (blockDefinedSchema && isJsonObject) {
    return (
      <OutputViewerShell
        json={json}
        title={title}
        friendlyContent={
          <OutputFieldList json={json} schema={blockDefinedSchema} />
        }
      />
    );
  }
