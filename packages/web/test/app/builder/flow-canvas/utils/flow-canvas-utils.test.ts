...
import { IbEdgeType, IbNodeType } from '@/app/builder/flow-canvas/utils/types';
...
const getStepNode = (
  ...
  const node = graph.nodes.find(
    (n) => n.id === name && n.type === IbNodeType.STEP,
  );
...
describe('flowCanvasUtils.createFlowGraph', () => {
  ...
  it('connects each step edge to its subgraph end node', () => {
    ...
    expect(triggerEdge?.type).toEqual(IbEdgeType.STRAIGHT_LINE);
