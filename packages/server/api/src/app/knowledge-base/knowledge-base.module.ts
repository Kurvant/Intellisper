    ...
    app.addHook('preHandler', async () => {
        ...
        if (!available) {
            throw new IntellisperError({
                ...
                params: {
                    message: 'Knowledge base requires the pgvector extension. Ask your database administrator to run `CREATE EXTENSION vector;` in the Intellisper database, then restart.',
                },
            })
        }
    })
