...
export const redisMetadataKey = (runId: IbId): string => `runs_metadata:${runId}`
        ...
        async init(config: RunsMetadataQueueConfig): Promise<void> {
            queueInstance = new Queue<RunsMetadataJobData>(QueueName.RUNS_METADATA, {
                ...
                defaultJobOptions: {
                    ...
                    backoff: {
                        type: 'exponential',
                        delay: ibDayjsDuration(8, 'minute').asMilliseconds(),
                    },
                    removeOnComplete: true,
                    removeOnFail: {
                        age: ibDayjsDuration(config.redisFailedJobRetentionDays, 'day').asSeconds(),
