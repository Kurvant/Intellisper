import { ibDayjsDuration, memoryLock } from '@intelblocks/server-utils'
import { IbId, EventDestinationJobData, ExecuteChatAgentJobData, ExecuteFlowJobData, getDefaultJobPriority, isNil, JOB_PRIORITY, JobData, PollingJobData, RenewWebhookJobData, ScheduleOptions, UserInteractionJobData, WebhookJobData, WorkerJobType } from '@intelblocks/shared'
import { Job, Queue } from 'bullmq'
import { BullMQOtel } from 'bullmq-otel'
import { FastifyBaseLogger } from 'fastify'
import { redisConnections } from '../../database/redis-connections'
import { workerGroupService } from '../../enterprise/platform/platform-plan/worker-group.service'
import { system } from '../../helper/system/system'
import { AppSystemProp } from '../../helper/system/system-props'
import { getWorkerGroupQueueName, QueueName } from '../job'

const EIGHT_MINUTES_IN_MILLISECONDS = ibDayjsDuration(8, 'minute').asMilliseconds()
const REDIS_FAILED_JOB_RETENTION_DAYS = ibDayjsDuration(system.getNumberOrThrow(AppSystemProp.REDIS_FAILED_JOB_RETENTION_DAYS), 'day').asSeconds()
...
export const jobQueue = (log: FastifyBaseLogger) => ({
    ...
    async removeRepeatingJob({ flowVersionId }: { flowVersionId: IbId }): Promise<void> {
    ...
    async removeOneTimeJob({ jobId, platformId }: { jobId: IbId, platformId: string | null }): Promise<void> {
        const queueName = await getQueueName(platformId, log)
        const queue = await ensureQueueExists({ log, queueName })
...
const USER_INTERACTION_JOB_TYPES = new Set([
    ...
    WorkerJobType.EXECUTE_EXTRACT_BLOCK_INFORMATION,
])
