...
import { concurrencyPoolService } from '../../../enterprise/platform/concurrency-pool/concurrency-pool.service'
...
const PLAN_CONCURRENT_JOBS_LIMITS: Record<string, number> = {
    [PlanName.STANDARD]: 5,
    [PlanName.APPSUMO_INTELLISPER_TIER1]: 5,
    [PlanName.APPSUMO_INTELLISPER_TIER2]: 5,
    [PlanName.APPSUMO_INTELLISPER_TIER3]: 10,
    [PlanName.APPSUMO_INTELLISPER_TIER4]: 15,
    [PlanName.APPSUMO_INTELLISPER_TIER5]: 20,
    [PlanName.APPSUMO_INTELLISPER_TIER6]: 25,
...
async function tryAcquireSlot({ jobId, jobData, log }: { jobId: string, jobData: ExecuteFlowJobData, log: FastifyBaseLogger }): Promise<boolean> {
    const flowTimeoutInMilliseconds = ibDayjsDuration(system.getNumberOrThrow(AppSystemProp.FLOW_TIMEOUT_SECONDS), 'seconds').add(1, 'minute').asMilliseconds()
