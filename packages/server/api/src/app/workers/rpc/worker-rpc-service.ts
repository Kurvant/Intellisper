...
export function createHandlers(log: FastifyBaseLogger, workerGroupId?: string): WorkerToApiContract {
    return {
        async poll(input) {
            ...
            const appVersion = ibVersionUtil.getCurrentRelease()
        ...
        async uploadRunLog(input) {
            const internalErrorEnabled = input.internalError?.source === RunInternalErrorSource.ENGINE || system.getEdition() !== IbEdition.CLOUD
        ...
        async getBlock(input) {
            return blockMetadataService(log).get({
                name: input.name,
        ...
        async getUsedBlocks() {
            ...
            const blocks = await distributedStore.get<BlockPackage[]>(redisKey)
            return blocks ?? []
        },

        async markBlockAsUsed(input) {
            const redisKey = `usedPieces:${workerGroupId ?? 'shared'}`
            const existing = await distributedStore.get<BlockPackage[]>(redisKey) ?? []
            const existingKeys = new Set(existing.map((p) => `${p.blockName}@${p.blockVersion}`))
            const newBlocks = input.blocks.filter((p) => !existingKeys.has(`${p.blockName}@${p.blockVersion}`))
            if (newBlocks.length > 0) {
                await distributedStore.put(redisKey, [...existing, ...newBlocks])
            }
        },
