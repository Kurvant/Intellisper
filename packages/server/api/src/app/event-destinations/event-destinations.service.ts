...
// The delivery listener is a process-global registration on the application-events seam; guard
// against re-registering it if setup() runs more than once (e.g. a defensive re-init), so a single
// emitted event is never delivered to the same destination twice.
let deliveryListenerRegistered = false
...
export const eventDestinationService = (log: FastifyBaseLogger) => ({
    setup(): void {
        if (deliveryListenerRegistered) {
            return
        }
        deliveryListenerRegistered = true
    ...
    update: async ({ id, platformId, request }: UpdateParams): Promise<EventDestination> => {
        // Updating a destination that does not exist (or belongs to another organization) is a
        // 404 — not a raw findOneByOrFail 500. The existence check is platform-scoped, so a
        // destination owned by another organization is reported as not-found too.
        const exists = await eventDestinationRepo().existsBy({ id, platformId })
        if (!exists) {
            throw new IntellisperError({
                code: ErrorCode.ENTITY_NOT_FOUND,
                params: { entityType: 'event_destination', entityId: id },
            })
        }
