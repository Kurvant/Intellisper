...
import { platformMustBeOwnedByCurrentUser } from '../enterprise/authentication/ee-authorization'
...
export const templateController: FastifyPluginAsyncZod = async (app) => {
    ...
    app.get('/categories', GetCategoriesParams, async (request) => {
        if (edition === IbEdition.CLOUD) {
            return flagService(request.log).getOne(IbFlagId.TEMPLATES_CATEGORIES)
        }
