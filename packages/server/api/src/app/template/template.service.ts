...
import { platformTemplateService } from '../enterprise/template/platform-template.service'
...
export const templateService = (log: FastifyBaseLogger) => ({
    ...
    async create({ platformId, params }: CreateParams): Promise<Template> {
        ...
        const { flows, blocks } = preparedTemplate
    ...
    async update({ id, params }: UpdateParams): Promise<Template> {
        ...
        if (!isNil(params.flows) && params.flows.length > 0) {
            ...
            blocks = preparedTemplate.blocks
        }
        ...
        switch (template.type) {
            ...
            case TemplateType.CUSTOM: {
                return platformTemplateService(log).update({ id, params })
            }
        }
    },

    async list({ platformId, blocks, tags, search, type, category }: ListParams): Promise<SeekPage<Template>> {
        const commonFilters: Record<string, unknown> = {}

        if (blocks) {
            commonFilters.blocks = ArrayOverlap(blocks)
        }
        if (category) {
