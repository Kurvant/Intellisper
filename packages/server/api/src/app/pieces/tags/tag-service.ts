import { ibId, ListTagsRequest, SeekPage, Tag } from '@intelblocks/shared'
...
export const tagService = {
    async delete(platformId: string, id: string): Promise<void> {
        await blockTagService.deleteByTagId(id)
    ...
    async upsert(platformId: string, name: string): Promise<Tag> {
        ...
        await repo().upsert({ id: ibId(), name: clonedName, platformId }, ['name', 'platformId'])
