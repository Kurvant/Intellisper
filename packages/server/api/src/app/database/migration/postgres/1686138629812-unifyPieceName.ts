...
//
// This is an Activepieces-era migration that normalized the legacy `appName`
// piece-package naming. It is:
//   - NOT registered in getMigrations()/getEnterpriseMigrations() — it never runs;
//   - imported nowhere;
//   - operating on `piece_metadata`, a table this edition dropped (it is now
//     `block_metadata`), so its body would fail if it ever executed.
//
// Per the Intellisper rebrand it must not carry `@activepieces/piece-*` strings,
// but rewriting them would leave a still-broken, misleading migration. Instead the
// original body is commented out below and the class is reduced to a no-op, so the
// file remains a valid MigrationInterface without any live Activepieces reference.
export class UnifyPieceName1686138629812 implements MigrationInterface {
    name = 'UnifyPieceName1686138629812'

    public async up(): Promise<void> {
        // no-op: superseded by the clean-room baseline; see note above.
    }
...
/* ---------------------------------------------------------------------------
Original upstream implementation, retained verbatim for historical reference.
Do not re-enable: it targets the dropped `piece_metadata` table and predates the
piece->block rename.
