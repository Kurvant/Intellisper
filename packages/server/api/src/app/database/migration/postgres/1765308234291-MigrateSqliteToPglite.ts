...
// NOTE: `sqlite-connection` is imported lazily (dynamic import inside up()),
// not statically. It transitively does `import 'sqlite3'`, a native module.
// A static import here would load sqlite3 at boot for EVERY PGLite startup —
// even though this one-way migration almost always early-returns with nothing
// to do — crashing any environment where the sqlite3 native binary isn't built
// (e.g. local dev on PGLite). Loading it only when a real SQLite→PGLite data
// migration actually runs keeps the common PGLite path free of sqlite3.
type CreateSqlLiteDataSourceForMigrations = typeof import('../../sqlite-connection')['createSqlLiteDataSourceForMigrations']
...
export class MigrateSqliteToPglite1765308234291 implements MigrationInterface {
    ...
    public async up(queryRunner: QueryRunner): Promise<void> {
        ...
        if (databaseType !== DatabaseType.PGLITE || environment === IbEnvironment.TESTING) {
        ...
        // Skip (and never load the native sqlite3 module) when there is no
        // legacy SQLite database file to migrate from — the normal case for a
        // fresh PGLite instance. This guard must run BEFORE the dynamic import
        // of sqlite-connection below, since that module pulls in `sqlite3`.
        if (!this.legacySqliteDatabaseExists()) {
            log.info('[MigrateSqliteToPglite] Skipping - no legacy SQLite database found')
            return
        }
        ...
        try {
            // Lazy-load sqlite3 only now that we know real data migration is needed.
            const { createSqlLiteDataSourceForMigrations } = await import('../../sqlite-connection')
    ...
    // Mirrors getSqliteDatabaseFilePath() in sqlite-connection.ts
    // (<CONFIG_PATH>/database.sqlite) without importing that module, so we can
    // decide whether a legacy SQLite DB exists without loading the sqlite3
    // native binary.
    private legacySqliteDatabaseExists(): boolean {
        const configPath = system.get(AppSystemProp.CONFIG_PATH)
        if (!configPath) {
            return false
        }
        const sqliteFilePath = path.resolve(path.join(configPath, 'database.sqlite'))
        return existsSync(sqliteFilePath)
    }

    private async sqliteHasData(sqliteDataSource: ReturnType<CreateSqlLiteDataSourceForMigrations>): Promise<boolean> {
        try {
