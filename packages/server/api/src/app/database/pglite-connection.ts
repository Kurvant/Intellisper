...
import { IbEdition, IbEnvironment, spreadIfDefined } from '@intelblocks/shared'
...
const getPGliteDataPath = (): string | undefined => {
    const env = system.getOrThrow<IbEnvironment>(AppSystemProp.ENVIRONMENT)

    if (env === IbEnvironment.TESTING) {
...
export const createPGliteDataSource = (): DataSource => {
    ...
    const env = system.getOrThrow<IbEnvironment>(AppSystemProp.ENVIRONMENT)
    if (edition !== IbEdition.COMMUNITY && env !== IbEnvironment.TESTING) {
    ...
    return new DataSource({
        ...
        migrationsRun: env !== IbEnvironment.TESTING,
        migrationsTransactionMode: 'each',
        migrations: env !== IbEnvironment.TESTING ? getMigrations() : [],
        synchronize: env === IbEnvironment.TESTING,
        ...commonProperties,
    })
}
