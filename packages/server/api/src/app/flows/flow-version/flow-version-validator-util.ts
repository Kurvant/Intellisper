            ...
            case FlowOperationType.ADD_ACTION:
                switch (clonedRequest.request.action.type) {
                    ...
                    case FlowActionType.BLOCK: {
                        clonedRequest.request.action.settings.blockVersion = flowBlockUtil.getExactVersion(clonedRequest.request.action.settings.blockVersion)
            ...
            case FlowOperationType.UPDATE_ACTION:
                switch (clonedRequest.request.type) {
                    case FlowActionType.LOOP_ON_ITEMS:
                        ...
                        break
                    case FlowActionType.BLOCK: {
                        clonedRequest.request.settings.blockVersion = flowBlockUtil.getExactVersion(clonedRequest.request.settings.blockVersion)
                        const result = await validateAction(
...
async function validateAction({ settings, platformId, log }: ValidateActionParams): Promise<ValidationResult> {
    ...
    return validateProps(props, settings.input, block.auth, action.requireAuth)
}
...
async function validateTrigger({ settings, platformId, log }: ValidateTriggerParams): Promise<ValidationResult> {
    ...
    return validateProps(props, settings.input, block.auth, trigger.requireAuth)
}
...
function validateProps(
    ...
    const propsSchema = blockPropertiesUtils.buildSchema(props, auth, requireAuth)
