...
export const migrateV9AiBlocks: Migration = {
    ...
    migrate: async (flowVersion: FlowVersion): Promise<FlowVersion> => {
        const newVersion = flowStructureUtil.transferFlow(flowVersion, (step) => {
            ...
            if (step.settings.blockName === '@intelblocks/block-text-ai') {
                return migrateTextai(step)
            }
            if (step.settings.blockName === '@intelblocks/block-utility-ai') {
                return migrateUtilityAction(step)
            }
            if (step.settings.blockName === '@intelblocks/block-image-ai') {
...
function migrateUtilityAction(step: BlockAction): FlowAction {
...
function migrateTextai(step: BlockAction): FlowAction {
...
function migrateImageai(step: BlockAction): FlowAction {
    const input = step.settings?.input as Record<string, unknown>
