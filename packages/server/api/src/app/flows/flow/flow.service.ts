...
export const flowService = (log: FastifyBaseLogger) => ({
    ...
    async getTemplate({
        ...
        const template: SharedTemplate = {
            ...
            summary: '',
            description: '',
            blocks: Array.from(new Set(flowBlockUtil.getUsedBlocks(flow.version.trigger))),
            flows: [flow.version],
            tags: [],
            blogUrl: '',
            metadata: {
    ...
    addDeleteFlowJob: async (flow: Flow): Promise<void> => {
        await systemJobsSchedule(log).upsertJob({
            ...
            customConfig: {
                backoff: {
                    type: 'exponential',
                    delay: ibDayjsDuration(5, 'second').asMilliseconds(),
                },
            },
        })
    },
...
async function createNewDraftIfVersionIsPublished({
    ...
    if (lastVersion.state === FlowVersionState.LOCKED) {
        ...
        if (
            lockedVersion.trigger.type === FlowTriggerType.BLOCK &&
