...
export enum AppSystemProp {
    ...
    LOAD_TRANSLATIONS_FOR_DEV_BLOCKS = 'LOAD_TRANSLATIONS_FOR_DEV_BLOCKS',
    LOG_LEVEL = 'LOG_LEVEL',
    ...
    BLOCKS_REGISTRY_URL = 'BLOCKS_REGISTRY_URL',
    ...
    // Capacity cap for the capped chat rollout funnel (H.2.k/H.2.m). Once `chatted` reaches this,
    // the funnel is `closed` (monotonic). Unset ⇒ default cap.
    CHAT_ROLLOUT_CAP = 'CHAT_ROLLOUT_CAP',
    // Internal-admin chat analytics (H.2.m): metrics are written to a LOCAL table only — no
    // outbound HTTP. CHAT_METRICS_ENABLED is the recorder master switch (default on);
...
export const environmentVariables = {
    ...
    getEnvironment: (prop: AppSystemProp): string | undefined => {
        ...
        return environmnetVariables['IB_' + prop]
    },
