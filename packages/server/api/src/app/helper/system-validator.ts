...
const systemPropValidators: {
    ...
    [AppSystemProp.ENVIRONMENT]: enumValidator(Object.values(IbEnvironment)),
    ...
    [AppSystemProp.BLOCKS_REGISTRY_URL]: urlValidator,
    ...
    [AppSystemProp.LOAD_TRANSLATIONS_FOR_DEV_BLOCKS]: booleanValidator,
    ...
    // Chat analytics (internal admin, local store)
...
export const validateEnvPropsOnStartup = async (log: FastifyBaseLogger): Promise<void> => {
    ...
    const fileStorageLocation = process.env.IB_FILE_STORAGE_LOCATION
    
    if (environment !== IbEnvironment.TESTING && fileStorageLocation === FileLocation.S3) {
        try {
    ...
    const codeSandboxType = process.env.IB_CODE_SANDBOX_TYPE
    ...
    if ([IbEdition.CLOUD, IbEdition.ENTERPRISE].includes(edition) && environment === IbEnvironment.PRODUCTION) {
