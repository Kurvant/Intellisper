...
export const flowTriggerSideEffect = (log: FastifyBaseLogger) => {
    return {
        async enable(params: EnableFlowTriggerParams): Promise<ActiveTriggerReturn> {
            ...
            const { flowId, flowVersionId, projectId, simulate, blockTrigger } = params
...
async function handleAppWebhookTrigger({ engineHelperResponse, flowId, projectId, blockName }: ActiveTriggerParams): Promise<ActiveTriggerReturn> {
...
async function handleWebhookTrigger({ flowId, flowVersionId, projectId, blockTrigger, log }: ActiveTriggerParams): Promise<ActiveTriggerReturn> {
    const renewConfiguration = blockTrigger.renewConfiguration
