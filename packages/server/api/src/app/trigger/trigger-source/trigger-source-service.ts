...
export const triggerSourceService = (log: FastifyBaseLogger) => {
    return {
        async enable(params: EnableTriggerParams): Promise<TriggerSource> {
            ...
            const blockTrigger = await triggerUtils(log).getBlockTriggerOrThrow({ flowVersion, projectId })
        ...
        async disable(params: DisableTriggerParams): Promise<void> {
            ...
            const blockTrigger = await triggerUtils(log).getBlockTrigger({ flowVersion, projectId })
