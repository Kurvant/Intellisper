import { facebookLeads } from '@intelblocks/block-facebook-leads'
import { intercom } from '@intelblocks/block-intercom'
import { slack } from '@intelblocks/block-slack'
...
const appWebhooks: Record<string, Block<BlockAuthProperty | BlockAuthProperty[] | undefined>> = {
    slack,
    square,
...
const blockNames: Record<string, string> = {
    slack: '@intelblocks/block-slack',
    square: '@intelblocks/block-square',
    'facebook-leads': '@intelblocks/block-facebook-leads',
    intercom: '@intelblocks/block-intercom',
}
            ...
            if (isNil(block)) {
                throw new IntellisperError({
                    ...
                    params: {
                        entityType: 'piece',
                        entityId: blockUrl,
                        message: 'Block is not found in app event routing',
                    },
                })
            }
            const appName = blockNames[blockUrl]
            assertNotNullOrUndefined(block.events, 'Event is possible in this piece')
