...
const EDITION_REQUIRES_RBAC = [IbEdition.CLOUD, IbEdition.ENTERPRISE].includes(system.getEdition())
...
export async function resolvePermissionChecker({ userId, projectId, log }: {
    ...
    try {
        ...
        if (err instanceof IntellisperError && err.error.code === ErrorCode.AUTHORIZATION) {
            return buildChecker((permission, toolTitle) => {
                if (isNil(permission)) {
                    return null
                }
