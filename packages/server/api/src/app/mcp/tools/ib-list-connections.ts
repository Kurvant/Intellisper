...
const listConnectionsSchema = z.object({
    blockName: z
        ...
        .optional()
        .describe(
            'Filter by piece name. Short names like "slack" or "google-drive" are auto-expanded to full format (e.g. "@intelblocks/block-slack"). You can also pass the full name directly.',
        ),
    displayName: z
        .string()
        .optional()
...
export const ibListConnectionsTool = (mcp: ProjectScopedMcpServer, log: FastifyBaseLogger): McpToolDefinition => {
    return {
        title: 'ib_list_connections',
        permission: Permission.READ_APP_CONNECTION,
        description:
            'List OAuth/app connections in the project. Returns externalId needed for the auth parameter on steps.',
        inputSchema: {
            blockName: listConnectionsSchema.shape.blockName,
        ...
        execute: async (args) => {
            try {
                ...
                const connections = await appConnectionService(log).list({
                    ...
                    blockName: mcpUtils.normalizeBlockName(params.blockName),
                    limit: 200,
                    externalIds: undefined,
                })
                const lines = connections.data.map(c => `- externalId: ${c.externalId} | displayName: "${c.displayName}" | piece: ${c.blockName} | status: ${c.status} | scope: ${c.scope}`)
                const structured = {
