import { FlowRetryStrategy, FlowRunStatus, isFlowRunStateTerminal, isNil, McpToolDefinition, Permission, ProjectScopedMcpServer } from '@intelblocks/shared'
...
const retryRunInput = z.object({
    flowRunId: z.string().describe('The ID of the failed flow run to retry. Use ib_list_runs to find it.'),
...
export const ibRetryRunTool = (mcp: ProjectScopedMcpServer, log: FastifyBaseLogger): McpToolDefinition => {
    return {
        title: 'ib_retry_run',
        ...
        execute: async (args) => {
            try {
                ...
                if (existingRun.status === FlowRunStatus.SUCCEEDED) {
                    return { content: [{ type: 'text', text: '⚠️ Run already succeeded. Use ib_test_flow to run a new test instead.' }] }
                }
                ...
                if (strategy === FlowRetryStrategy.ON_LATEST_VERSION) {
                    ...
                    if (isNil(flow.publishedVersionId)) {
                        return { content: [{ type: 'text', text: '❌ Cannot retry with ON_LATEST_VERSION — this flow has not been published yet. Use ib_lock_and_publish first.' }] }
                    }
                }
                ...
                if (!isFlowRunStateTerminal({ status: completedRun.status, ignoreInternalError: false })) {
                    return {
                        content: [{
                            type: 'text',
                            text: `⏳ Retry still running after 120s. Run ID: ${completedRun.id} (status: ${completedRun.status}). Use ib_get_run to check results later.`,
                        }],
                    }
                }
                ...
                log.error({ err, projectId: mcp.projectId }, 'ib_retry_run failed')
