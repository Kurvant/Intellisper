import { FilterOperator, isNil, McpToolDefinition, Permission, ProjectScopedMcpServer } from '@intelblocks/shared'
...
const findRecordsInput = z.object({
    tableId: z.string().describe('The table ID. Use ib_list_tables to find it.'),
...
export const ibFindRecordsTool = (mcp: ProjectScopedMcpServer, log: FastifyBaseLogger): McpToolDefinition => {
    return {
        title: 'ib_find_records',
        ...
        execute: async (args) => {
            try {
                ...
                log.error({ err, projectId: mcp.projectId }, 'ib_find_records failed')
