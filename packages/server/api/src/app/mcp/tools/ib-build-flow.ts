...
const stepSpec = z.object({
    type: z.enum([FlowActionType.CODE, FlowActionType.BLOCK, FlowActionType.LOOP_ON_ITEMS]),
...
export const ibBuildFlowTool = ({ mcp, userId }: McpToolContext, log: FastifyBaseLogger): McpToolDefinition => {
    return {
        title: 'ib_build_flow',
        permission: Permission.WRITE_FLOW,
        description: 'Create a NEW flow from scratch in one call: trigger + steps. Steps are added sequentially by default (trigger → step_1 → step_2 → ...). To nest steps inside a loop, set parentStepName to the loop step name and stepLocationRelativeToParent to INSIDE_LOOP. ROUTER steps are NOT supported here (branches and conditions cannot be configured in one call) — build the rest of the flow first, then add the router with ib_add_step and configure branches with ib_add_branch / ib_update_branch. For EDITING an existing flow, do NOT rebuild it — use the granular ib_add_step / ib_update_step / ib_update_trigger instead.',
        inputSchema: {
            flowName: z.string().describe('Name for the new flow'),
            trigger: z.object({
                blockName: z.string().describe('Trigger block name (e.g. "@intelblocks/block-webhook")'),
            ...
            steps: z.array(stepSpec).describe('Array of steps. By default added sequentially after trigger. Use parentStepName + stepLocationRelativeToParent to nest steps inside loops. Each step supports: BLOCK (blockName+actionName+input), CODE (sourceCode+input), LOOP_ON_ITEMS (loopItems). ROUTER is not supported here — add it afterwards with ib_add_step + ib_add_branch.'),
        },
        ...
        execute: async (args) => {
            ...
            try {
                ...
                const triggerVersionResult = await mcpUtils.resolveLatestBlockVersion({ blockName: trigger.blockName, projectId, platformId, log })
                ...
                const triggerPayload = BlockTrigger.parse({
                    name: 'trigger',
                    ...
                    settings: {
                        blockName: triggerVersionResult.normalizedBlockName,
                        blockVersion: triggerVersionResult.blockVersion,
                ...
                for (const step of steps) {
                    ...
                    let resolvedBlockVersion: string | undefined
                    let resolvedBlockName: string | undefined
                    if (step.type === FlowActionType.BLOCK) {
                        if (!step.blockName) {
                            skippedSteps.push(step.displayName)
                            continue
                        }
                        const versionResult = await mcpUtils.resolveLatestBlockVersion({ blockName: step.blockName, projectId, platformId, log })
                        if (versionResult.error) {
                            skippedSteps.push(step.displayName)
                            continue
                        }
                        resolvedBlockVersion = versionResult.blockVersion
                        resolvedBlockName = versionResult.normalizedBlockName
                    }
                    ...
                    const skeleton = buildSkeleton({ step: rewrittenStep, name: stepName, resolvedBlockVersion, resolvedBlockName })
                ...
                return { content: [{ type: 'text', text: `⚠️ Flow "${flowName}" created (id: ${flowId}) with ${allSteps.length} ${stepWord} (${validCount} valid, ${invalidSteps.length} invalid: ${invalidSteps.join(', ')}).${skippedHint} Use ib_update_step or ib_update_trigger to fix. Open: ${flowUrl}` }], structuredContent: structured }
            }
            catch (err) {
                if (flowId) {
...
function buildSkeleton({ step, name, resolvedBlockVersion, resolvedBlockName }: {
    step: z.infer<typeof stepSpec>
    name: string
    resolvedBlockVersion?: string
    ...
    switch (step.type) {
        ...
        case FlowActionType.BLOCK:
            return {
                ...
                valid: false,
                settings: {
                    blockName: resolvedBlockName ?? step.blockName ?? '',
                    blockVersion: resolvedBlockVersion ?? '',
