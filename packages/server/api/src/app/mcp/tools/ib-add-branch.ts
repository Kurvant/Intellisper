...
export const ibAddBranchTool = (mcp: ProjectScopedMcpServer, log: FastifyBaseLogger): McpToolDefinition => {
    return {
        title: 'ib_add_branch',
        ...
        inputSchema: {
            ...
            routerStepName: z.string().describe('The name of the ROUTER step to add a branch to. Use ib_flow_structure to get valid values.'),
        ...
        execute: async (args) => {
            try {
                ...
                return {
                    content: [{
                        type: 'text',
                        text: `✅ Branch "${branchName}" added at index ${branchIndex} in router "${routerStepName}". Use ib_add_step with stepLocationRelativeToParent=INSIDE_BRANCH and branchIndex=${branchIndex} to add steps inside this branch.${invalidWarning}`,
                    }],
                }
            }
            catch (err) {
