...
const getRunInput = z.object({
    flowRunId: z.string().describe('The ID of the flow run. Use ib_list_runs to find it.'),
})

export const ibGetRunTool = (mcp: ProjectScopedMcpServer, log: FastifyBaseLogger): McpToolDefinition => {
    return {
        title: 'ib_get_run',
        ...
        execute: async (args) => {
            try {
                ...
                log.error({ err, projectId: mcp.projectId }, 'ib_get_run failed')
