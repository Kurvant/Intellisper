import { isNil, McpToolDefinition } from '@intelblocks/shared'
...
export const ibSetProjectContextTool = ({ platformId, userId, selectionScope, log }: {
    platformId: string
    userId: string
    selectionScope: ProjectSelectionScope
    log: FastifyBaseLogger
}): McpToolDefinition => ({
    title: 'ib_set_project_context',
    ...
    execute: async (args: Record<string, unknown>) => {
        ...
        if (!isNil(projectId) && projectId !== '') {
            ...
            return {
                content: [{
                    type: 'text' as const,
                    text: `Project context set to "${targetProject.displayName}".\n\nAll tools will now operate on this project. Use ib_set_project_context without a projectId to clear the selection.\n\nAvailable projects:\n${projectList}`,
                }],
            }
        }
        ...
        return {
            content: [{
                type: 'text' as const,
                text: `Project context cleared.\n\nAvailable projects:\n${projectList}\n\nUse ib_set_project_context with a projectId to select one.`,
            }],
        }
    },
})
