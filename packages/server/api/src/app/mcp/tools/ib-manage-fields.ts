import { FieldType, isNil, McpToolDefinition, Permission, ProjectScopedMcpServer } from '@intelblocks/shared'
...
const manageFieldsInput = z.object({
    ...
    fieldId: z.string().optional().describe('The field ID (required for UPDATE and DELETE). Use ib_list_tables to find it.'),
...
export const ibManageFieldsTool = (mcp: ProjectScopedMcpServer, log: FastifyBaseLogger): McpToolDefinition => {
    return {
        title: 'ib_manage_fields',
        ...
        execute: async (args) => {
            try {
                ...
                log.error({ err, projectId: mcp.projectId }, 'ib_manage_fields failed')
