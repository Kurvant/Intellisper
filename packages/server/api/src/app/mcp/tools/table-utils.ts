import { Field, FieldType, McpToolResult, PopulatedRecord, tryCatch } from '@intelblocks/shared'
import { z } from 'zod'
...
export function tableNotFoundError(tableId: string): McpToolResult {
    return { content: [{ type: 'text', text: `❌ Table "${tableId}" not found. Use ib_list_tables to see available tables.` }] }
}
...
export function resolveFieldNameToId(
    ...
    for (const name of fieldNames) {
        ...
        if (duplicates.has(lower)) {
            errors.push(`Duplicate field name "${name}". Rename one of them using ib_manage_fields before proceeding.`)
        }
