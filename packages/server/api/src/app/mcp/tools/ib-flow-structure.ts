...
import type { Step } from '@intelblocks/shared'
...
function getConfigStatus(step: Step): string {
    ...
    switch (step.type) {
        case FlowTriggerType.EMPTY:
            return 'unconfigured (no trigger set — use ib_update_trigger)'
        case FlowTriggerType.BLOCK:
            return s.triggerName ? 'invalid (use ib_update_trigger to fix — check required inputs)' : 'unconfigured (triggerName missing)'
        case FlowActionType.BLOCK:
            return s.actionName ? 'invalid (use ib_update_step to fix — check required inputs)' : 'unconfigured (actionName missing)'
...
function formatStepSettings(step: Step): string[] {
    ...
    if (step.type === FlowTriggerType.BLOCK || step.type === FlowActionType.BLOCK) {
...
function buildFlowStructure(trigger: Step): { structure: StepInfo[], stepByName: Map<string, Step> } {
    ...
    const structure = allSteps.map((step): StepInfo => {
        ...
        for (const parent of allSteps) {
            ...
            if (parent.type === FlowActionType.CODE || parent.type === FlowActionType.BLOCK) {
...
function formatFlowStructure(
    ...
    for (const step of structure) {
        ...
        if (step.relationship === 'trigger') {
            let triggerDetail = ''
            if (fullStep && fullStep.type === FlowTriggerType.BLOCK) {
                triggerDetail = ` (block: ${fullStep.settings.blockName}, trigger: ${fullStep.settings.triggerName ?? 'not set'})`
            }
        ...
        if (step.type === FlowActionType.BLOCK) {
            const s = fullStep?.settings as { blockName?: string, actionName?: string } | undefined
            if (s?.blockName) stepDetail = ` (block: ${s.blockName}, action: ${s.actionName ?? 'not set'})`
        }
    ...
    lines.push('')
    lines.push('## Valid insert locations for ib_add_step')
    ...
    for (const step of structure) {
        ...
        if (step.type === FlowActionType.CODE || step.type === FlowActionType.BLOCK) {
            const fullStep = stepByName.get(step.name)
            const isFullStepCodeOrBlock = fullStep?.type === FlowActionType.CODE || fullStep?.type === FlowActionType.BLOCK
            const cofEnabled = isFullStepCodeOrBlock && fullStep?.settings.errorHandlingOptions?.continueOnFailure?.value
            if (cofEnabled) {
    ...
    lines.push('')
    lines.push('Step types: trigger = EMPTY | BLOCK_TRIGGER; action = CODE | BLOCK | LOOP_ON_ITEMS | ROUTER')
    lines.push('')
    lines.push('## Referencing step outputs')
    lines.push(mcpUtils.STEP_REFERENCE_HINT)
    lines.push('Use ib_test_step or ib_test_flow to generate sample data and see available output fields.')
...
export const ibFlowStructureTool = (mcp: ProjectScopedMcpServer, log: FastifyBaseLogger): McpToolDefinition => {
    return {
        title: 'ib_flow_structure',
        permission: Permission.READ_FLOW,
        description: 'Get the structure of a flow: step tree (parent/child), each step type, configuration status (configured/unconfigured/invalid), and valid insert locations for ib_add_step.',
        inputSchema: {
