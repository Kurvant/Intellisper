...
const updateRecordInput = z.object({
    ...
    recordId: z.string().describe('The record ID to update. Use ib_find_records to find it.'),
...
export const ibUpdateRecordTool = (mcp: ProjectScopedMcpServer, log: FastifyBaseLogger): McpToolDefinition => {
    return {
        title: 'ib_update_record',
        ...
        execute: async (args) => {
            try {
                ...
                log.error({ err, projectId: mcp.projectId }, 'ib_update_record failed')
