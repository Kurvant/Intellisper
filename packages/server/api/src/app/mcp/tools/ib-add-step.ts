...
const addStepInput = z.object({
    ...
    stepType: z.enum([FlowActionType.CODE, FlowActionType.BLOCK, FlowActionType.LOOP_ON_ITEMS, FlowActionType.ROUTER]),
...
export const ibAddStepTool = (mcp: ProjectScopedMcpServer, log: FastifyBaseLogger): McpToolDefinition => {
    return {
        title: 'ib_add_step',
        permission: Permission.WRITE_FLOW,
        description: 'Add a new step to a flow. Optionally configure it in the same call by providing input/auth/sourceCode. Prefer BLOCK over CODE.',
        inputSchema: {
            flowId: z.string().describe('The id of the flow'),
            parentStepName: z.string().describe('The step name to insert after/into (e.g. "trigger", "step_1"). Use ib_flow_structure to get valid values.'),
            ...
            stepType: z.enum([FlowActionType.CODE, FlowActionType.BLOCK, FlowActionType.LOOP_ON_ITEMS, FlowActionType.ROUTER]).describe('The type of step to add. Prefer BLOCK over CODE — only use CODE when no block exists for the task.'),
            displayName: z.string().describe('Display name for the step'),
            blockName: z.string().optional().describe('For BLOCK steps: the block name (e.g. "@intelblocks/block-gmail"). Use ib_research_blocks to get valid values.'),
            actionName: z.string().optional().describe('For BLOCK steps: the action name within the block. Use ib_research_blocks with includeActions=true to get valid values.'),
            input: z.record(z.string(), z.unknown()).optional().describe(`For BLOCK/CODE steps: input config (key-value pairs). ${mcpUtils.STEP_REFERENCE_HINT}`),
            auth: z.string().optional().describe('Connection externalId from ib_list_connections. Auto-wrapped as {{connections[\'externalId\']}}.'),
            ...
            continueOnFailure: z.boolean().optional().describe('For CODE/BLOCK steps: set true on the step that can fail (the one whose failure you want to react to), NOT on the recovery step. Defaults to false. When true the flow keeps running on failure and the step gains On success / On failure branches — add handler steps into them with stepLocationRelativeToParent INSIDE_ON_SUCCESS_BRANCH / INSIDE_ON_FAILURE_BRANCH and parentStepName = this step.'),
            retryOnFailure: z.boolean().optional().describe('For CODE/BLOCK steps: whether to retry this step on failure. Defaults to false.'),
        },
        annotations: { readOnlyHint: false, destructiveHint: false, idempotentHint: false, openWorldHint: false },
        execute: async (args) => {
            const { flowId, parentStepName, stepLocationRelativeToParent, branchIndex, stepType, displayName, blockName, actionName, input, auth, sourceCode, packageJson, loopItems, continueOnFailure, retryOnFailure } = addStepInput.parse(args)
                ...
                case FlowActionType.BLOCK: {
                    if (!blockName) {
                        return {
                            content: [{
                                type: 'text',
                                text: '❌ blockName is required for BLOCK steps. Use ib_research_blocks to get valid values.',
                            }],
                        }
                    }
                    ...
                    const blockSettings: Record<string, unknown> = {
                        blockName: versionResult.normalizedBlockName,
                        blockVersion: versionResult.blockVersion,
                    ...
                    if (input !== undefined) {
                        await mcpUtils.fillDefaultsForMissingOptionalProps({ settings: blockSettings, platformId: project.platformId, log })
                    }
                    skeletonAction = {
            ...
            if (stepLocationRelativeToParent === StepLocationRelativeToParent.INSIDE_BRANCH && branchIndex === undefined) {
                return {
                    content: [{ type: 'text', text: '❌ branchIndex is required when stepLocationRelativeToParent is INSIDE_BRANCH. Use ib_flow_structure to get valid branch indices.' }],
                }
            }
            ...
            try {
                ...
                if (hasConfig && addedStep && !addedStep.valid) {
                    return {
                        content: [{
                            type: 'text',
                            text: `⚠️ Step "${displayName}" (${stepName}) added but still invalid. Use ib_get_block_props to check required fields, then ib_update_step to fix.${draftWarning}`,
                        }],
                ...
                return {
                    content: [{
                        type: 'text',
                        text: `✅ Step "${displayName}" (${stepName}) added. Now use ib_update_step with stepName="${stepName}" to configure its settings.${draftWarning}`,
                    }],
                    structuredContent: structured,
                }
            }
