...
export const ibChangeFlowStatusTool = (mcp: ProjectScopedMcpServer, log: FastifyBaseLogger): McpToolDefinition => {
    return {
        title: 'ib_change_flow_status',
        ...
        execute: async (args) => {
            ...
            if (status === FlowStatus.ENABLED && isNil(flow.publishedVersionId)) {
                return {
                    content: [{
                        type: 'text',
                        text: `❌ Flow "${flow.version.displayName}" has no published version. Use ib_lock_and_publish first.`,
                    }],
                }
            }
