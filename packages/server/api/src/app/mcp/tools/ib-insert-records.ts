import { isNil, McpToolDefinition, Permission, ProjectScopedMcpServer } from '@intelblocks/shared'
...
const insertRecordsInput = z.object({
    tableId: z.string().describe('The table ID (the "id" from ib_list_tables; the externalId is also accepted)'),
...
export const ibInsertRecordsTool = (mcp: ProjectScopedMcpServer, log: FastifyBaseLogger): McpToolDefinition => {
    return {
        title: 'ib_insert_records',
        ...
        execute: async (args) => {
            try {
                ...
                log.error({ err, projectId: mcp.projectId }, 'ib_insert_records failed')
