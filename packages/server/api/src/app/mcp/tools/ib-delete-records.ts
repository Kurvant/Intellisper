...
const deleteRecordsInput = z.object({
    recordIds: z.array(z.string()).describe('Array of record IDs to delete. Use ib_find_records to find them.'),
...
export const ibDeleteRecordsTool = (mcp: ProjectScopedMcpServer, log: FastifyBaseLogger): McpToolDefinition => {
    return {
        title: 'ib_delete_records',
        ...
        execute: async (args) => {
            try {
                ...
                log.error({ err, projectId: mcp.projectId }, 'ib_delete_records failed')
