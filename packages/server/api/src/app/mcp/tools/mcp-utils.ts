import { BlockMetadataModel, BlockPropertyMap, PropertyType } from '@intelblocks/blocks-framework'
import { BranchOperator, EngineResponse, EngineResponseStatus, FlowActionType, flowStructureUtil, isNil, isObject, McpServerType, McpToolResult, ProjectScopedMcpServer, singleValueConditions, tryCatch, WorkerJobType } from '@intelblocks/shared'
import type { RouterAction, Step } from '@intelblocks/shared'
import { FastifyBaseLogger } from 'fastify'
import { z } from 'zod'
import { expressionRewriter } from '../../flows/flow-version/migrations/expression-rewriter'
import { getBlockPackageWithoutArchive, blockMetadataService } from '../../pieces/metadata/piece-metadata-service'
...
function diagnoseBlockProps({ props, input, blockAuth, requireAuth, componentType }: DiagnoseBlockPropsParams): DiagnosisResult {
    ...
    const hasAuth = blockAuth !== undefined && blockAuth !== null && requireAuth
    if (hasAuth && !input.auth) {
        missing.push('auth (connection required — use ib_list_connections)')
    }
    const parts: string[] = []
    ...
    if (uiRequired.length > 0) {
        parts.push(`These inputs require selection from your account and must be configured in the Intellisper UI: ${uiRequired.join(', ')}.`)
    }
...
function buildPropSummaries(props: BlockPropertyMap, depth = 0): PropSummary[] {
    return Object.entries(props)
        ...
        .map(([name, prop]) => {
            ...
            if (prop.type === PropertyType.DROPDOWN || prop.type === PropertyType.MULTI_SELECT_DROPDOWN) {
                summary.note = 'Resolve with ib_resolve_property_options. Use the returned value (ID), not label.'
            }
            if (prop.type === PropertyType.DYNAMIC) {
                summary.note = 'DYNAMIC — call ib_get_block_props with auth+input to resolve sub-fields.'
            }
            if (prop.type === PropertyType.ARRAY && 'properties' in prop && isObject(prop.properties) && depth < MAX_PROP_DEPTH) {
                const arraySubProps: BlockPropertyMap = prop.properties
...
function normalizeBlockName(blockName: string | undefined): string | undefined {
    if (isNil(blockName)) {
        return undefined
    }
    ...
    const stripped = blockName.startsWith('block-') ? blockName.slice('block-'.length) : blockName
    const normalized = stripped.replace(/_/g, '-')
    return `@intelblocks/block-${normalized}`
}

async function lookupBlockComponent({ blockName, componentName, componentType, projectId, platformId, log }: LookupBlockComponentParams): Promise<LookupBlockComponentResult> {
    const normalized = normalizeBlockName(blockName)
    if (isNil(normalized)) {
        return { error: mcpToolError('Validation failed', new Error('blockName is required')) }
    }
    // platformId is needed so private (CUSTOM) blocks on this platform are discoverable.
    ...
    if (!isNil(platformId)) {
        ...
        return { error: mcpToolError('Validation failed', new Error('Either platformId or projectId is required to look up a block')) }
    }
    const block = await blockMetadataService(log).get({ name: normalized, projectId, platformId: resolvedPlatformId })
    if (isNil(block)) {
        return { error: { content: [{ type: 'text', text: `❌ Block "${normalized}" not found. Use ib_research_blocks to get valid block names.` }] } }
    }
    const componentMap = componentType === 'action' ? block.actions : block.triggers
    ...
    return { block, component, blockName: normalized }
}
...
function publishedFlowWarning(publishedVersionId: string | null | undefined): string {
    ...
    return '\n⚠️ This flow is published. Changes apply to the draft only — use ib_lock_and_publish to push them live.'
}

function validateAuth(auth: string | undefined): { content: [{ type: 'text', text: string }] } | null {
    if (auth !== undefined && /['{}\[\]]/.test(auth)) {
        return { content: [{ type: 'text', text: '❌ auth must be a plain externalId with no special characters. Use the exact value from ib_list_connections.' }] }
    }
    return null
}
...
async function fillDefaultsForMissingOptionalProps({ settings, platformId, log }: {
    ...
    const blockVersion = settings.blockVersion
    const actionName = settings.actionName
    if (typeof blockName !== 'string' || typeof blockVersion !== 'string' || typeof actionName !== 'string') {
        return
    }
    try {
        const block = await blockMetadataService(log).getOrThrow({ platformId, name: blockName, version: blockVersion })
        const action = block.actions[actionName]
        if (isNil(action)) {
            return
        }
        ...
        log.warn({ err, blockName, actionName }, 'fillDefaultsForMissingOptionalProps: failed, skipping defaults')
    }
}
...
async function resolveLatestBlockVersion({ blockName, projectId, platformId, log }: {
    blockName: string
    projectId: string
    platformId: string
    log: FastifyBaseLogger
}): Promise<ResolveLatestBlockVersionResult> {
    const normalized = normalizeBlockName(blockName)
    if (isNil(normalized)) {
        return { error: mcpToolError('Validation failed', new Error('blockName is required')) }
    }
    const block = await blockMetadataService(log).get({ name: normalized, projectId, platformId })
    if (isNil(block)) {
        return { error: { content: [{ type: 'text', text: `❌ Block "${normalized}" not found. Use ib_research_blocks to get valid block names.` }] } }
    }
    return { blockVersion: `~${block.version}`, normalizedBlockName: normalized }
}
...
async function executePropertyResolution({ blockName, blockVersion, actionOrTriggerName, propertyName, auth, input, searchValue, projectId, platformId, log }: {
    blockName: string
    blockVersion: string
    ...
    const blockPackage = await getBlockPackageWithoutArchive(log, platformId, { blockName, blockVersion })
...
export type PropertyResolutionResult =
    ...
    | { status: 'dynamic', props: BlockPropertyMap }
...
type LookupBlockComponentParams = {
    blockName: string
...
type LookupBlockComponentResult =
    | { block: BlockMetadataModel, component: { props: BlockPropertyMap, requireAuth: boolean, name: string, displayName: string, description: string }, blockName: string, error?: never }
    | { error: McpToolResult, block?: never, component?: never, blockName?: never }

type ResolveRouterStepResult =
    | { routerStep: RouterAction, error?: never }
    | { error: McpToolResult, routerStep?: never }

type ResolveLatestBlockVersionResult =
    | { blockVersion: string, normalizedBlockName: string, error?: never }
    | { error: McpToolResult, blockVersion?: never, normalizedBlockName?: never }
