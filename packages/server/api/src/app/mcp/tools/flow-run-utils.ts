...
export async function executeFlowTest({ flowId, projectId, stepName, triggerTestData, log }: {
    ...
    if (!flow.version.trigger.valid) {
        return { content: [{ type: 'text', text: '❌ Flow trigger is not configured. Use ib_update_trigger to set up the trigger before testing.' }] }
    }
    ...
    if (!isFlowRunStateTerminal({ status: completedRun.status, ignoreInternalError: false })) {
        return {
            content: [{
                type: 'text',
                text: `⏳ Test still running after 120s. Run ID: ${completedRun.id} (status: ${completedRun.status}). Use ib_get_run to check results later.`,
            }],
        }
    }
    ...
    if (completedRun.status === FlowRunStatus.INTERNAL_ERROR && isNil(completedRun.steps)) {
        return {
            content: [{
                type: 'text',
                text: '❌ Test failed with INTERNAL_ERROR (no step data). Check that all steps are properly configured using ib_flow_structure.',
            }],
        }
    }
...
export async function executeAdhocAction({
    ...
    const normalizedBlockName = mcpUtils.normalizeBlockName(blockName)
    if (isNil(normalizedBlockName)) {
        return mcpUtils.mcpToolError('Validation failed', new Error('blockName is required'))
    }
    ...
    const { block, component: action, blockName: resolvedBlockName } = lookup
    ...
    const resolvedBlockVersion = blockVersion ?? block.version

    const { data: flow, error: flowError } = await tryCatch(
        () => flowService(log).create({
            projectId,
            request: { displayName: `__adhoc_${ibId()}__`, projectId },
        }),
    )
    if (flowError) {
    ...
    try {
        ...
        if (!isFlowRunStateTerminal({ status: completedRun.status, ignoreInternalError: false })) {
            return {
                content: [{
                    type: 'text',
                    text: `⏳ Action still running after 120s. Run ID: ${completedRun.id} (status: ${completedRun.status}). Use ib_get_run to check results later.`,
                }],
            }
        }
        ...
        if (completedRun.status === FlowRunStatus.INTERNAL_ERROR && isNil(completedRun.steps)) {
            return {
                content: [{
                    type: 'text',
                    text: `❌ ${action.displayName} failed with INTERNAL_ERROR (no step data) — the engine crashed while loading or executing the block. Run ID: ${completedRun.id}.`,
                }],
            }
        }
...
const FORMS_PIECE_NAME = '@intelblocks/block-forms'

function buildTriggerShapeHint(trigger: Step): string {
    if (trigger.type !== FlowTriggerType.BLOCK || trigger.settings.blockName !== FORMS_PIECE_NAME) {
        return ''
    }
...
function stepDataUnavailableReason(run: FlowRun): string {
    ...
    if (isOutsideRetentionWindow(run.created, retentionDays)) {
        return `not available — execution data is purged after ${retentionDays} days. Re-run the flow with ib_test_flow or ib_retry_run to capture fresh step data.`
    }
