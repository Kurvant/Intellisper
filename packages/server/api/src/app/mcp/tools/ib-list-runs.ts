import { FlowRunStatus, isNil, McpToolDefinition, Permission, ProjectScopedMcpServer, RunEnvironment } from '@intelblocks/shared'
...
const listRunsInput = z.object({
    flowId: z.string().optional().describe('Filter by flow ID. Use ib_list_flows to find it.'),
...
export const ibListRunsTool = (mcp: ProjectScopedMcpServer, log: FastifyBaseLogger): McpToolDefinition => {
    return {
        title: 'ib_list_runs',
        ...
        execute: async (args) => {
            try {
                ...
                log.error({ err, projectId: mcp.projectId }, 'ib_list_runs failed')
