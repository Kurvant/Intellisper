import { AIProviderModelType, AIProviderName, McpToolDefinition, ProjectScopedMcpServer } from '@intelblocks/shared'
...
export const ibListAiModelsTool = (mcp: ProjectScopedMcpServer, log: FastifyBaseLogger): McpToolDefinition => {
    return {
        title: 'ib_list_ai_models',
        ...
        execute: async (args) => {
            try {
                ...
                const sections = await Promise.all(
                    filteredProviders.map(async (p) => {
                        try {
                            ...
                            log.warn({ err, provider: p.provider }, 'ib_list_ai_models: failed to fetch models for provider')
                ...
                log.error({ err, projectId: mcp.projectId }, 'ib_list_ai_models failed')
