...
import { AIProviderName, isNil, McpToolDefinition, ProjectScopedMcpServer } from '@intelblocks/shared'
...
const setupGuideInput = z.object({
    ...
    blockName: z.string().optional().describe('For connections: the block that needs auth (e.g., "@intelblocks/block-gmail"). Omit for general instructions.'),
})

export const ibSetupGuideTool = (mcp: ProjectScopedMcpServer, log: FastifyBaseLogger): McpToolDefinition => {
    return {
        title: 'ib_setup_guide',
        ...
        execute: async (args) => {
            try {
                const { topic, blockName } = setupGuideInput.parse(args)

                if (topic === 'connection') {
                    return await connectionGuide(mcp, log, blockName)
                }
                return await aiProviderGuide(mcp, log)
            }
            catch (err) {
                log.error({ err, projectId: mcp.projectId }, 'ib_setup_guide failed')
...
async function connectionGuide(mcp: ProjectScopedMcpServer, log: FastifyBaseLogger, blockName?: string): Promise<{ content: [{ type: 'text', text: string }] }> {
    if (isNil(blockName)) {
        return {
            content: [{
                type: 'text',
                text: [
                    ...
                    '',
                    '1. Open your Intellisper dashboard',
                    '2. Go to Settings → Connections',
                    '3. Click "+ New Connection"',
                    '4. Select the block/app you want to connect',
                    '5. Follow the prompts (OAuth login, API key, etc.)',
                    '6. Save the connection',
                    '',
                    'Then use ib_list_connections to find the connection\'s externalId for use in flow steps.',
                    '',
                    'Tip: Pass blockName to this tool for specific instructions (e.g., blockName: "@intelblocks/block-gmail").',
                ].join('\n'),
            }],
        }
    }

    // Resolve platformId so private (CUSTOM) blocks on this platform are discoverable.
    ...
    if (isNil(block)) {
        return { content: [{ type: 'text', text: `❌ Block "${blockName}" not found. Use ib_research_blocks to find valid block names.` }] }
    }

    const rawAuth = block.auth
    if (isNil(rawAuth)) {
        return { content: [{ type: 'text', text: `✅ "${block.displayName}" does not require authentication. No connection setup needed.` }] }
    }

    const authOptions = Array.isArray(rawAuth) ? rawAuth : [rawAuth]

    if (authOptions.length > 1) {
        const lines: string[] = [`How to connect "${block.displayName}" (${authOptions.length} methods available):`, '']
        for (let i = 0; i < authOptions.length; i++) {
            lines.push(`**Option ${i + 1}: ${formatAuthTypeName(authOptions[i].type)}**`)
            lines.push(...formatAuthSteps({ auth: authOptions[i], displayName: block.displayName }))
            lines.push('')
        }
    ...
    const lines: string[] = [`How to connect "${block.displayName}":`, '']
    ...
    lines.push('', 'After setup, use ib_list_connections to find the connection\'s externalId for use in flow steps.')
...
async function aiProviderGuide(mcp: ProjectScopedMcpServer, log: FastifyBaseLogger): Promise<{ content: [{ type: 'text', text: string }] }> {
    ...
    if (providers.length > 0) {
        lines.push(
            ...
            '',
            'Use ib_list_ai_models to see available models.',
            '',
    ...
    lines.push(
        ...
        '5. Click Save',
        '',
        'After setup, use ib_list_ai_models to discover available models for Run Agent steps.',
    )
