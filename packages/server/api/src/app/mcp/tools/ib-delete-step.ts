...
export const ibDeleteStepTool = (mcp: ProjectScopedMcpServer, log: FastifyBaseLogger): McpToolDefinition => {
    return {
        title: 'ib_delete_step',
        permission: Permission.WRITE_FLOW,
        description: 'Delete a step from a flow. Prefer ib_update_step to modify — delete destroys sample data.',
        inputSchema: {
            flowId: z.string().describe('The id of the flow'),
            stepName: z.string().describe('The name of the step to delete. Use ib_flow_structure to get valid values.'),
        ...
        execute: async (args) => {
            ...
            if (flowStructureUtil.isTrigger(step.type)) {
                return { content: [{ type: 'text', text: `❌ "${stepName}" is the trigger step and cannot be deleted. Use ib_update_trigger to reconfigure it.` }] }
            }
