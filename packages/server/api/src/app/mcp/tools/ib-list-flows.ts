import { FlowStatus, FlowTriggerType, isNil, McpToolDefinition, Permission, PopulatedFlow, ProjectScopedMcpServer } from '@intelblocks/shared'
...
export const ibListFlowsTool = (mcp: ProjectScopedMcpServer, log: FastifyBaseLogger): McpToolDefinition => {
    return {
        title: 'ib_list_flows',
            ...
            try {
                ...
                const structured = {
                    flows: flows.data.map((flow) => ({
                        ...
                        triggerType: flow.version.trigger.type === FlowTriggerType.BLOCK
                            ? (flow.version.trigger.settings.blockName ?? 'piece (unconfigured)')
...
export function formatFlowLine(flow: PopulatedFlow): string {
    ...
    const triggerLabel = trigger.type === FlowTriggerType.BLOCK
        ? (trigger.settings.blockName ?? 'piece (unconfigured)')
        : 'no trigger'
