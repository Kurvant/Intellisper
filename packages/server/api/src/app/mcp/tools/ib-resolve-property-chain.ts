...
export const ibResolvePropertyChainTool = (mcp: ProjectScopedMcpServer, log: FastifyBaseLogger): McpToolDefinition => {
    return {
        title: 'ib_resolve_property_chain',
        ...
        execute: async (args) => {
            try {
                const { blockName, actionOrTriggerName, type, propertyChain, auth, currentInput: providedInput } = resolvePropertyChainInput.parse(args)
                ...
                if (isNil(firstPropDef)) {
                    return {
                        content: [{ type: 'text', text: `❌ Property "${propertyChain[0].propertyName}" not found on ${normalized}/${actionOrTriggerName}. Use ib_get_block_props to see available properties.` }],
                    }
                }
                const blockPackage = await getBlockPackageWithoutArchive(log, platformId, { blockName: normalized, blockVersion: block.version })
                ...
                for (const chainItem of propertyChain) {
                    ...
                    if (isNil(propDef)) {
                        return {
                            content: [{ type: 'text', text: `❌ Property "${chainItem.propertyName}" not found on ${normalized}/${actionOrTriggerName}. Use ib_get_block_props to see available properties.` }],
                        }
                    }

                    let result: EngineResponse<{ options: Array<{ label: string, value: unknown }> | BlockPropertyMap, disabled?: boolean }>
                    try {
                    ...
                    if (propDef.type === PropertyType.DYNAMIC && isObject(options) && !Array.isArray(options)) {
                        const dynamicFields = mcpUtils.buildPropSummaries(options as BlockPropertyMap)
                        resolvedProperties.push({
                            propertyName: chainItem.propertyName,
                            dynamicFields,
                            resolved: true,
...
const resolvePropertyChainInput = z.object({
    blockName: z.string().describe('The piece name (e.g. "@intelblocks/block-google-sheets").'),
