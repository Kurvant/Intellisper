...
const deleteTableInput = z.object({
    tableId: z.string().describe('The ID of the table to delete. Use ib_list_tables to find it.'),
...
export const ibDeleteTableTool = (mcp: ProjectScopedMcpServer, log: FastifyBaseLogger): McpToolDefinition => {
    return {
        title: 'ib_delete_table',
        ...
        execute: async (args) => {
            try {
                ...
                log.error({ err, projectId: mcp.projectId }, 'ib_delete_table failed')
