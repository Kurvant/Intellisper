...
const testFlowInput = z.object({
    flowId: z.string().describe('The ID of the flow to test. Use ib_list_flows to find it.'),
...
export const ibTestFlowTool = (mcp: ProjectScopedMcpServer, log: FastifyBaseLogger): McpToolDefinition => {
    return {
        title: 'ib_test_flow',
        ...
        execute: async (args) => {
            try {
                ...
                log.error({ err, projectId: mcp.projectId }, 'ib_test_flow failed')
