...
export const ibLockAndPublishTool = (mcp: ProjectScopedMcpServer, log: FastifyBaseLogger): McpToolDefinition => {
    return {
        title: 'ib_lock_and_publish',
        ...
        execute: async (args) => {
            ...
            if (invalidSteps.length > 0) {
                ...
                return {
                    content: [{
                        type: 'text',
                        text: `❌ Flow has invalid steps: ${stepList}. Fix these steps using ib_update_step or ib_update_trigger before publishing.`,
                    }],
                }
            }
