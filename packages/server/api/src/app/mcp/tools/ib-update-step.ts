...
export const ibUpdateStepTool = (mcp: ProjectScopedMcpServer, log: FastifyBaseLogger): McpToolDefinition => {
    return {
        title: 'ib_update_step',
        permission: Permission.WRITE_FLOW,
        description: 'Update an existing step\'s settings. Provide only the fields you want to change.',
        inputSchema: {
            flowId: z.string().describe('The id of the flow'),
            stepName: z.string().describe('The name of the step to update (e.g. "step_1"). Use ib_flow_structure to get valid values.'),
            ...
            actionName: z.string().optional().describe('For BLOCK steps: the action to perform. Use ib_research_blocks to get valid values.'),
            ...
            continueOnFailure: z.boolean().optional().describe('For CODE/BLOCK steps: set true on the step that can fail (the one whose failure you want to react to), NOT on the recovery step. The flow keeps running on failure and the step gains On success / On failure branches — add handler steps into them with ib_add_step using stepLocationRelativeToParent INSIDE_ON_SUCCESS_BRANCH / INSIDE_ON_FAILURE_BRANCH and parentStepName = this step.'),
            retryOnFailure: z.boolean().optional().describe('For CODE/BLOCK steps: whether to retry this step on failure.'),
        },
        ...
        execute: async (args) => {
            ...
            if (flowStructureUtil.isTrigger(step.type)) {
                return {
                    content: [{ type: 'text', text: `❌ "${stepName}" is a trigger step. Use ib_update_trigger to configure triggers.` }],
                }
            }
            ...
            if (actionName !== undefined) {
                if (step.type === FlowActionType.BLOCK) {
                    ...
                    return { content: [{ type: 'text', text: `❌ actionName can only be set on BLOCK steps, but "${stepName}" is type ${step.type}.` }] }
                }
            }
            ...
            if (continueOnFailure !== undefined || retryOnFailure !== undefined) {
                if (step.type !== FlowActionType.CODE && step.type !== FlowActionType.BLOCK) {
                    return { content: [{ type: 'text', text: `❌ continueOnFailure/retryOnFailure can only be set on CODE or BLOCK steps, but "${stepName}" is type ${step.type}.` }] }
                }
            ...
            if (step.type === FlowActionType.BLOCK && input !== undefined) {
            ...
            try {
                ...
                if (updatedStep && !updatedStep.valid) {
                    const diagnosis = updatedStep.type === FlowActionType.BLOCK
                    ...
                    const hint = (diagnosis || null)
                        ?? (step.type === FlowActionType.BLOCK
                            ? 'Use ib_research_blocks to verify blockName, blockVersion, actionName and required inputs, then retry.'
...
async function diagnoseMissingInputs({ settings, platformId, log }: {
    ...
    const { blockName, blockVersion, actionName } = settings
    if (isNil(actionName)) {
    ...
    try {
        ...
        if (isNil(action)) {
            return `Action "${actionName}" not found in block "${blockName}". Use ib_research_blocks with includeActions=true to get valid action names.`
        }
        const input = settings.input ?? {}
        const { parts } = mcpUtils.diagnoseBlockProps({ props: action.props, input, blockAuth: block.auth, requireAuth: action.requireAuth, componentType: 'action' })
        return parts.join(' ')
    }
    catch (err) {
        log.warn({ err, blockName, actionName }, 'diagnoseMissingInputs: failed to fetch block metadata')
        return null
    }
}
