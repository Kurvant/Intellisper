import { FlowStatus, isNil, McpProperty, McpPropertyType, McpToolDefinition, mcpToolNameUtils, McpTrigger, Permission, PopulatedMcpServer, ProjectScopedMcpServer, TelemetryEventName } from '@intelblocks/shared'
...
const MCP_SERVER_INSTRUCTIONS = `## Intellisper MCP Server

### Workflow
1. Discover: ib_research_blocks, ib_list_connections, ib_list_ai_models
2. Schema: ib_get_block_props (get field names/types before configuring)
3. Build: ib_build_flow (one call for new flows) OR ib_create_flow → ib_update_trigger → ib_add_step (granular)
4. Validate: ib_validate_flow
5. Publish: ib_lock_and_publish → ib_change_flow_status

### Key patterns
- **Auth**: ib_list_connections → get \`externalId\` → pass as \`auth\` param on ib_update_step/ib_update_trigger.
...
- **Step names**: \`trigger\`, \`step_1\`, \`step_2\`, etc. Use ib_flow_structure to see all names.
- **Block names**: full format (e.g. "@intelblocks/block-slack") for ib_add_step/ib_update_trigger. Short names work for lookup tools.
- **Modifying steps**: use ib_update_step/ib_update_trigger. Never delete+recreate — loses sample data.
...
export async function buildMcpServer({ mcp, userId, selectionScope, log, resolveProjectMcp }: {
    ...
    // Self-hosted logo, absolute so external MCP clients can load it.
    const logoUrl = await domainHelper.getPublicUrl({ path: '/intellisper-logo.png' })
    const websiteUrl = await domainHelper.getPublicUrl({ path: '/' })
    ...
    const server = new McpServer({
        ...
        version: '1.0.0',
        websiteUrl,
        description: 'Automation and workflow MCP server by Intellisper',
        icons: [
            {
                src: logoUrl,
                mimeType: 'image/png',
...
function registerPlatformTools({ server, mcp, userId, selectionScope, resolveProjectMcp, log }: {
    ...
    const contextTool = ibSetProjectContextTool({ platformId, userId, selectionScope, log })
    ...
    const allTools = intellisperTools(templateMcp, userId, log)
        ...
        server.registerTool(tool.title, buildToolConfig(tool), async (args: Record<string, unknown>) => {
            ...
            if (isNil(selectedProjectId)) {
                return {
                    content: [{
                        type: 'text' as const,
                        text: 'No project selected. Use ib_set_project_context to select a project first.',
                    }],
                }
            }
            ...
            const realTools = intellisperTools(projectScopedMcp, userId, log)
...
function registerStaticTools({ server, mcp, projectId, userId, permissionChecker, log }: RegisterToolsParams): void {
    const allTools = intellisperTools({ ...mcp, projectId }, userId, log)
    const disabledToolSet = new Set(mcp.disabledTools ?? [])
    const tools = allTools.filter(t => LOCKED_TOOL_NAMES.includes(t.title) || !disabledToolSet.has(t.title))
...
function registerEmptyResourcesAndPrompts(server: McpServer): void {
    server.registerResource(
        '_',
        new ResourceTemplate('intellisper://empty', {
