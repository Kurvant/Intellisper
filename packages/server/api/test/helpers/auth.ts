import { ibId, DefaultProjectRole, Principal, SigningKeyId } from '@intelblocks/shared'
import { faker } from '@faker-js/faker'
import jwt, { Algorithm, JwtPayload, SignOptions } from 'jsonwebtoken'
import {
    ExternalPrincipal,
    ExternalTokenPayload,
} from '../../src/app/enterprise/managed-authn/lib/external-token-extractor'
...
export const generateMockExternalToken = (
    ...
    const mockExternalTokenPayload: ExternalTokenPayload = {
        externalUserId: params?.externalUserId ?? ibId(),
        role: params?.projectRole as DefaultProjectRole ?? DefaultProjectRole.ADMIN,
        externalProjectId: params?.externalProjectId ?? ibId(),
    ...
    const keyId = params?.signingKeyId ?? ibId()
