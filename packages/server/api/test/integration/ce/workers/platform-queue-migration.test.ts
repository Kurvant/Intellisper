import { ibId, FlowTriggerType, LATEST_JOB_DATA_SCHEMA_VERSION, WorkerJobType } from '@intelblocks/shared'
...
describe('platformQueueMigrationService', () => {
    ...
    beforeEach(() => {
        fromQueueName = getWorkerGroupQueueName(ibId())
        toQueueName = getWorkerGroupQueueName(ibId())
    })
    ...
    it('returns early without touching queues when fromQueueName equals toQueueName', async () => {
        ...
        const flowVersionId = ibId()
        const sameQueue = getWorkerGroupQueueName(ibId())
    ...
    it('does not migrate schedulers belonging to a different platform', async () => {
        const platformA = ibId()
        const platformB = ibId()
        const flowVersionA = ibId()
        const flowVersionB = ibId()
        const fromQueue = await jobQueue(app.log).getOrCreateQueue({ queueName: fromQueueName })
        const toQueue = await jobQueue(app.log).getOrCreateQueue({ queueName: toQueueName })
    ...
    it('moves regular (one-time) waiting jobs for the platform', async () => {
        ...
        await fromQueue.add(jobId, buildPollingJobData({ platformId, flowVersionId: ibId() }), { jobId })
    ...
    it('does not move regular jobs belonging to a different platform', async () => {
        ...
        const jobIdA = ibId()
        const jobIdB = ibId()
        await fromQueue.add(jobIdA, buildPollingJobData({ platformId: platformA, flowVersionId: ibId() }), { jobId: jobIdA })
        await fromQueue.add(jobIdB, buildPollingJobData({ platformId: platformB, flowVersionId: ibId() }), { jobId: jobIdB })
...
function buildPollingJobData({ platformId, flowVersionId }: { platformId: string, flowVersionId: string }) {
    return {
        ...
        flowVersionId,
        flowId: ibId(),
        triggerType: FlowTriggerType.BLOCK,
