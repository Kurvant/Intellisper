...
async function setupSubflowFixtures() {
    ...
    const webhookPiece = createMockBlockMetadata({
    ...
    const subflowsPiece = createMockBlockMetadata({
        name: '@intelblocks/block-subflows',
        version: '0.4.11',
        platformId: undefined,
        packageType: PackageType.REGISTRY,
        blockType: BlockType.OFFICIAL,
    })
    await databaseConnection().getRepository('block_metadata').save([webhookPiece, subflowsPiece])
    ...
    const childReturnResponseAction = {
        ...
        valid: true,
        settings: {
            blockName: '@intelblocks/block-subflows',
            blockVersion: '0.4.11',
...
describe('Execute Flow E2E', () => {
    it('executes a webhook → data mapper → code flow end-to-end', async () => {
        ...
        const dataMapperPiece = createMockBlockMetadata({
            name: '@intelblocks/block-data-mapper',
            version: '0.3.15',
            platformId: undefined,
            packageType: PackageType.REGISTRY,
            blockType: BlockType.OFFICIAL,
        })
        await databaseConnection().getRepository('block_metadata').save([webhookPiece, dataMapperPiece])
        ...
        const dataMapperAction = {
            ...
            valid: true,
            settings: {
                blockName: '@intelblocks/block-data-mapper',
                blockVersion: '0.3.15',
    ...
    it('handles concurrent flow run executions without jobs getting stuck', async () => {
        ...
        await databaseConnection().getRepository('block_metadata').save([webhookPiece])
    ...
    it('executes a webhook → delay_for → code flow without infinite loop', async () => {
        ...
        const delayPiece = createMockBlockMetadata({
            name: '@intelblocks/block-delay',
            version: '0.3.26',
            platformId: undefined,
            packageType: PackageType.REGISTRY,
            blockType: BlockType.OFFICIAL,
        })
        await databaseConnection().getRepository('block_metadata').save([webhookPiece, delayPiece])
        ...
        const delayAction = {
            ...
            valid: true,
            settings: {
                blockName: '@intelblocks/block-delay',
                blockVersion: '0.3.26',
                actionName: 'delayFor',
                input: {
                    unit: 'seconds',
