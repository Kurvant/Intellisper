import { ibId, ExecutionType, FlowRunStatus, FlowVersionState, StreamStepProgress, RunEnvironment } from '@intelblocks/shared'
...
describe('Resume flow run', () => {
    it('should resume legacy PAUSED flow with no waitpoint via async endpoint', async () => {
        ...
        const response = await app.inject({
            method: 'POST',
            url: `/api/v1/flow-runs/${flowRun.id}/requests/${ibId()}`,
            body: { data: 'test' },
        })
    ...
    it('V0 sync: should return 409 when flow run is in terminal state', async () => {
        ...
        const response = await app.inject({
            method: 'POST',
            url: `/api/v1/flow-runs/${flowRun.id}/requests/${ibId()}/sync`,
            body: { data: 'test' },
        })
