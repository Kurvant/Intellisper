    ...
    describe('POST /v1/flows/:id UPDATE_TRIGGER', () => {
        it('should update trigger to piece trigger', async () => {
            ...
            const mockPiece = createMockBlockMetadata({
                name: '@intelblocks/block-schedule',
                version: '0.2.0',
            ...
            const response = await ctx.post(`/v1/flows/${flow.id}`, {
                ...
                request: {
                    ...
                    settings: {
                        blockName: '@intelblocks/block-schedule',
                        blockVersion: '0.2.0',
                        input: {},
            ...
            expect(response?.statusCode).toBe(StatusCodes.OK)
            const body = response?.json()
            expect(body.version.trigger.type).toBe(FlowTriggerType.BLOCK)
            expect(body.version.trigger.settings.blockName).toBe('@intelblocks/block-schedule')
        })
    })
