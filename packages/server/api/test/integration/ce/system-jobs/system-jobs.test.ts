...
afterEach(async () => {
    ...
    for (const s of schedulers) {
        ...
        if (key.startsWith(TEST_PREFIX) || key.includes('::') || key === 'blocks-analytics') {
...
describe('System Jobs', () => {
    ...
    it('should keep new-format schedulers while removing legacy ones', async () => {
        ...
        const legacyKey = `${SystemJobName.BLOCKS_ANALYTICS}::0:UTC:0 12 * * *`
        await systemJobsQueue.upsertJobScheduler(legacyKey, {
            pattern: '0 12 * * *',
            tz: 'UTC',
        }, {
        ...
        const before = await systemJobsQueue.getJobSchedulers()
        const analyticsBefore = before.filter(s => s.name === SystemJobName.BLOCKS_ANALYTICS)
        ...
        const after = await systemJobsQueue.getJobSchedulers()
        const legacyAfter = after.filter(
            s => s.name === SystemJobName.BLOCKS_ANALYTICS && s.key.includes('::'),
        )
