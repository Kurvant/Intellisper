...
const mockPieceMetadata = {
    ...
    displayName: 'Discord Test Block',
...
describe('POST /v1/pieces — private piece installation', () => {
    it('should install a private piece from a tgz archive and persist metadata', async () => {
        ...
        formData.append('blockName', PIECE_NAME)
        formData.append('blockVersion', PIECE_VERSION)
        formData.append('packageType', PackageType.ARCHIVE)
        formData.append('scope', BlockScope.PLATFORM)
        ...
        const saved = await blockMetadataService(mockLog).getOrThrow({
            name: PIECE_NAME,
            version: PIECE_VERSION,
        ...
        expect(saved.blockType).toBe(BlockType.CUSTOM)
