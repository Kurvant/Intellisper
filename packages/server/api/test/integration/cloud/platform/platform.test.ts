...
import { ibId, IbEdition, FileCompression, FileLocation, FileType, FilteredBlockBehavior,
    FlowOperationStatus,
    FlowStatus,
    PlanName,
    PlatformRole,
...
describe('Platform API', () => {
    describe('update platform endpoint', () => {
        it('patches a platform by id', async () => {
            ...
            const requestBody: UpdatePlatformRequestBody = {
                ...
                filteredBlockNames: ['updated filtered piece names'],
            ...
            expect(responseBody.filteredBlockNames).toStrictEqual([
                'updated filtered piece names',
            ])
            expect(responseBody.filteredBlockBehavior).toBe('ALLOWED')
        ...
        it('updates platform with boolean and array fields via multipart form data', async () => {
            ...
            formData.append('filteredBlockNames', 'piece-1')
            formData.append('allowedAuthDomains', 'example.com')
            formData.append('pinnedBlocks', 'pinned-1')
            formData.append('name', 'updated name')
            formData.append('filteredBlockBehavior', 'ALLOWED')
            ...
            expect(responseBody.filteredBlockNames).toStrictEqual(['piece-1'])
            expect(responseBody.allowedAuthDomains).toStrictEqual(['example.com'])
            expect(responseBody.pinnedBlocks).toStrictEqual(['pinned-1'])
            expect(responseBody.name).toBe('updated name')
            expect(responseBody.filteredBlockBehavior).toBe('ALLOWED')
    ...
    describe('get platform asset endpoint', () => {
        ...
        it('returns 404 for unknown asset ids', async () => {
            const response = await app?.inject({
                method: 'GET',
                url: `/api/v1/platforms/assets/${ibId()}`,
            })
            expect(response?.statusCode).toBe(StatusCodes.NOT_FOUND)
        })
    })
