...
import { ibSetupGuideTool } from '../../../../src/app/mcp/tools/ib-setup-guide'
...
describe('MCP Tool RBAC', () => {
    describe('with userId (OAuth path)', () => {
        it('EDITOR can execute write tools (ib_create_flow)', async () => {
            ...
            const tool = ibCreateFlowTool({ mcp, userId: memberCtx.user.id }, mockLog)
        ...
        it('VIEWER cannot execute write tools (ib_create_flow)', async () => {
            const ctx = await createTestContext(app)
            const memberCtx = await createMemberContext(app, ctx, { projectRole: DefaultProjectRole.VIEWER })

            const checker = await resolvePermissionChecker({ userId: memberCtx.user.id, projectId: ctx.project.id, log: mockLog })
            const error = checker.check(Permission.WRITE_FLOW, 'ib_create_flow')
        ...
        it('VIEWER can execute read tools (ib_list_flows)', async () => {
            const ctx = await createTestContext(app)
            const memberCtx = await createMemberContext(app, ctx, { projectRole: DefaultProjectRole.VIEWER })
            const mcp = makeMcp(ctx.project.id)

            const checker = await resolvePermissionChecker({ userId: memberCtx.user.id, projectId: ctx.project.id, log: mockLog })
            const tool = ibListFlowsTool(mcp, mockLog)
            const execute = checker.wrapExecute({ execute: tool.execute, permission: tool.permission, toolTitle: tool.title })
        ...
        it('VIEWER cannot execute UPDATE_FLOW_STATUS tools', async () => {
            ...
            const checker = await resolvePermissionChecker({ userId: memberCtx.user.id, projectId: ctx.project.id, log: mockLog })
            const error = checker.check(Permission.UPDATE_FLOW_STATUS, 'ib_lock_and_publish')
        ...
        it('any role can execute tools with no permission requirement', async () => {
            ...
            const checker = await resolvePermissionChecker({ userId: memberCtx.user.id, projectId: ctx.project.id, log: mockLog })
            const tool = ibSetupGuideTool(mcp, mockLog)
