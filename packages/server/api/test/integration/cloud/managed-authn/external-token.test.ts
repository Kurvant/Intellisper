...
import { ibId, DefaultProjectRole, BlocksFilterType, BlockType, ProjectRole } from '@intelblocks/shared'
...
describe('Managed Authentication API', () => {
    describe('External token endpoint', () => {
        ...
        it('Sync Pieces when exchanging external token', async () => {
            ...
            const mockPieceMetadata1 = createMockBlockMetadata({
                name: '@ap/a',
                version: '0.0.1',
                blockType: BlockType.OFFICIAL,
            })
            await db.save('block_metadata', mockPieceMetadata1)
            ...
            const mockPieceTag = createMockBlockTag({
                platformId: mockPlatform.id,
                tagId: mockTag.id,
                blockName: '@ap/a',
            })

            await db.save('block_tag', mockPieceTag)
            ...
            const { mockExternalToken } = generateMockExternalToken({
                ...
                signingKeyId: mockSigningKey.id,
                blocks: {
                    filterType: BlocksFilterType.ALLOWED,
                    tags: ['free'],
                },
            })
            ...
            expect(generatedProject?.blocksFilterType).toBe('ALLOWED')
            expect(generatedProject?.blocks).toStrictEqual(['@ap/a'])
        })
        ...
        it('Adds new user to existing project', async () => {
            ...
            const mockExternalProjectId = ibId()
        ...
        it('Fails if signing key is not found', async () => {
            ...
            const nonExistentSigningKeyId = ibId()
