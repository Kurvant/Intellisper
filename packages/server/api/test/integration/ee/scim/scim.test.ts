import { ibId, PrincipalType, ProjectType } from '@intelblocks/shared'
...
function mockIdpUser(overrides?: {
    ...
    return {
        ...
        externalId: overrides?.externalId ?? `idp-user-${ibId()}`,
...
function mockIdpGroup(overrides?: {
    ...
    return {
        ...
        externalId: overrides?.externalId ?? `idp-group-${ibId()}`,
    ...
    describe('Full IdP Lifecycle Simulation', () => {
        ...
        it('should handle a complete Okta-style provisioning flow', async () => {
            ...
            const engineerAlice = mockIdpUser({
                ...
                email: `alice-${ibId()}@example.com`,
                firstName: 'Alice',
                lastName: 'Engineer',
            })
            const engineerBob = mockIdpUser({
                externalId: 'okta-bob-002',
                email: `bob-${ibId()}@example.com`,
                firstName: 'Bob',
                lastName: 'Developer',
            })
