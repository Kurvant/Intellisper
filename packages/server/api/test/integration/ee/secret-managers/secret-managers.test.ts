...
import { secretManagersService } from '../../../../src/app/enterprise/secret-managers/secret-managers.service'
import { generateMockToken } from '../../../helpers/auth'
import { mockAndSaveBasicSetup, mockBlockMetadata } from '../../../helpers/mocks'
import {
    hashicorpMock,
    mockVaultConfig,
} from './hashicorp-mock'
import { AppConnectionScope, AppConnectionType, ErrorCode, PrincipalType, SecretManagerConnectionScope, SecretManagerFieldsSeparator, SecretManagerProviderId, UpsertGlobalConnectionRequestBody } from '@intelblocks/shared'
import { validatePathFormat } from 'packages/server/api/src/app/enterprise/secret-managers/secret-manager-providers/hashicorp-provider'
...
describe('Secret Managers API', () => {
    ...
    describe('Resolve Secret', () => {
        ...
        it('should not allow persisting resolved secrets in the database', async () => {
            const blockMetadata = await mockBlockMetadata(mockLog)
            const { mockOwner, mockPlatform, mockProject } = await mockAndSaveBasicSetup({
                plan: {
                    secretManagersEnabled: true,
