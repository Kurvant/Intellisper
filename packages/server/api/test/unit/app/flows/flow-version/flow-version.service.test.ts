...
function makePieceTriggerSettings(extras: Partial<BlockTrigger['settings']> = {}): BlockTrigger['settings'] {
    return {
...
describe('flowVersionService.applyOperation - USE_AS_DRAFT', () => {
    ...
    it('preserves PIECE trigger sample data from the previous version', async () => {
        ...
        expect(result.trigger.type).toBe(FlowTriggerType.BLOCK)
        const settings = (result.trigger as BlockTrigger).settings
    ...
    it('does not set trigger sample data when previous version has no sampleData', async () => {
        ...
        expect(result.trigger.type).toBe(FlowTriggerType.BLOCK)
        expect((result.trigger as BlockTrigger).settings.sampleData).toBeUndefined()
    })
