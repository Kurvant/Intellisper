...
import { resolveSamlAttributes } from '../../../../../../src/app/enterprise/authentication/saml-authn/saml-attributes'
...
describe('resolveSamlAttributes', () => {
    ...
    describe('error reporting', () => {
        it('throws INVALID_SAML_RESPONSE listing missing fields and received keys', () => {
            ...
            expect(run).toThrow(IntellisperError)

            const error = captureIntellisperError(run)
        ...
        it('throws when rawAttributes is null or undefined', () => {
            expect(() => resolveSamlAttributes({ rawAttributes: null })).toThrow(IntellisperError)
            expect(() => resolveSamlAttributes({ rawAttributes: undefined })).toThrow(IntellisperError)
        })
        ...
        it('treats empty-string and null-valued claims as missing', () => {
            ...
            })).toThrow(IntellisperError)
        })
    })
})

function captureIntellisperError(run: () => unknown): IntellisperError {
    try {
        run()
    }
    catch (error) {
        if (error instanceof IntellisperError) {
            return error
        }
        throw error
    }
    throw new Error('Expected IntellisperError to be thrown')
}

function readMessage(error: IntellisperError): string {
