    ...
    for (const part of content) {
        switch (part.type) {
            ...
            case 'tool-call': {
                ...
                if (toolName === 'ib_update_thinking_status') {
                ...
                if (toolName === 'ib_execute_action' && typeof rawOutput === 'object' && rawOutput !== null && 'batchProgress' in rawOutput) {
                    parts.push({
                        type: PersistedChatPartType.BATCH_PROGRESS,
                        data: (rawOutput as Record<string, unknown>)['batchProgress'] as Record<string, unknown>,
                    })
                }
                if (toolName === 'ib_execute_action' && result) {
                    ...
                    parts.push({
                        ...
                        blockName: typeof input['pieceName'] === 'string' ? input['pieceName'] : '',
