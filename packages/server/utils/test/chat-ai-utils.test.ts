...
describe('sanitizeTruncatedAssistantTail', () => {
    it('returns messages unchanged when the last message is not an assistant message', () => {
        const messages: ModelMessage[] = [
            { role: 'assistant', content: [{ type: 'tool-call', toolCallId: 'call_1', toolName: 'ib_research_blocks', input: {} }] },
            { role: 'tool', content: [{ type: 'tool-result', toolCallId: 'call_1', toolName: 'ib_research_blocks', output: { type: 'json', value: { ok: true } } }] },
        ]
        ...
        const messages: ModelMessage[] = [
            {
                ...
                content: [
                    ...
                    { type: 'tool-call', toolCallId: 'call_truncated', toolName: 'ib_show_questions', input: {} },
                ],
            },
        ]
        ...
        const messages: ModelMessage[] = [
            {
                role: 'assistant',
                content: [
                    { type: 'tool-call', toolCallId: 'call_done', toolName: 'ib_get_block_props', input: {} },
                ],
            },
            { role: 'tool', content: [{ type: 'tool-result', toolCallId: 'call_done', toolName: 'ib_get_block_props', output: { type: 'json', value: {} } }] },
        ]
        expect(sanitizeTruncatedAssistantTail(messages)).toEqual(messages)
    })
        ...
        const messages: ModelMessage[] = [
            { role: 'user', content: 'hello' },
            {
                ...
                content: [
                    ...
                    { type: 'tool-call', toolCallId: 'call_x', toolName: 'ib_build_flow', input: {} },
                ],
            },
        ]
        const result = sanitizeTruncatedAssistantTail(messages)
    ...
    it('leaves earlier completed messages untouched', () => {
        const earlierAssistant: ModelMessage = {
            ...
            content: [
                ...
                { type: 'tool-call', toolCallId: 'call_a', toolName: 'ib_research_blocks', input: {} },
            ],
        }
        const toolResult: ModelMessage = {
            role: 'tool',
            content: [{ type: 'tool-result', toolCallId: 'call_a', toolName: 'ib_research_blocks', output: { type: 'json', value: {} } }],
        }
...
describe('collectStepMessages', () => {
    const listCall: ModelMessage = {
        role: 'assistant',
        content: [{ type: 'tool-call', toolCallId: 'call_list', toolName: 'ib_list_tables', input: {} }],
    }
    const listResult: ModelMessage = {
        role: 'tool',
        content: [{ type: 'tool-result', toolCallId: 'call_list', toolName: 'ib_list_tables', output: { type: 'json', value: { tables: ['leads', 'orders'] } } }],
    }
