...
import { IntellisperError, assertNotNullOrUndefined, createNotifyServer, createRpcClient, createRpcServer, EngineContract, EngineOperation, EngineOperationType, EngineResponse, EngineStderr, EngineStdout, ErrorCode, isNil, WorkerContract, WorkerNotifyContract } from '@intelblocks/shared'
...
export function createSandbox(
    ...
    return {
        ...
        start: async ({ flowVersionId, platformId, mounts }) => {
            ...
            const customBlockMounts: SandboxMount[] = []
            if (platformId) {
                assertSafePathSegment(platformId, 'platformId')
                const customBlocksHostPath = path.resolve(getGlobalCachePathLatestVersion(), 'custom_blocks', platformId)
                customBlockMounts.push({
                    hostPath: customBlocksHostPath,
            ...
            childProcess = await processMaker.create({
                ...
                env: {
                    ...options.env,
                    IB_SANDBOX_WS_PORT: String(port),
                    IB_SANDBOX_WS_TOKEN: wsRpcToken,
                    ...(customBlockMounts.length > 0
...
type ProcessExitParams = {
    ...
    stdOut: string
    stdError: string
    reject: (error: IntellisperError) => void
}
