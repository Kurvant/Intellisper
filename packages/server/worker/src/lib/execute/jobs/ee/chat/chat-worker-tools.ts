...
function normalizeBlockName(block: string): string {
    ...
    const stripped = block.startsWith('block-') ? block.slice('block-'.length) : block
...
function createDisplayTools({ waitForApproval, displayToolTimeoutMs, onConnectionSelected, onGateOpened, log }: {
    ...
    onConnectionSelected?: (params: { blockName: string, connectionExternalId: string, label: string, projectId: string }) => Promise<void>
    ...
    return {
        ib_show_connection_required: tool({
            description: 'Display a card prompting the user to connect a service. Use when no connection exists for a required block. After the user connects, briefly confirm before proceeding. If the user dismisses, respect their decision — do not proceed without the connection.',
            inputSchema: z.object({
                block: z.string().describe('Block short name (e.g. "gmail", "slack")'),
            ...
            execute: blockingExecute({
                toolName: 'ib_show_connection_required',
                ...
                onApproved: async ({ input, payload }) => {
                    ...
                    const displayName = typeof input['displayName'] === 'string' ? input['displayName'] : block
                        ...
                        if (Array.isArray(connections)) {
                            await Promise.all(connections
                                ...
                                .map((conn) => onConnectionSelected({
                                    blockName: normalizeBlockName(typeof conn['block'] === 'string' ? conn['block'] : block),
                            ...
                            log?.warn({ block, payload }, 'ib_show_connection_required approved but payload missing connections array')
                        }
                    }
        ...
        ib_show_connection_picker: tool({
            description: 'Display a card for the user to choose which connection to use. The system manages connection details — just provide the block name. After selection, briefly confirm the account chosen. If the user dismisses without selecting, do not pick a connection on their behalf.',
            inputSchema: z.object({
                block: z.string().describe('Block short name'),
                displayName: z.string().describe('Human-readable block name'),
            }),
            execute: blockingExecute({
                toolName: 'ib_show_connection_picker',
                ...
                onApproved: async ({ input, payload = {} }) => {
                    ...
                    if (typeof connectionExternalId === 'string' && onConnectionSelected) {
                        await onConnectionSelected({
                            blockName: normalizeBlockName(typeof input['block'] === 'string' ? input['block'] : ''),
        ...
        ib_show_project_picker: tool({
            ...
            execute: blockingExecute({ dismissMessage: 'The user chose not to select a project. Ask which project they would like to work in, or if they need help deciding.', successKey: 'selected', toolName: 'ib_show_project_picker' }),
        }),

        ib_show_questions: tool({
            ...
            execute: blockingExecute({ dismissMessage: 'The user skipped these questions. Proceed with reasonable defaults where possible, and let the user know what assumptions you made.', successKey: 'answered', toolName: 'ib_show_questions' }),
        }),

        ib_show_quick_replies: tool({
...
function createCrossProjectTools({ executeTool, eventEmitter, waitForApproval, onGateOpened, guides }: {
    ...
    return {
        ib_discover_action_auth: tool({
            description: 'Check what authentication a block needs and find available connections. Call this BEFORE ib_execute_action to determine if auth is needed.',
            inputSchema: z.object({
                ...
                blockName: z.string().describe('Block name, e.g. "@intelblocks/block-gmail"'),
            }),
            execute: async (input) => {
                return executeWithTimeout('ib_discover_action_auth', input)
            },
        }),

        ib_execute_action: tool({
            description: 'Execute a block action once or in batch. Use ib_discover_action_auth first to check if auth is needed. The system manages connections automatically after the user selects one. For batch execution, provide an items array where each element is a complete input object for one invocation.',
            inputSchema: z.object({
            ...
            execute: async (toolInput, options) => {
                ...
                const rawResult = await executeWithTimeout('ib_execute_action', toolInput)
        ...
        ib_list_across_projects: tool({
            ...
            execute: async (input) => {
                return executeWithTimeout('ib_list_across_projects', input)
            },
        }),

        ib_explore_data: tool({
            description: 'Read-only look at the user\'s real data during discovery — list/get/search/read a sheet\'s rows and columns, channels, records, etc. — to understand what they have and build something that fits. Only runs read actions (never writes). Needs a connection like ib_execute_action; ensure one is selected first. Keep samples small (~20 rows). This is for understanding, NOT for performing the task — use ib_execute_action to actually do things.',
            inputSchema: z.object({
                title: z.string().optional().describe('Short human-friendly label, e.g. "Look at sheet", "List channels"'),
                blockName: z.string().describe('Block name, e.g. "@intelblocks/block-google-sheets"'),
            ...
            execute: async (toolInput) => {
                ...
                const rawResult = await executeWithTimeout('ib_explore_data', toolInput)
...
async function executeBatchAction({ executeWithTimeout, eventEmitter, toolCallId, blockName, actionName, items, description }: {
    ...
    for (const batch of chunks) {
        ...
        const batchResults = await Promise.all(
            batch.map(async (item, offset) => {
                ...
                const { data: result, error } = await tryCatch(() => executeWithTimeout('ib_execute_action', {
                    blockName, actionName, input: item,
                }))
...
function createThinkingTools(): ToolSet {
    return {
        ib_update_thinking_status: tool({
