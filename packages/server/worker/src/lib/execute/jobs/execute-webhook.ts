...
import { provisionFlowBlocks } from '../utils/flow-helpers'
...
function getAppWebhookDetails(flowVersion: FlowVersion, publicApiUrl: string, appWebhookSecretsJson: string): { appWebhookUrl?: string, webhookSecret?: string | Record<string, string> } {
    const trigger = flowVersion.trigger as BlockTrigger
    const blockName = trigger?.settings?.blockName
    if (isNil(blockName)) {
        return {}
    }
    ...
    const webhookSecret = secrets?.[blockName]?.webhookSecret
    const blockUrlName = blockName.replace('@intelblocks/block-', '')
    return {
        appWebhookUrl: getAppWebhookUrl(publicApiUrl, blockUrlName),
        webhookSecret,
    }
}
...
export const executeWebhookJob: JobHandler<WebhookJobData, FireAndForgetJobResult> = {
    ...
    async execute(ctx: JobContext, data: WebhookJobData): Promise<FireAndForgetJobResult> {
        ...
        const provisioned = await provisionFlowBlocks({ flowVersion, platformId: data.platformId, flowId: data.flowId, projectId: data.projectId, log: ctx.log, apiClient: ctx.apiClient })
        if (!provisioned) {
            return { kind: JobResultKind.FIRE_AND_FORGET, status: EngineResponseStatus.OK }
        }
