...
import { onCallService } from '@intelblocks/server-utils'
import {
    IntellisperError,
...
export const executeFlowJob: JobHandler<ExecuteFlowJobData, FireAndForgetJobResult> = {
    ...
    async execute(ctx: JobContext, data: ExecuteFlowJobData): Promise<FireAndForgetJobResult> {
        ...
        const { data: provisioned, error: provisionError } = await tryCatch(() => provisionFlowBlocks({ flowVersion, platformId: data.platformId, flowId: data.flowId, projectId: data.projectId, log: ctx.log, apiClient: ctx.apiClient }))
        if (provisionError) {
        ...
        try {
            ...
            if (e instanceof IntellisperError) {
...
function toInternalError(source: RunInternalErrorSource, error: unknown): RunInternalError {
    const isApError = error instanceof IntellisperError
