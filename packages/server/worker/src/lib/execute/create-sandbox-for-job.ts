import { ExecutionMode, maxSocketHttpBufferSizeBytes, NetworkMode, WorkerContract, WorkerToApiContract } from '@intelblocks/shared'
...
function buildSandboxEnv({ settings, proxyPort }: {
    ...
    // drop IB_EGRESS_PROXY_URL from the sandbox env while iptables stays in place —
...
function baseEnv({ settings, networkMode }: { settings: WorkerSettings, networkMode: NetworkMode }): Record<string, string> {
    return {
        HOME: '/tmp/',
        IB_EXECUTION_MODE: settings.EXECUTION_MODE,
        IB_MAX_FLOW_RUN_LOG_SIZE_MB: String(settings.MAX_FLOW_RUN_LOG_SIZE_MB),
        IB_MAX_FILE_SIZE_MB: String(settings.MAX_FILE_SIZE_MB),
        NODE_PATH: '/usr/src/node_modules',
        IB_NETWORK_MODE: networkMode,
    }
}

function ssrfEnv(settings: WorkerSettings): Record<string, string> {
    const env: Record<string, string> = {}
    if (settings.DEV_BLOCKS.length > 0) {
        env['IB_DEV_BLOCKS'] = settings.DEV_BLOCKS.join(',')
    }
    if (settings.SSRF_ALLOW_LIST.length > 0) {
        env['IB_SSRF_ALLOW_LIST'] = settings.SSRF_ALLOW_LIST.join(',')
    }
    return env
}
...
function proxyEnv({ proxyPort }: { proxyPort: number | null }): Record<string, string> {
    ...
    // "Only HTTP protocol is supported". IB_EGRESS_PROXY_URL is a private signal
    // read by the engine to install http/https globalAgent + undici ProxyAgent.
    return {
        IB_EGRESS_PROXY_URL: `http://127.0.0.1:${proxyPort}`,
    }
}
