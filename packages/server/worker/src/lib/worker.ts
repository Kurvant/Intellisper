...
import os from 'os'
import { ibVersionUtil, systemUsage } from '@intelblocks/server-utils'
import {
    IntellisperError,
    ConsumeJobRequest,
    createRpcClient,
...
import { blockInstaller } from './cache/pieces/piece-installer'
...
const AP_VERSION = ibVersionUtil.getCurrentRelease()
...
export const worker = {
    async start({ apiUrl, socketUrl, workerToken, withHealthServer = false }: WorkerStartParams): Promise<void> {
        ...
        socket.on('connect', async () => {
            ...
            void warmupBlocksOnStartup(apiClient)
        ...
        sock.emit(WebsocketServerEvent.FETCH_WORKER_SETTINGS, request, (response: WorkerSettingsResponse) => {
            ...
            if (!isNil(workerGroupId)) {
                ...
                if (!processSandboxedModes.includes(response.EXECUTION_MODE as ExecutionMode)) {
                    throw new Error(`Worker group "${workerGroupId}" requires IB_EXECUTION_MODE to be one of: ${processSandboxedModes.join(', ')}. Got: ${response.EXECUTION_MODE}`)
                }
...
async function warmupBlocksOnStartup(apiClient: WorkerToApiContract): Promise<void> {
    const { data: blocks, error } = await tryCatch(() => apiClient.getUsedBlocks({}))
    if (error) {
    ...
    logger.info({ count: blocks.length }, 'Starting piece cache warmup')
    const { error: installError } = await tryCatch(() =>
        blockInstaller(logger, apiClient).install({ blocks, includeFilters: false }),
    )
    if (installError) {
        logger.error({ error: installError }, 'Failed to install pieces during startup warmup')
    }
    else {
        void tryCatch(() => apiClient.markBlockAsUsed({ blocks }))
    }
    logger.info({ count: blocks.length }, 'Block cache warmup complete')
}
...
function extractLogs(execError: Error | undefined, result: JobResult | undefined): string | undefined {
    if (execError instanceof IntellisperError) {
