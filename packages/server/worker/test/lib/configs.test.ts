...
const savedContainerType = process.env.IB_CONTAINER_TYPE
const savedFrontendUrl = process.env.IB_FRONTEND_URL

function cleanEnv() {
    delete process.env.IB_CONTAINER_TYPE
    delete process.env.IB_FRONTEND_URL
}

function restoreEnv() {
    if (savedContainerType !== undefined) process.env.IB_CONTAINER_TYPE = savedContainerType
    else delete process.env.IB_CONTAINER_TYPE
    if (savedFrontendUrl !== undefined) process.env.IB_FRONTEND_URL = savedFrontendUrl
    else delete process.env.IB_FRONTEND_URL
}
...
describe('getApiUrl', () => {
    ...
    it('returns localhost when CONTAINER_TYPE is WORKER_AND_APP', () => {
        process.env.IB_CONTAINER_TYPE = 'WORKER_AND_APP'
    ...
    it('returns FRONTEND_URL/api/ when CONTAINER_TYPE is WORKER (with trailing slash)', () => {
        ...
        process.env.IB_FRONTEND_URL = 'https://app.activepieces.com/'
