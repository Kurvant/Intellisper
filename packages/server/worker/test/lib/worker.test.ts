...
function buildExtractPieceJob(): ExecuteExtractBlockMetadataJobData {
    return {
        schemaVersion: 4,
        ...
        platformId: 'plat-1',
        block: {
            blockName: '@intelblocks/block-test',
            blockVersion: '0.1.0',
...
describe('worker integration', () => {
    ...
    it('polls for a job, executes it, and reports completion', async () => {
        ...
        expect(mockGetHandler).toHaveBeenCalledWith(WorkerJobType.EXECUTE_EXTRACT_BLOCK_INFORMATION)
    }, 15_000)
    ...
    it('propagates INTERNAL_ERROR status from fire-and-forget result to completeJob', async () => {
        mockGetHandler.mockReturnValue({
            ...
            execute: vi.fn().mockResolvedValue({
                ...
                logs: 'MODULE_NOT_FOUND: @intelblocks/shared',
            }),
        })
        ...
        expect(completeJobCalls[0].status).toBe(EngineResponseStatus.INTERNAL_ERROR)
        expect(completeJobCalls[0].logs).toBe('MODULE_NOT_FOUND: @intelblocks/shared')
    }, 15_000)
    ...
    describe('resilience to invalid job data', () => {
        it('survives a job with invalid jobData fields and continues processing', async () => {
            ...
            const invalidJob = buildConsumeJobRequest({
                ...
                jobData: { jobType: 'EXECUTE_EXTRACT_BLOCK_INFORMATION', schemaVersion: 4 } as any,
            })
    ...
    describe('health endpoint', () => {
        ...
        beforeEach(async () => {
            ...
            process.env.IB_PORT = String(healthPort)
        })
