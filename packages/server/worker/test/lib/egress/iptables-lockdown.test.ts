...
describe('iptables-lockdown command builder', () => {
    ...
    it('apply commands start with chain creation and end with jump rule', () => {
        ...
        expect(cmds[0]).toEqual(['-N', 'IB_EGRESS_LOCKDOWN'])
        const last = cmds[cmds.length - 1]
        expect(last).toEqual(['-A', 'OUTPUT', '-m', 'owner', '--uid-owner', '60000-60009', '-j', 'IB_EGRESS_LOCKDOWN'])
    })
    ...
    it('apply commands allowlist the proxy port exactly once', () => {
        ...
        expect(matches[0]).toEqual(['-A', 'IB_EGRESS_LOCKDOWN', '-o', 'lo', '-p', 'tcp', '--dport', '4444', '-j', 'ACCEPT'])
    })
    ...
    it('apply commands allowlist the WS RPC port range with a single rule', () => {
        ...
        expect(rangeRules[0]).toEqual(['-A', 'IB_EGRESS_LOCKDOWN', '-o', 'lo', '-p', 'tcp', '--dport', '52000:52999', '-j', 'ACCEPT'])
    })
    ...
    it('apply commands end the chain with a REJECT rule before the jump', () => {
        ...
        expect(cmds[rejectIdx]).toEqual(['-A', 'IB_EGRESS_LOCKDOWN', '-j', 'REJECT', '--reject-with', 'icmp-host-prohibited'])
    })
    ...
    it('remove commands detach from OUTPUT before flushing + deleting the chain', () => {
        ...
        expect(cmds).toEqual([
            ['-D', 'OUTPUT', '-m', 'owner', '--uid-owner', '60000-60009', '-j', 'IB_EGRESS_LOCKDOWN'],
            ['-F', 'IB_EGRESS_LOCKDOWN'],
            ['-X', 'IB_EGRESS_LOCKDOWN'],
        ])
    })
    ...
    it('uid range reflects numBoxes correctly (off-by-one)', () => {
        ...
        expect(jump).toEqual(['-A', 'OUTPUT', '-m', 'owner', '--uid-owner', '60000-60000', '-j', 'IB_EGRESS_LOCKDOWN'])
    })
    ...
    describe('DNS nameserver allow rules', () => {
        it('emits a UDP and a TCP /53 ACCEPT for a single IPv4 nameserver', () => {
            ...
            expect(udp).toEqual(['-A', 'IB_EGRESS_LOCKDOWN', '-d', '10.0.0.2', '-p', 'udp', '--dport', '53', '-j', 'ACCEPT'])
            expect(tcp).toEqual(['-A', 'IB_EGRESS_LOCKDOWN', '-d', '10.0.0.2', '-p', 'tcp', '--dport', '53', '-j', 'ACCEPT'])
        })
        ...
        it('emits UDP+TCP /53 rules for each nameserver in input order', () => {
            ...
            const dnsRules = cmds.filter((c) => c[0] === '-A' && c[1] === 'IB_EGRESS_LOCKDOWN' && c.includes('--dport') && c[c.indexOf('--dport') + 1] === '53')
            expect(dnsRules).toEqual([
                ['-A', 'IB_EGRESS_LOCKDOWN', '-d', '10.0.0.2', '-p', 'udp', '--dport', '53', '-j', 'ACCEPT'],
                ['-A', 'IB_EGRESS_LOCKDOWN', '-d', '10.0.0.2', '-p', 'tcp', '--dport', '53', '-j', 'ACCEPT'],
                ['-A', 'IB_EGRESS_LOCKDOWN', '-d', '169.254.169.253', '-p', 'udp', '--dport', '53', '-j', 'ACCEPT'],
                ['-A', 'IB_EGRESS_LOCKDOWN', '-d', '169.254.169.253', '-p', 'tcp', '--dport', '53', '-j', 'ACCEPT'],
            ])
        })
