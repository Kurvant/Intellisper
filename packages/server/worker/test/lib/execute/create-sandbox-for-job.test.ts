...
import { IbEnvironment, ExecutionMode, NetworkMode } from '@intelblocks/shared'
...
function buildSettings(overrides: Partial<Settings> = {}): Settings {
    const base: Settings = {
        ...
        LOG_LEVEL: 'info',
        LOG_PRETTY: 'false',
        ENVIRONMENT: IbEnvironment.PRODUCTION,
        APP_WEBHOOK_SECRETS: '{}',
...
describe('createSandboxForJob', () => {
    ...
    describe('buildSandboxEnv', () => {
        it('emits all required keys including NODE_PATH (STRICT when proxyPort is set)', () => {
            ...
            expect(env).toMatchObject({
                HOME: '/tmp/',
                IB_EXECUTION_MODE: ExecutionMode.SANDBOX_PROCESS,
                IB_MAX_FLOW_RUN_LOG_SIZE_MB: '25',
                IB_MAX_FILE_SIZE_MB: '50',
                NODE_PATH: '/usr/src/node_modules',
                IB_NETWORK_MODE: NetworkMode.STRICT,
                IB_EGRESS_PROXY_URL: 'http://127.0.0.1:49321',
            })
        })

        it('omits IB_DEV_BLOCKS when DEV_BLOCKS is empty', () => {
            getSettingsMock.mockReturnValue(buildSettings({ DEV_BLOCKS: [] }))
            createSandboxForJob({ log, apiClient, boxId: 1, reusable: false, proxyPort: null })

            const env = createSandboxMock.mock.calls[0][2].env
            expect(env.IB_DEV_BLOCKS).toBeUndefined()
        })

        it('joins DEV_BLOCKS with comma', () => {
            getSettingsMock.mockReturnValue(buildSettings({ DEV_BLOCKS: ['a', 'b', 'c'] }))
            createSandboxForJob({ log, apiClient, boxId: 1, reusable: false, proxyPort: null })

            const env = createSandboxMock.mock.calls[0][2].env
            expect(env.IB_DEV_BLOCKS).toBe('a,b,c')
        })
    ...
    // STRICT iptables + missing IB_EGRESS_PROXY_URL drift that surfaces as
    ...
    describe('sandbox network env follows proxyPort, not live settings', () => {
        it('proxyPort=null + NETWORK_MODE=STRICT in settings → engine sees UNRESTRICTED, no proxy URL', () => {
            ...
            const env = createSandboxMock.mock.calls[0][2].env
            expect(env.IB_NETWORK_MODE).toBe(NetworkMode.UNRESTRICTED)
            expect('IB_EGRESS_PROXY_URL' in env).toBe(false)
        })
        ...
        it('proxyPort=<port> + NETWORK_MODE=UNRESTRICTED in settings → engine sees STRICT, proxy URL set', () => {
            ...
            const env = createSandboxMock.mock.calls[0][2].env
            expect(env.IB_NETWORK_MODE).toBe(NetworkMode.STRICT)
            expect(env.IB_EGRESS_PROXY_URL).toBe('http://127.0.0.1:49322')
        })
