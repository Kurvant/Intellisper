...
import type { ExecuteFlowJobData, FlowVersion } from '@intelblocks/shared'
...
vi.mock('../../../../src/lib/execute/utils/flow-helpers', () => ({
    provisionFlowBlocks: vi.fn().mockResolvedValue(true),
}))
    ...
    describe('RESUME validation', () => {
        it('still throws when logsFileId is missing for RESUME', async () => {
            ...
            try {
                ...
                expect(e).toBeInstanceOf(IntellisperError)
                expect((e as IntellisperError).error.code).toBe(ErrorCode.RESUME_LOGS_FILE_MISSING)
            }
