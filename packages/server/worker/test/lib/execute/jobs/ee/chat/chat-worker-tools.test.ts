import { ToolProgressEvent } from '@intelblocks/shared'
...
describe('chatWorkerTools', () => {
    ...
    describe('ib_execute_action batch mode', () => {
        it('calls executeTool for each item and emits progress events', async () => {
            ...
            const result = await tools.ib_execute_action.execute({
                blockName: 'slack',
            ...
            expect(executeTool).toHaveBeenCalledWith('ib_execute_action', expect.objectContaining({
                blockName: 'slack',
                actionName: 'send_message',
        ...
        it('uses default label when description is not provided', async () => {
            ...
            const tools = chatWorkerTools.createCrossProjectTools({ executeTool, eventEmitter, waitForApproval: vi.fn().mockResolvedValue({ approved: true }) })
            await tools.ib_execute_action.execute({
                blockName: 'http',
