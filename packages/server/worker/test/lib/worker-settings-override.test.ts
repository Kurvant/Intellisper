...
import type { WorkerToApiContract, WorkerSettingsResponse } from '@intelblocks/shared'
...
function buildMinimalHandlers(): WorkerToApiContract {
    return {
        ...
        getBlock: vi.fn(),
        getBlockArchive: vi.fn(),
        extendLock: vi.fn(),
        getUsedBlocks: vi.fn().mockResolvedValue([]),
        markBlockAsUsed: vi.fn(),
        disableFlow: vi.fn(),
    }
}
...
describe('worker settings override', () => {
    ...
    beforeEach(async () => {
        ...
        process.env.IB_FRONTEND_URL = `http://127.0.0.1:${port}`
        process.env.IB_CONTAINER_TYPE = 'WORKER'
        delete process.env.IB_EXECUTION_MODE
        delete process.env.IB_WORKER_GROUP_ID
    ...
    afterEach(async () => {
        ...
        delete process.env.IB_EXECUTION_MODE
        delete process.env.IB_WORKER_GROUP_ID
        delete process.env.IB_REUSE_SANDBOX
        await new Promise<void>((resolve) => {
    ...
    it('local IB_EXECUTION_MODE overrides server-provided mode', async () => {
        process.env.IB_EXECUTION_MODE = ExecutionMode.SANDBOX_CODE_ONLY
        const serverSettings = buildWorkerSettingsResponse({ EXECUTION_MODE: ExecutionMode.SANDBOX_CODE_AND_PROCESS })
        await connectAndWaitForSettings(serverSettings)
    ...
    it('worker group + SANDBOX_PROCESS passes validation', async () => {
        process.env.IB_WORKER_GROUP_ID = 'group-1'
        process.env.IB_EXECUTION_MODE = ExecutionMode.SANDBOX_PROCESS
        process.env.IB_REUSE_SANDBOX = 'false'
    ...
    it('worker group + SANDBOX_CODE_AND_PROCESS passes validation', async () => {
        process.env.IB_WORKER_GROUP_ID = 'group-1'
        process.env.IB_EXECUTION_MODE = ExecutionMode.SANDBOX_CODE_AND_PROCESS
        process.env.IB_REUSE_SANDBOX = 'false'
        const serverSettings = buildWorkerSettingsResponse()
        await connectAndWaitForSettings(serverSettings)
    ...
    it('worker group + SANDBOX_CODE_ONLY throws error', async () => {
        ...
        expect(err.message).toMatch(/Worker group "group-1" requires IB_EXECUTION_MODE/)
    }, 10_000)

    it('worker group + UNSANDBOXED throws error', async () => {
        process.env.IB_WORKER_GROUP_ID = 'group-1'
        process.env.IB_EXECUTION_MODE = ExecutionMode.UNSANDBOXED
        const serverSettings = buildWorkerSettingsResponse()
