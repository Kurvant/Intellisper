...
import { IntellisperError, EngineResponseStatus, ErrorCode, WorkerContract } from '@intelblocks/shared'
...
function createTestProcessMaker() {
    ...
    const maker: SandboxProcessMaker = {
        create: vi.fn(async (params) => {
            const port = params.env.IB_SANDBOX_WS_PORT
            const token = params.env.IB_SANDBOX_WS_TOKEN ?? null
...
describe('createSandbox', () => {
    ...
    describe('start', () => {
            ...
            expect(testPM.maker.create).toHaveBeenCalledWith(
                expect.objectContaining({
                    ...
                    mounts: expect.arrayContaining([
                        expect.objectContaining({
                            sandboxPath: '/root/custom_blocks',
                        }),
                    ]),
                    env: expect.objectContaining({
                        MY_VAR: 'value',
                        IB_SANDBOX_WS_PORT: expect.any(String),
                        IB_CUSTOM_BLOCKS_PATHS: '/root/custom_blocks',
                    }),
                    resourceLimits: {
                        memoryLimitMb: 256,
                        cpuMsPerSec: 1000,
        ...
        it('does not add custom piece mount when platformId is empty', async () => {
            ...
            const customPieceMount = createCall.mounts.find((m: { sandboxPath: string }) => m.sandboxPath === '/root/custom_blocks')
            expect(customPieceMount).toBeUndefined()
            expect(createCall.env.IB_CUSTOM_BLOCKS_PATHS).toBeUndefined()
        })
        ...
        it('resolves custom_blocks hostPath to cache/custom_blocks/<platformId>', async () => {
            const log = createMockLogger()
            const workerHandlers = createMockWorkerHandlers()
            testPM = createTestProcessMaker()
            ...
            const createCall = (testPM.maker.create as ReturnType<typeof vi.fn>).mock.calls[0][0]
            const customPieceMount = createCall.mounts.find((m: SandboxMount) => m.sandboxPath === '/root/custom_blocks')
            expect(customPieceMount).toEqual({
                hostPath: '/tmp/test-cache/custom_blocks/plat-xyz',
                sandboxPath: '/root/custom_blocks',
                optional: true,
            })
        })
        ...
        it.each([
            ...
            expect((caughtErr as IntellisperError).error.code).toBe(ErrorCode.VALIDATION)
        ...
        it('composes mounts in order: baseMounts, codeMount, callerMounts, customPieceMounts', async () => {
            ...
            expect(createCall.mounts).toEqual([
                ...
                { hostPath: '/tmp/test-cache/custom_blocks/plat-1', sandboxPath: '/root/custom_blocks', optional: true },
            ])
        })
        ...
        it('does not inject IB_CUSTOM_BLOCKS_PATHS when platformId is undefined', async () => {
            const log = createMockLogger()
            const workerHandlers = createMockWorkerHandlers()
            testPM = createTestProcessMaker()
        ...
        it('passes a per-start IB_SANDBOX_WS_TOKEN to the child', async () => {
            const log = createMockLogger()
            const workerHandlers = createMockWorkerHandlers()
            testPM = createTestProcessMaker()
            ...
            const createCall = (testPM.maker.create as ReturnType<typeof vi.fn>).mock.calls[0][0]
            expect(createCall.env.IB_SANDBOX_WS_TOKEN).toMatch(/^[a-f0-9]{64}$/)
        })

        it('rotates IB_SANDBOX_WS_TOKEN between successive start() calls on a reusable sandbox', async () => {
            const log = createMockLogger()
            const workerHandlers = createMockWorkerHandlers()
            ...
            await sandbox.start(startOptions)
            const firstToken = (testPM.maker.create as ReturnType<typeof vi.fn>).mock.calls[0][0].env.IB_SANDBOX_WS_TOKEN
            await sandbox.shutdown()
            await sandbox.start(startOptions)
            const secondToken = (testPM.maker.create as ReturnType<typeof vi.fn>).mock.calls[1][0].env.IB_SANDBOX_WS_TOKEN
        ...
        it('rejects an unauthenticated Socket.IO connection', async () => {
            ...
            await sandbox.start(startOptions)
            const port = (testPM.maker.create as ReturnType<typeof vi.fn>).mock.calls[0][0].env.IB_SANDBOX_WS_PORT
        ...
        it('disconnects a second authenticated connection while keeping the first active', async () => {
            ...
            await sandbox.start(startOptions)
            const createCall = (testPM.maker.create as ReturnType<typeof vi.fn>).mock.calls[0][0]
            const port = createCall.env.IB_SANDBOX_WS_PORT
            const token = createCall.env.IB_SANDBOX_WS_TOKEN
    ...
    describe('execute', () => {
        ...
        it('rejects with SANDBOX_EXECUTION_TIMEOUT on real timeout', async () => {
            ...
            try {
                ...
                expect((err as IntellisperError).error.code).toBe(ErrorCode.SANDBOX_EXECUTION_TIMEOUT)
            }
        })
        ...
        it('rejects with SANDBOX_MEMORY_ISSUE on exit code 134 / SIGABRT', async () => {
            ...
            try {
                ...
                expect((err as IntellisperError).error.code).toBe(ErrorCode.SANDBOX_MEMORY_ISSUE)
            }
        })
        ...
        it('rejects with SANDBOX_LOG_SIZE_EXCEEDED', async () => {
            ...
            try {
                ...
                expect((err as IntellisperError).error.code).toBe(ErrorCode.SANDBOX_LOG_SIZE_EXCEEDED)
            }
        })
        ...
        it('rejects with SANDBOX_INTERNAL_ERROR for other exit codes', async () => {
            ...
            try {
                ...
                expect((err as IntellisperError).error.code).toBe(ErrorCode.SANDBOX_INTERNAL_ERROR)
            }
        })
