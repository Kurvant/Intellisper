...
const BASE_ENV: Record<string, string> = {
    ...
    IB_EXECUTION_MODE: 'SANDBOX_PROCESS',
    IB_SANDBOX_WS_PORT: '12345',
    IB_SANDBOX_WS_TOKEN: 'test-token-aaaaaaaaaaaaaaaaaaaaaaaa',
}
...
describe('isolateProcess', () => {
    ...
    describe('argv', () => {
        ...
        it('appends :maybe suffix on optional mounts', async () => {
            const mounts: SandboxMount[] = [
                { hostPath: '/host/custom', sandboxPath: '/root/custom_blocks', optional: true },
            ]
            await callCreate({ mounts })

            const args: string[] = spawnMock.mock.calls[0][1]
            expect(args).toContain('--dir=/root/custom_blocks=/host/custom:maybe')
        })
    ...
    describe('env', () => {
        it('injects IB_BASE_CODE_DIRECTORY=/root/codes and SANDBOX_ID=<sandboxId>', async () => {
            await callCreate({ sandboxId: 'sb-xyz' })
            const args: string[] = spawnMock.mock.calls[0][1]
            expect(args).toContain('--env=IB_BASE_CODE_DIRECTORY=/root/codes')
        ...
        it('forwards every caller-provided env entry as --env=K=V', async () => {
            ...
            expect(args).toContain('--env=IB_EXECUTION_MODE=SANDBOX_PROCESS')
        })

        it('does not allow caller-supplied env to override injected IB_BASE_CODE_DIRECTORY or SANDBOX_ID', async () => {
            await callCreate({
                sandboxId: 'sb-xyz',
                env: {
                    ...BASE_ENV,
                    IB_BASE_CODE_DIRECTORY: '/etc',
                    SANDBOX_ID: 'spoofed',
                },
            })
            const args: string[] = spawnMock.mock.calls[0][1]
            const baseCodeDirArgs = args.filter((a) => a.startsWith('--env=IB_BASE_CODE_DIRECTORY='))
            const sandboxIdArgs = args.filter((a) => a.startsWith('--env=SANDBOX_ID='))
            expect(baseCodeDirArgs).toEqual(['--env=IB_BASE_CODE_DIRECTORY=/root/codes'])
