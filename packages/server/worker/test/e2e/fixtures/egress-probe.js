...
function httpGetViaProxy(action) {
    return new Promise((resolve) => {
        const proxyUrl = new URL(process.env.IB_EGRESS_PROXY_URL || '')
...
//  - the IB_EGRESS_PROXY_URL env propagation chain (engine reads it; missing → fetch
...
async function fetchViaUndici(action) {
    ...
    try {
        ...
        const proxyUrl = process.env.IB_EGRESS_PROXY_URL
