...
describe.skipIf(PRIVILEGE_SKIP)('sandbox real third-party connectivity (SANDBOX_PROCESS + STRICT)', () => {
    ...
    it.skipIf(PRIVILEGE_SKIP)('Group F — bug repro: STRICT iptables WITHOUT IB_EGRESS_PROXY_URL → fetch fails with EHOSTUNREACH on every host', async (ctx) => {
        if (internetSkip) ctx.skip()
        ...
        // IB_EGRESS_PROXY_URL didn't reach the sandbox env. fetch falls back to direct
...
async function runProbeInSandbox({ commonDir, plan, proxyPort, omitProxyUrl }: {
    ...
    const baseEnv: Record<string, string> = {
        ...
        IB_SANDBOX_WS_PORT: '0',
        IB_BASE_CODE_DIRECTORY: '/root/codes',
        SANDBOX_ID: 'e2e-real-3p',
        IB_NETWORK_MODE: 'STRICT',
        AP_UNDICI_REQUIRE_PATH: '/root/common/undici',
        AP_PROBE_PLAN: JSON.stringify(plan),
    }
    if (!omitProxyUrl) baseEnv.IB_EGRESS_PROXY_URL = proxyUrl
