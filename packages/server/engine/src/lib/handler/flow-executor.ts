...
import { EngineGenericError, ExecutionType, FlowAction, FlowActionType, FlowRunStatus, FlowTrigger, GenericStepOutput, isNil, StepOutputStatus } from '@intelblocks/shared'
...
function getExecuteFunction(): Record<FlowActionType, BaseExecutor<FlowAction>> {
    return {
        ...
        [FlowActionType.BLOCK]: pieceExecutor,
...
async function runContinueOnFailureBranchIfNeeded({ action, executionState, constants }: {
    ...
    if (action.type !== FlowActionType.CODE && action.type !== FlowActionType.BLOCK) {
        return executionState
    }
