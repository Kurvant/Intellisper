import { ActionContext, backwardCompatabilityContextUtils, ConstructToolParams, CreateWaitpointHook, CreateWaitpointParams, CreateWaitpointResult, InputPropertyMap, BlockAuthProperty, BlockPropertyMap, RespondHook, RespondHookParams, StaticPropsValue, StopHook, StopHookParams, TagsManager, WaitForWaitpointHook } from '@intelblocks/blocks-framework'
import { AUTHENTICATION_PROPERTY_NAME, EngineGenericError, ExecutionType, FlowActionType, FlowRunStatus, GenericStepOutput, isNil, PausedFlowTimeoutError, BlockAction, RespondResponse, StepOutputStatus } from '@intelblocks/shared'
...
const AP_PAUSED_FLOW_TIMEOUT_DAYS = Number(process.env.IB_PAUSED_FLOW_TIMEOUT_DAYS)

export const pieceExecutor: BaseExecutor<BlockAction> = {
    async handle({
        action,
        executionState,
        constants,
...
const executeAction: ActionHandler<BlockAction> = async ({ action, executionState, constants }) => {
    ...
    const { data: executionStateResult, error: executionStateError } = await utils.tryCatchAndThrowOnEngineError((async () => {
        ...
        const { resolvedInput, censoredInput } = await constants.getPropsResolver(piece.getContextInfo?.().version).resolve<StaticPropsValue<BlockPropertyMap>>({
        ...
        const context: ActionContext<BlockAuthProperty, InputPropertyMap> = {
        ...
        const isSamePiece = constants.triggerBlockName === action.settings.blockName
