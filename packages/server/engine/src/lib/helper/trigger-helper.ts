import { BlockPropertyMap, StaticPropsValue, TriggerStrategy } from '@intelblocks/blocks-framework'
import { assertEqual, AUTHENTICATION_PROPERTY_NAME, EngineGenericError, EventPayload, ExecuteTriggerResponse, FlowTrigger, InvalidCronExpressionError, isNil, BlockTrigger, PropertySettings, ScheduleOptions, TriggerHookType, TriggerSourceScheduleType } from '@intelblocks/shared'
...
export const triggerHelper = {
    async executeOnStart(trigger: FlowTrigger, constants: EngineConstants, payload: unknown) {
        const { blockName, blockVersion, triggerName, input, propertySettings } = (trigger as BlockTrigger).settings
    ...
    async executeTrigger({ params, constants }: ExecuteTriggerParams): Promise<ExecuteTriggerResponse<TriggerHookType>> {
        const { blockName, blockVersion, triggerName, input, propertySettings } = (params.flowVersion.trigger as BlockTrigger).settings
                ...
                if (pieceTrigger.type === TriggerStrategy.APP_WEBHOOK) {
                    ...
                    const { data: verified, error: verifiedError } = await utils.tryCatchAndThrowOnEngineError(async () => {
                        if (!params.appWebhookUrl) {
                            throw new EngineGenericError('AppWebhookUrlNotAvailableError', `App webhook url is not available for piece name ${blockName}`)
                        }
                        if (!params.webhookSecret) {
                            throw new EngineGenericError('WebhookSecretNotAvailableError', `Webhook secret is not available for piece name ${blockName}`)
                        }
...
async function prepareTriggerExecution({ blockName, blockVersion, triggerName, input, propertySettings, projectId, apiUrl, engineToken, devBlocks, stepNames }: PrepareTriggerExecutionParams) {
