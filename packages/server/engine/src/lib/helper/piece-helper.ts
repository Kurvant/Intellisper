...
import {
    ...
    blockTranslation,
    PropertyType,
    StaticPropsValue } from '@intelblocks/blocks-framework'
import {
    AppConnectionType,
    AppConnectionValue,
    EngineGenericError,
...
export const pieceHelper = {
    async executeProps( operation: ExecutePropsParams): Promise<ExecutePropsResult<PropertyType.DROPDOWN | PropertyType.MULTI_SELECT_DROPDOWN | PropertyType.DYNAMIC>> {
        ...
        const { property, piece } = await pieceLoader.getPropOrThrow({ blockName: operation.blockName, blockVersion: operation.blockVersion, actionOrTriggerName: operation.actionOrTriggerName, propertyName: operation.propertyName, devBlocks: EngineConstants.DEV_BLOCKS })
    ...
    async executeValidateAuth(
        { params, devBlocks }: { params: ExecuteValidateAuthOperation, devBlocks: string[] },
    ): Promise<ExecuteValidateAuthResponse> {
        const { block: piecePackage } = params

        const piece = await pieceLoader.loadPieceOrThrow({ blockName: piecePackage.blockName, blockVersion: piecePackage.blockVersion, devBlocks })
        const server = {
    ...
    async extractPieceMetadata({ devBlocks, params }: { devBlocks: string[], params: ExecuteExtractBlockMetadata }): Promise<BlockMetadata> {
        const { blockName, blockVersion } = params
        const piece = await pieceLoader.loadPieceOrThrow({ blockName, blockVersion, devBlocks })
        const pieceAlias = pieceLoader.getPackageAlias({ blockName, blockVersion, devBlocks })
        const pieceIndexPath = await pieceLoader.getPiecePath({ packageName: pieceAlias, devBlocks })
        const pieceDistRoot = path.dirname(path.dirname(pieceIndexPath))
        const i18n = await blockTranslation.initializeI18n(pieceDistRoot)
...
type ExecutePropsParams = Omit<ExecutePropsOptions, 'block'> & { blockName: string, blockVersion: string }
...
type ValidateAuthParams = {
    ...
    pieceAuth: BlockAuthProperty | BlockAuthProperty[] | undefined
}