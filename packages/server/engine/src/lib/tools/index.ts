import { Action, DropdownOption, ExecutePropsResult, BlockProperty, PropertyType } from '@intelblocks/blocks-framework'
import { AgentBlockTool, ExecuteToolOperation, ExecuteToolResponse, ExecutionToolStatus, FieldControlMode, FlowActionType, isNil, BlockAction, PropertyExecutionType, StepOutputStatus } from '@intelblocks/shared'
            ...
            return {
                ...
                execute: async ({ instruction }: { instruction: string }) =>
                    execute({
                        ...
                        blockVersion: tool.blockMetadata.blockVersion,
                        actionName: tool.blockMetadata.actionName,
                        predefinedInput: tool.blockMetadata.predefinedInput,
                        model,
                    }),
            }
        }))
...
async function execute(operation: ExecuteToolOperationWithModel): Promise<ExecuteToolResponse> {
    try {
        const { pieceAction } = await pieceLoader.getPieceAndActionOrThrow({
            ...
            blockVersion: operation.blockVersion,
...
async function propertyToSchema(propertyName: string, property: BlockProperty, operation: ExecuteToolOperation, resolvedInput: Record<string, unknown>): Promise<z.ZodTypeAny> {
...
async function buildObjectSchemaFromProperties(properties: Record<string, BlockProperty>, operation: ExecuteToolOperation, resolvedInput: Record<string, unknown>): Promise<z.ZodTypeAny> {
...
async function buildPropertyDetail(propertyName: string, property: BlockProperty, operation: ExecuteToolOperation, input: Record<string, unknown>): Promise<PropertyDetail | null> {
...
async function loadOptions(propertyName: string, property: BlockProperty, operation: ExecuteToolOperation, input: Record<string, unknown>): Promise<DropdownOption<unknown>[]> {
