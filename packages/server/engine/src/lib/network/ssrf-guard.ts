import { NetworkMode, ssrfIpClassifier, tryCatchSync } from '@intelblocks/shared'
...
function isGuardEnabled(options: InstallOptions): boolean {
    return options.enabled ?? process.env['IB_NETWORK_MODE'] === NetworkMode.STRICT
}

function buildGuardPolicy(options: InstallOptions): GuardPolicy {
    return {
        allowList: options.allowList ?? splitCsv(process.env['IB_SSRF_ALLOW_LIST']),
...
function readSandboxRpcPortFromEnv(): number[] {
    const rpcPort = parseInt(process.env['IB_SANDBOX_WS_PORT'] ?? '', 10)
