...
export const workerSocket = {
    init: (sandboxId: string): void => {
        const wsUrl = `ws://127.0.0.1:${process.env.IB_SANDBOX_WS_PORT ?? '12345'}`
...
function buildSocketOptions(sandboxId: string): Partial<ManagerOptions & SocketOptions> {
    const base: Partial<ManagerOptions & SocketOptions> = {
        ...
        auth: {
            sandboxId,
            connectionToken: process.env.IB_SANDBOX_WS_TOKEN,
        },
        autoConnect: false,
    ...
    if (process.env['IB_NETWORK_MODE'] === NetworkMode.STRICT) {
