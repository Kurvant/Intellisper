...
export async function execute(operationType: EngineOperationType, operation: EngineOperation): Promise<EngineResponse<unknown>> {
    const result = await tryCatch(async () => {
        switch (operationType) {
            case EngineOperationType.EXTRACT_BLOCK_METADATA: {
                return pieceMetadataOperation.extract(operation as ExecuteExtractBlockMetadataOperation)
            }
    ...
    if (result.error) {
        ...
        return {
            ...
            error: JSON.stringify(formatBlockError(result.error, { raw: inspect(result.error) })),
        }
    }
    return result.data
}