import { getAuthPropertyForValue, InputPropertyMap, BlockAuthProperty, BlockProperty, BlockPropertyMap, PropertyType, StaticPropsValue } from '@intelblocks/blocks-framework'
import { AppConnectionValue, AUTHENTICATION_PROPERTY_NAME, isNil, isObject, PropertySettings } from '@intelblocks/shared'
...
export const propsProcessor = {
    ...
    ): Promise<{ processedInput: StaticPropsValue<BlockPropertyMap>, errors: PropsValidationError }> => {
...
const validateProperty = (property: BlockProperty, value: unknown, originalValue: unknown): string[] => {
...
function getAuthPropsToProcess(authValue: AppConnectionValue, auth: BlockAuthProperty | BlockAuthProperty[] | undefined): | null {
    if (isNil(auth)) {
        return null
    }
