import { FlowActionType, FlowRunStatus, GenericStepOutput, StepOutputStatus, StepRunResponse, StreamStepProgress, UpdateRunProgressRequest, UploadRunLogsRequest } from '@intelblocks/shared'
...
describe('flow-run-progress-reporter slicing in single-step test mode', () => {
    ...
    it('streams the original payload via updateStepProgress even when upsertStep slices the output', async () => {
        ...
        const seedStep = GenericStepOutput.create({
            ...
            input: {},
            output: undefined,
        }) as GenericStepOutput<FlowActionType.BLOCK, unknown>
