import { IbFile, LATEST_CONTEXT_VERSION, BlockAuth, Property } from '@intelblocks/blocks-framework'
import { FlowActionType, FlowTriggerType, formulaEvaluator, GenericStepOutput, PropertyExecutionType, PropertySettings, StepOutputStatus } from '@intelblocks/shared'
...
describe('Props resolver', () => {
    ...
    it('should return base64 from base64 with mime only', async () => {
        ...
        const { processedInput, errors } = await propsProcessor.applyProcessorsAndValidators(input, props, BlockAuth.None(), false, {})
        expect(processedInput).toEqual({
            base64: null,
            base64WithMime: new IbFile('unknown.png', Buffer.from('iVBORw0KGgoAAAANSUhEUgAAAiAAAAC4CAYAAADaI1cbAAA0h0lEQVR4AezdA5AlPx7A8Zxt27Z9r5PB2SidWTqbr26S9Hr/tm3btu3723eDJD3r15ec17vzXr+Z', 'base64'), 'png'),
        })
    ...
    it('should not error if auth configured, but no auth provided in input', async () => {
        ...
        const { processedInput, errors } = await propsProcessor.applyProcessorsAndValidators(input, props, BlockAuth.CustomAuth({
            required: true,
            props: {},
        }), false, {})
    ...
    it('should flatten arrays inside DYNAMIC properties', async () => {
        ...
        const { processedInput, errors } = await propsProcessor.applyProcessorsAndValidators(input, props, BlockAuth.None(), false, propertySettings)
