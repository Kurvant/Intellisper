...
import { NetworkMode } from '@intelblocks/shared'
...
describe('workerSocket — STRICT-mode handshake', () => {
    const originalNetworkMode = process.env['IB_NETWORK_MODE']
    const originalWsPort = process.env['IB_SANDBOX_WS_PORT']
    const originalWsToken = process.env['IB_SANDBOX_WS_TOKEN']
    ...
    beforeEach(async () => {
        ...
        process.env['IB_SANDBOX_WS_TOKEN'] = 'test-token-aaaaaaaaaaaaaaaaaaaaaaaa'
    })

    afterEach(async () => {
        workerSocket.disconnect()
        http.globalAgent = originalHttpAgent
        if (originalNetworkMode === undefined) delete process.env['IB_NETWORK_MODE']
        else process.env['IB_NETWORK_MODE'] = originalNetworkMode
        if (originalWsPort === undefined) delete process.env['IB_SANDBOX_WS_PORT']
        else process.env['IB_SANDBOX_WS_PORT'] = originalWsPort
        if (originalWsToken === undefined) delete process.env['IB_SANDBOX_WS_TOKEN']
        else process.env['IB_SANDBOX_WS_TOKEN'] = originalWsToken
        await io.close()
    ...
    it('connects to the worker RPC server when ssrf-guard rebinds http.globalAgent to HttpProxyAgent', async () => {
        ...
        process.env['IB_NETWORK_MODE'] = NetworkMode.STRICT
        process.env['IB_SANDBOX_WS_PORT'] = String(wsPort)
    ...
    it('does not pin an agent outside STRICT mode', async () => {
        delete process.env['IB_NETWORK_MODE']
        process.env['IB_SANDBOX_WS_PORT'] = String(wsPort)
