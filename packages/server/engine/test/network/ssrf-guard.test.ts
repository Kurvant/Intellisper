...
import { SSRFBlockedError } from '@intelblocks/shared'
...
describe('ssrf-guard', () => {
    ...
    describe('egress proxy wiring', () => {
        const originalEgressProxy = process.env['IB_EGRESS_PROXY_URL']
        ...
        afterEach(() => {
            ssrfGuard.uninstall()
            if (originalEgressProxy === undefined) delete process.env['IB_EGRESS_PROXY_URL']
            else process.env['IB_EGRESS_PROXY_URL'] = originalEgressProxy
            setGlobalDispatcher(originalDispatcher)
            http.globalAgent = originalHttpAgent
            https.globalAgent = originalHttpsAgent
        })

        it('installs http/https globalAgent and undici ProxyAgent when IB_EGRESS_PROXY_URL is set', () => {
            process.env['IB_EGRESS_PROXY_URL'] = 'http://127.0.0.1:4444'
            ssrfGuard.install({ enabled: true, allowList: [] })
        ...
        it('leaves agents and dispatcher untouched when no egress proxy env is set', () => {
            delete process.env['IB_EGRESS_PROXY_URL']
