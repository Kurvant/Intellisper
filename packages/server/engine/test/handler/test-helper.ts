import { ActionErrorHandlingOptions, BeginExecuteFlowOperation, BranchCondition, BranchExecutionType, CodeAction, ExecutionType, FlowAction, FlowActionType, FlowVersionState, LoopOnItemsAction, BlockAction, StreamStepProgress, PropertyExecutionType, RouterExecutionType, RunEnvironment } from '@intelblocks/shared'
    ...
    return new EngineConstants(
        {
            ...
            triggerBlockName: params?.triggerBlockName ?? 'mcp-trigger-piece-name',
...
export function buildBlockAction({ name, input, skip, blockName, actionName, nextAction, errorHandlingOptions }: { errorHandlingOptions?: ActionErrorHandlingOptions, name: string, input: Record<string, unknown>, skip?: boolean, blockName: string, actionName: string, nextAction?: FlowAction }): BlockAction {
    return {
        name,
        displayName: 'Your Action Name',
