...
Open-source AI-first workflow automation platform. Self-hosted or cloud. 400+ blocks. MCP support.

## Architecture (Non-Obvious Rules)

- **Multi-tenant**: Platform → Projects → Users. ALL queries MUST filter by `projectId` or `platformId`.
- **Editions**: CE (`ce`), EE (`ee`), Cloud (`cloud`) via `IB_EDITION`. EE extends CE via `hooksFactory` — **never import `src/app/ee/` in CE code**.
...
- **Go-style error handling** — Use `tryCatch` / `tryCatchSync` from `@intelblocks/shared`
- **Zod error messages must be i18n keys** — Every `.min()`, `.refine()`, `.superRefine()`, etc. that surfaces a user-facing message must pass a string that exists as a key in `packages/web/public/locales/en/translation.json`. For common messages (e.g. required fields) use the `formErrors` constant from `@intelblocks/shared`. Add a new translation key if none fits; never use raw English sentences that are not in the translation file.
- **`@intelblocks/shared` version bump** — Any change to `packages/shared` must be accompanied by a version bump in `packages/shared/package.json`: bump the **patch** version for non-breaking additions or fixes, bump the **minor** version for new exports or behaviour changes after you check if it has already been bumped in the current branch or not
...
- **Safe outbound HTTP (SSRF)** — For any outbound HTTP in `packages/server/{api,worker,utils}`, use `safeHttp.axios` / `safeHttp.createAxios({ ... })` from `@intelblocks/server-utils`. Never use raw `fetch` or `axios.create` for URLs that come from user input, admin config, OAuth endpoints, or third-party integrations — they bypass the SSRF filter (private/loopback/metadata IPs). See `.claude/rules/safe-http.md`.
...
- **Do NOT add** `showErrorDialog` to minor/auxiliary queries (feature flags, block metadata, single-item fetches, filter options, user details). These should fail silently.
...
## Key Utilities (`@intelblocks/shared`)

`apId()`, `tryCatch()`, `tryCatchSync()`, `isNil()`, `spreadIfDefined()`, `spreadIfNotUndefined()`, `IntellisperError({ code, params })`, `SeekPage<T>`, `formErrors`, `BaseModelSchema`, `chunk()`, `partition()`, `unique()`, `omit()`, `sanitizeObjectForPostgresql()`
...
- If the PR includes any contributions to blocks (integrations under `packages/blocks`), also add the appropriate blocks label (in addition to the primary label above):
  - **`area/third-party-blocks`** — for third-party integrations (most blocks under `packages/blocks/community/`)
  - **`area/core-blocks`** — for core blocks (under `packages/blocks/core/`)
...
- **All customer-facing UI must be white-labeled.** Sign-in/signup pages, email templates, logos, and any user-visible branding must use the platform's configured appearance (name, colors, logos) — never hardcode "Intellisper" in user-facing surfaces.
- **Test across all edition paths.** Every customer-facing feature must be verified on:
  - **Community Edition** (self-hosted, `IB_EDITION=ce`) — no custom branding, open-source plan
  - **Enterprise Edition** (self-hosted, `IB_EDITION=ee`) — custom branding behind `customAppearanceEnabled` flag
  - **Cloud Freemium** (`IB_EDITION=cloud`, standard plan) — always applies platform branding
  - **Cloud Self-Serve Paid** (`IB_EDITION=cloud`, upgraded plan) — same as freemium with higher limits
  - **Cloud Enterprise** (`IB_EDITION=cloud`, enterprise plan) — full feature set
