# Intellisper Design System

A design system for **Intellisper** — an open-source AI automation platform ("an open source replacement for Zapier"). The product lets both technical and non-technical users build automated workflows with 280+ integrations ("blocks") and exposes every block as an MCP server for use with Claude, Cursor, Windsurf, etc.

This folder contains the brand/visual foundations, CSS tokens, fonts, icon conventions, HTML preview cards for the Design System tab, and a UI kit that recreates core Intellisper product surfaces.
...
| Intellisper codebase | `github.com/yazeed-prog/intellisper` (`packages/web` is the React UI) |
...
Intellisper uses **Shadcn/Radix UI** primitives on top of Tailwind, with **Lucide** as its icon library (confirmed in `packages/web/components.json` → `"iconLibrary": "lucide"`). Shadcn base color is `"neutral"`.
...
- `assets/` — `logo.svg` (brand mark), block-tile SVGs
- `preview/` — ~700px-wide HTML cards that populate the Design System tab
- `ui_kits/web/` — UI kit for the Intellisper web app (builder, dashboard, sidebar, forms showcase)
...
**Product**: Intellisper is an all-in-one AI automation platform. The core surface is a visual **flow builder** (React + XYFlow) where users assemble triggers + actions from 280+ open-source **blocks** into runnable flows. Every block doubles as an MCP server, so LLM agents can call them directly.

**Audience**: mixed — "developers set up the tools, and anyone in the organization can use the no-code builder" (from README). Non-technical users live in the builder; developers contribute new blocks as typed npm packages.
...
Intellisper copy is **functional, direct, and product-led**. It talks about workflows, blocks, and runs in concrete terms — no marketing puffery inside the app.

- **Voice**: second-person ("**you** can build", "**your** flows"). Feature names and verbs lead; adjectives are rare.
- **Casing**: **Sentence case** for every UI string — headings, buttons, menu items, page titles. Proper nouns are the feature itself: "Blocks", "Flows", "Runs", "MCP", "Agents", "Connections".
...
- **Microcopy examples (from repo strings & feature names)**: "Create a Block", "Deploy", "Hot reloading for local block development", "Chat Interface", "Form Interface", "Ask AI in Code Block", "Human in the Loop".
...
- **Block tiles**: small rounded-square icons (48×48) with an 8% tinted background and the block's own logo. Code block uses amber `#E5AE43`, etc. Each block owns its colour.
...
- **Block icons**: each integration has its own SVG (Google, OpenAI, Slack, …). These live at `packages/web/src/assets/img/block/` and as npm-published per-block packages. **Copy the real SVG** — do not redraw.
...
Intellisper is a Tailwind v4 codebase. When generating production-style code off this system:
...
- The **Pro-Blocks** Figma pages (Landing, Application, etc.) are Shadcn's stock templates and do NOT reflect the real Intellisper marketing site (which isn't in the repo). We use them as secondary reference for Shadcn patterns only.
...
- **Sentient** (uploaded) is included as a display option but **is not used in shipping Intellisper UI**. Treat as optional branding exploration only.
