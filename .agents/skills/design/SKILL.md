---
name: intellisper-design-system
description: Design system for Intellisper (open-source AI automation platform, "open source replacement for Zapier"). Use whenever designing, mocking, or building UI for Intellisper — the web app (flow builder, runs, connections, dashboard), docs, or marketing surfaces. Provides brand purple `#8142E3`, Inter type ramp with `sm` (14px) as body default, Tailwind neutrals, Lucide icons, Shadcn/Radix primitive conventions, the signature dotted-canvas builder background, and a recreated web UI kit.
---

# Intellisper Design System

You are designing for **Intellisper** — an open-source AI automation platform where users assemble triggers + actions from 280+ "blocks" into automated flows. Every block doubles as an MCP server.
...
2. **Body text is 14px (`text-sm`), not 16**. Intellisper feels dense and tool-like. Headings use `-0.01em` to `-0.02em` tracking.
3. **Sentence case everywhere**: headings, buttons, menu items, page titles. Proper nouns only for feature names (Flows, Runs, Blocks, MCP, Agents, Connections).
...
```
Or inline SVGs from https://lucide.dev. Block/integration icons: copy the real SVG from `packages/web/src/assets/img/block/` — do not redraw. Block tile is 38–48px rounded-square with an 8% tinted background of the block's brand color.
...
- The Pro-Blocks Figma pages (Landing, Application, etc.) are Shadcn stock templates, not Intellisper marketing. Use as Shadcn pattern reference only.
