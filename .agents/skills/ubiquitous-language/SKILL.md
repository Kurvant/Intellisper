...
description: Maintains feature documentation in .agents/features/ and performs mandatory feature overlap detection before any new feature is proposed. Also builds a shared domain vocabulary for Intellisper. Use when the user asks to define domain terms, build a glossary, harden terminology, create ubiquitous language, references "domain model" or "DDD", or asks about adding a new feature.
---

# Intellisper Ubiquitous Language

Maintain per-feature documentation in `.agents/features/`, establish a shared vocabulary for the Intellisper domain, and prevent redundant features by detecting overlap with existing functionality before any new work begins.
...
Every feature in Intellisper must have a corresponding `.md` file in `.agents/features/`. This is a living registry that agents and developers use to understand what exists before building something new.
...
- Names a core entity (e.g. Flow, Block, Step, Run, Connection, Project, Platform)
...
| **Undefined jargon** — term used but never defined | "block" first appears without explanation | Ask for a one-sentence definition |
...
- Group terms into **domain clusters** (e.g., Automation Core, Platform & Multi-tenancy, Blocks & Integrations, Tables & Data, Authentication).
