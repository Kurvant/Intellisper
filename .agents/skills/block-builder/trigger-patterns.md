...
Use when the API does NOT support webhooks. Intellisper polls every ~5 minutes.
...
```typescript
import { createTrigger, TriggerStrategy, AppConnectionValueForAuthProperty } from '@intelblocks/blocks-framework';
import { DedupeStrategy, Polling, pollingHelper, httpClient, HttpMethod, AuthenticationType } from '@intelblocks/blocks-common';
...
**Real example:** `packages/blocks/community/airtable/src/lib/trigger/new-record.trigger.ts`
...
```typescript
import { createTrigger, TriggerStrategy } from '@intelblocks/blocks-framework';
...
export const newRecordWebhookTrigger = createTrigger({
  ...
  async onEnable(context) {
    ...
    const response = await httpClient.sendRequest<{ id: string }>({
      ...
      body: {
        url: context.webhookUrl,         // Intellisper provides this
...
**Real example:** `packages/blocks/community/stripe/src/lib/trigger/new-customer.ts`
...
```typescript
import { WebhookHandshakeStrategy } from '@intelblocks/blocks-framework';
