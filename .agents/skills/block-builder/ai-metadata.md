...
Blocks power both human flow-builders and AI agents (via the MCP server and the agent tooling). Two fields declare how an action or trigger appears to agents. They are additive — they change nothing for human users — but the catalog is now fully curated (every existing action carries them), so **new actions and triggers must ship with them**: a block authored without AI metadata is a regression that has to be backfilled later.
...
**Write the value explicitly — do not omit it.** Block metadata is serialized from the raw action objects with no default injection, so downstream filters only see `audience` when it is physically present in the file. For a normal integration action, write `audience: 'both'`.
