...
> **Always pass `auth`.** Every `Property.Dropdown`, `Property.MultiSelectDropdown`, and `Property.DynamicProperties` whose `options`/`props` callback reads `auth` MUST set `auth: <pieceAuth>` (e.g. `auth: myAppAuth`). Without it, `auth` is `undefined` in the callback and the dropdown can never load. Import the auth object from the block's `auth.ts` (or `../..`). See `packages/blocks/community/github/src/lib/common/index.ts` for the real pattern.
...
**Real example:** `packages/blocks/community/github/src/lib/common/index.ts` -- see `repositoryDropdown`, `issueDropdown`, `labelDropDown`
...
For the `BlockCategory` values used in `createBlock({ categories: [...] })`, see `block-types.md` — it lists every category with guidance on which to pick. (Categories aren't a prop type, so they live there rather than here.)
