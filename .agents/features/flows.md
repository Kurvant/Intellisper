...
## Summary
Flows are the core automation primitive in Intellisper. Each flow is a versioned directed graph of trigger and action steps stored as a JSONB tree. The module handles the full lifecycle: draft editing via a single-endpoint operation dispatch, publishing (locking a version and registering the trigger source), enabling/disabling, folder organization, sample data capture for testing, human-input forms/chat interfaces, PNG export of the canvas, and the visual builder frontend powered by XYFlow. All 26 flow modification types are dispatched through one endpoint (`POST /v1/flows/:id`) with a discriminated-union body.
...
- `packages/web/src/app/builder/state/` — Zustand-based builder state (flow, run, canvas, notes, step form, block selector)
- `packages/web/src/app/builder/step-settings/` — step configuration panel and split/drawer layout for the step data panel; `block-settings/connection-select.tsx` is the per-step connection picker — it renders an inline status indicator for every connection state (Connected / Error / Missing, via `getConnectionStatusDisplay`) alongside a single unified `Cable` reconnect icon button (replacing the old per-state mix of refresh icon + text button). The indicator overlay is `pointer-events-none` so clicking the status label opens the dropdown; only the reconnect button re-enables pointer events and opens the reconnect dialog.
...
- `packages/web/src/app/builder/blocks-selector/` — block/action browser
- `packages/web/src/app/builder/data-selector/` — variable picker (mentions / data selector); `index.tsx` hosts main's **Data** / **Variables** tabs, with a **Friendly** / **Advanced** toggle inside the Data tab. When a step is a PIECE action/trigger, friendly mode builds its tree from the block's `outputSchema` via `utils-schema.ts`; both modes render through the shared `data-selector-node.tsx` (friendly mode adds per-field type icons via `field-type-icon`). Otherwise it falls back to a generic field list. The schema is fetched via `usePieceOutputSchema` (see [blocks.md](../features/blocks.md))
...
- **Enterprise (EE) / Cloud**: Same core. Some operational features (owner transfer, block filtering, template sharing) integrate with EE plan flags. Active-flow quota enforcement on publish/enable.
...
- `block-selector-state.ts` — block browser visibility and search
...
The canvas supports two orientations, toggled from the canvas controls (↔ / ↕ button, preference persisted in localStorage): the default top-to-bottom **vertical** layout (unchanged, 232×60 step cards) and a left-to-right **horizontal** layout (compact 80×80 square nodes, step name + block type below the node, internal step name on hover, branches stacked vertically with labels on the entry lines, loops with a return lane).

How it works: the graph builder (`flow-canvas/utils/flow-canvas-utils.ts`) always builds in vertical "layout space" (y advances along the flow, x spreads branches) using orientation-aware sizes/gaps from `ORIENTATION_LAYOUT` (`flow-canvas/utils/layout-consts.ts`, a pure module with no component imports so tests can load it). For horizontal mode the finished graph is reflected across the `y = x` axis: node positions are transposed in the builder, and the untouched vertical edge-path code is transposed as SVG by `svgPathUtils.transposePath` (`flow-canvas/utils/svg-path-utils.ts`). Each edge component maps its endpoints back into layout space via the shared `useEdgeLayoutSpace` hook (`flow-canvas/edges/use-edge-layout-space.ts`). The `<ReactFlow>` element is keyed by orientation so handles (Top/Bottom vs Left/Right) re-measure on toggle.

Step dragging is unchanged in both orientations: dropping on an add button moves the step within the flow (dnd-kit), dropping anywhere else is a no-op.
...
Two builder surfaces consume an action/trigger's optional `outputSchema` (defined on the block — see [blocks.md](./blocks.md)):
