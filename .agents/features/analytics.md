...
- **Blocks analytics**: Separate service that counts how many projects actively use each block and updates `pieceMetadata.usage`.
...
## Blocks Analytics (`blocks-analytics.service.ts`)

**Scheduled**: Daily cron at 12:00 UTC

Tracks which blocks are actively used:
1. For each enabled flow → get published version → extract block steps
2. Group by block → count unique projects using each block
