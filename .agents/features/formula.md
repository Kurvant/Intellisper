...
- `function-registry.ts` — 81-entry array `IB_FUNCTIONS` and the `ApFunction` type (with optional `deprecated`, `argCompatibility` extension points).
...
### Editor UI (`packages/web/src/app/builder/block-properties/text-input-with-mentions/`)
...
- `components/function-search-popover.tsx` — filter-as-you-type list backed by `IB_FUNCTIONS`.
...
### Web-side (`packages/web/test/app/builder/block-properties/text-input-with-mentions/`)
...
- **Registry** — the `IB_FUNCTIONS` array; the single source of truth for function names, signatures, descriptions, examples, arg types, return types, and deprecation status.
...
3. **`wrapStringArgs`** — for every recognized `IB_FUNCTIONS` call, inspects each arg's expected type from the registry; if the slot expects `'string'` (or a union including string) and the arg isn't already quoted / a function call / a placeholder, wraps it in `"..."`. Recurses into nested calls.
...
- **Function impl change** — versioning policy: bump `@intelblocks/shared` minor + changelog entry.
...
- No new HTTP endpoints. Function metadata is bundled in `@intelblocks/shared` and read directly by the frontend; no `/v1/formulas/*` route exists.
