...
## Summary
Managed Auth (also called "Embedding" or "Managed Authentication") enables SaaS vendors to embed the Intellisper workflow builder inside their own product. The vendor's backend signs a short-lived JWT using an RSA private key (from a Signing Key), then passes it to the Intellisper embed SDK. The SDK calls `POST /v1/managed-authn/external-token` with that JWT. The backend verifies the JWT against the stored public key, then auto-provisions or retrieves the user, project, and project limits defined in the token claims, and returns a full Intellisper `AuthenticationResponse` (including an access token). This feature is gated by `platform.plan.embeddingEnabled`.
...
- **Signing Key**: An RSA key pair registered on the platform; the public key is stored in Intellisper, the private key is kept by the vendor.
- **External Principal**: The parsed, verified identity extracted from the JWT (platformId, externalUserId, externalProjectId, etc.).
- **externalUserId**: Vendor-assigned stable user identifier; hashed with platformId to create an Intellisper identity email (`managed_<platformId>_<externalUserId>`).
- **externalProjectId**: Vendor-assigned stable project identifier; maps to an Intellisper project via `externalId`.
...
|---|---|---|---|
| POST | `/v1/managed-authn/external-token` | Public | Exchange external JWT for Intellisper session token |
...
```
{ externalUserId, externalProjectId, firstName, lastName, role?, blocks?, concurrencyPoolKey?, concurrencyPoolLimit? }
```
...
5. Call `updateProjectLimits` — resolves allowed blocks from tags/filter type and upserts the project plan.
...
8. Generate a 7-day Intellisper access token and return the full `AuthenticationResponse`.
