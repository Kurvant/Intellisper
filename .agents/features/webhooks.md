...
**Async path**:
1. Offload payload to S3/DB if > `IB_WEBHOOK_PAYLOAD_INLINE_THRESHOLD_KB` (default 512KB). The job carries a `JobPayload` discriminated union — `inline` (value embedded) or `ref` (`fileId` of a `WEBHOOK_PAYLOAD` file).
...
3. Wait for engine to send response (default timeout: `IB_WEBHOOK_TIMEOUT_SECONDS`, default 30; callers may pass `timeoutMs` to override per-invocation, e.g. MCP uses 5 minutes)
...
- Submits HANDSHAKE hook job to worker → block validates signature → returns verification response

## Payload Size Limit

`IB_MAX_WEBHOOK_PAYLOAD_SIZE_MB` (default 5MB). Returns 413 if exceeded.
