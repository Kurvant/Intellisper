...
- `packages/server/api/src/app/ee/chat/chat-approval-gate.ts` — Redis pub/sub gate for tool execution approval (5-min timeout); uses atomic SET NX for first-decision-wins semantics; also stores per-conversation cancel signals with a 10-min TTL; manages server-side connection store (available/selected connections per conversation+block) and pending gate persistence for refresh resilience
- `packages/server/api/src/app/ee/chat/chat-file-utils.ts` — file attachment processing (base64, MIME validation, 10MB limit)
- `packages/server/api/src/app/ee/chat/tools/chat-tools.ts` — cross-project tool execution (auth discovery, action execution with server-managed connections, resource listing); `ib_execute_action` auto-fills connectionExternalId and projectId from Redis connection store — the LLM never sees credential IDs
...
- `packages/server/api/src/app/ee/chat/mcp/chat-mcp.ts` — connects to Intellisper MCP server for project-scoped tools with approval wrapping
...
- **Two-phase toolset gating** — the agent runs in a `discovery` or `build` phase (`tool-phases.ts`); a denylist hides build-only tools during discovery to shrink the tool surface. `ib_set_phase` flips the phase; the gate auto-widens if a build/manage tool fires so the agent can't get stuck
- **Local tools** — chat-specific tools not part of MCP: `ib_set_session_title`, `ib_select_project`, `ib_deselect_project`, `ib_execute_action`, `ib_list_across_projects`, `ib_explore_data`, `ib_load_guide`, `ib_set_phase`
- **Display tools** — tools that render interactive UI cards: `ib_show_connection_required`, `ib_show_connection_picker`, `ib_show_project_picker`, `ib_show_questions`, `ib_show_quick_replies`
- **MCP tools** — project-scoped tools loaded from the Intellisper MCP server when a project is selected; wrapped only with a per-call execution timeout (`withToolTimeouts`) — the chat no longer gates them behind approval
- **Tool call UX metadata** — optional `title` (2-4 word chip label) and `description` (first-person conversational sentence) stored on `PersistedToolCallPart`; description is sourced from the preceding `ib_update_thinking_status` text with `input.description` as fallback, rendered above the tool card chip
- **Server-managed connections** — connection externalIds are never exposed to the LLM; `ib_discover_action_auth` stores available connections in Redis (with `grantedScopes` and `requiredScopes` for scope-aware selection), `ib_show_connection_picker` stores the user's selection, and `ib_execute_action` auto-fills the connection from the store
...
- **Credits warning banner** — a dismissible amber banner shown when Intellisper AI credits usage reaches 70%; a non-dismissible red banner is shown when credits are fully exhausted
...
## Local Tools
- `ib_set_session_title` — auto-names the conversation after the first exchange
- `ib_select_project` — switches project context (scopes MCP tools to that project)
- `ib_execute_action` — executes a single block action ad-hoc (e.g. "check my inbox"); connections are managed server-side (the LLM never sees externalIds); write/destructive actions trigger an action preview gate before execution; emits `ACTION_RECEIPT` events after completion
- `ib_list_across_projects` — lists flows, tables, runs, or connections across all user-accessible projects
- `ib_deselect_project` — clears the selected project context
- `ib_explore_data` — read-only exploration of the user's data (sheets, channels, columns) to build understanding during discovery; never configures the automation
- `ib_load_guide` — loads an on-demand prompt guide (e.g. `build_flow`) so guidance is only in context when needed
- `ib_set_phase` — flips the agent between the `discovery` and `build` tool phases

## Display Tools
- `ib_show_connection_required` — prompts the user to connect a service
- `ib_show_connection_picker` — lets the user choose between multiple connections; no longer receives connections array from LLM (server-managed); returns only `{ selected: true, label }` to LLM, stripping externalIds
- `ib_show_project_picker` — lets the user select a project
- `ib_show_questions` — renders an interactive multi-question form
- `ib_show_quick_replies` — shows suggested response buttons
...
- `GET /v1/chat/conversations/:id/connections?blockName=` — get available connections for connection picker; falls back to `findConnectionsForPiece` when the Redis cache is empty and stores the result for future calls
...
5. Display-tool cards (connection picker, questions) and ad-hoc write actions (`ib_execute_action` previews) pause and emit a gate request to the UI via the stream; flow build/test/publish run without gating
