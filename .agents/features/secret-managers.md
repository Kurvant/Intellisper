...
Where `SecretManagerFieldsSeparator` is a constant string defined in `@intelblocks/shared`. A value that does not start with `{{` or does not contain the separator is treated as a plain literal (error code `SECRET_MANAGER_KEY_NOT_SECRET`), not an error condition.
