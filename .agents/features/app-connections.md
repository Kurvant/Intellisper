...
## Summary
App Connections store encrypted authentication credentials (OAuth2 tokens, API keys, basic auth, or custom block-defined fields) that flow steps use to call external services. They support automatic OAuth2 token refresh with distributed locking, a dual-scope model (project-level or platform-wide), and a "replace" operation that atomically rewires all flow references from one connection to another. The module handles all OAuth2 variants: user-supplied credentials, platform-managed OAuth apps, and Intellisper-hosted cloud OAuth, Users can optionally select a subset of a block's declared OAuth2 scopes when creating a connection.
...
**AppConnection**: id, displayName, externalId, type (AppConnectionType), status (ACTIVE/EXPIRED/ERROR), value (EncryptedObject), platformId, blockName, blockVersion, ownerId (nullable FK), projectIds[] (string array — multi-project), scope (PROJECT/PLATFORM), metadata (JSONB), preSelectForNewProjects (boolean).
...
| CUSTOM_AUTH | block-defined custom fields | None |
| NO_AUTH | (empty) | None |
...
- `GET /v1/app-connections` — list with filters (blockName, displayName ILIKE, status, scope, externalIds)
...
- `POST /v1/app-connections/oauth2/authorization-url` — generate OAuth redirect URL from block metadata; accepts optional scopes array to restrict the authorization to a user-selected subset of the block's declared scopes (all block scopes used when omitted)
