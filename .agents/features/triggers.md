...
- **Renewal job** — BullMQ repeating job that calls ON_RENEW hook for webhook blocks that need periodic re-registration (e.g., expiring webhooks)
- **sourceName** — format `blockName@version:triggerName`; used as a stable identifier for TriggerEvents

## Entities

**TriggerSource**: id, flowId, flowVersionId, projectId, type (TriggerStrategy), blockName, blockVersion, triggerName, simulate (boolean — true for test triggers), schedule (JSONB for polling cron). Unique on (projectId, flowId, simulate) with soft delete filter.

**TriggerEvent**: id, flowId, projectId, sourceName (format: `blockName@version:triggerName`), fileId (FK to File storing serialized payload).
...
- **WEBHOOK**: External service pushes events to Intellisper webhook URL.
...
- POLLING: Creates BullMQ repeating job with cron from block or default interval (`IB_TRIGGER_DEFAULT_POLL_INTERVAL`)
- WEBHOOK: Submits ON_ENABLE hook to worker (registers webhook with external service). If block has renewConfiguration with CRON strategy, creates renewal job.
...
- Redis key: `trigger_run:{platformId}:{blockName}:{date}:{status}`
