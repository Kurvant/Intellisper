...
- **Worker side** (`worker.ts#pollAndExecute`): if the connected app's `APP_VERSION !== IB_VERSION`, it pauses polling (`VERSION_MISMATCH_POLL_PAUSE_MS`, 10s) and retries. A new worker won't pull from an old app.
