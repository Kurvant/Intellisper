...
## Summary
A Platform is the top-level tenant namespace in Intellisper. Every installation has at least one platform. It owns branding (logo, colors, favicon), authentication settings (email auth toggle, allowed auth domains, federated SSO providers), block filtering rules, and a `PlatformPlan` that governs feature flags and resource limits. On Cloud a user can own multiple platforms; on CE/EE there is typically one. Platform admins can update branding, auth settings, and block pinning. Platform deletion is Cloud-only and triggers async cleanup.
...
## Domain Terms
- **Platform** — tenant root; owns branding, auth config, block filters
...
- **pinnedPieces** — ordered list of block names shown at the top of the block selector
- **cloudAuthEnabled** — whether platform-managed OAuth (Intellisper-hosted app credentials) is active
...
| pinnedPieces | string[] | ordered block name list |
...
| POST | `/v1/platforms/:id` | platformAdminOnly (USER) | Update branding, auth settings, block filters |
