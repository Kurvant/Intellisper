...
- **Step file**: A file produced by a block action during execution (e.g., a generated PDF). Uploaded by the engine, downloaded by end users via a signed JWT token.
...
| `FLOW_STEP_FILE` | Configurable | Yes | File output from a block action step |
...
| `PACKAGE_ARCHIVE` | Always DB | No | Block package archive |
