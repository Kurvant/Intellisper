...
## Domain Terms
- **copilot_code_chunks** — the vector index table holding parsed segments of the Intellisper codebase and docs
...
- `indexAll()` — globs `.ts/.tsx`, `.md/.mdx`, `package.json` files (excludes dist, node_modules, tests, blocks, secrets); parses with `copilotTsParser` or `copilotMdParser`; embeds in batches of 50; upserts to `copilot_code_chunks`; updates `searchVector` for new rows; deletes stale rows
