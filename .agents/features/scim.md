...
## Summary
SCIM (System for Cross-domain Identity Management) 2.0 integration allows enterprise Identity Providers (IdPs) such as Okta, Azure AD, and Google Workspace to automatically provision, update, and deprovision users and groups in Intellisper. Users map directly to Intellisper platform users; Groups map to Intellisper projects (only `ProjectType.TEAM` projects). Authentication uses an API key passed as a Bearer token (`platformAdminOnly SERVICE` principal). The module also exposes the SCIM discovery endpoints (`ServiceProviderConfig`, `ResourceTypes`, `Schemas`) for IdP auto-configuration. Gated by `platform.plan.scimEnabled`.
...
## Domain Terms
- **SCIM User** → Intellisper `User` + `UserIdentity` on the platform.
- **SCIM Group** → Intellisper `Project` with `type = TEAM`.
- **externalId**: Vendor-side stable identifier for users and groups, stored on the Intellisper entity.
- **active**: Boolean SCIM field; maps to `UserStatus.ACTIVE` / `UserStatus.INACTIVE`.
- **SCIM_CUSTOM_USER_ATTRIBUTES_SCHEMA**: Custom extension schema `urn:ietf:params:scim:schemas:intellisper:1.0:CustomUserAttributes` — carries `platformRole` for assigning admin vs. member roles during provisioning.
...
SCIM Groups map to Intellisper projects with `type = TEAM`:
