...
|---|---|---|---|
| Action | A single executable step within a flow that performs an operation (HTTP call, data transform, code execution, etc.). | task, command | Flow, Step, Block |
...
| AgentTool | A discriminated union of the four tool types attachable to an agent step: Block, Flow, MCP, or Knowledge Base. | — | Agent, PredefinedInputsStructure |
...
| Store Entry | A key-value pair scoped to a project, used by blocks to persist state across flow runs. | kv store, project store | — |
...
| Term | Definition (one sentence) | Aliases to avoid | Related terms |
|---|---|---|---|
| App Connection | A stored set of credentials (OAuth2, API key, basic auth, etc.) used by blocks to authenticate with external services. | credential, auth, integration key | Block, Connection Type, Global Connection |
...
| OAuth App | Custom OAuth2 client credentials registered per block to override Intellisper defaults. | — | App Connection, Block |
| Block | A packaged integration (npm package) that provides triggers and actions for a specific service or capability. | connector, plugin, integration, app | Action, Trigger, Block Metadata |
| Block Metadata | The registry entry for an installed block — name, version, auth schema, available actions/triggers. | — | Block |
...
| Managed Auth | JWT-based authentication for embedded Intellisper — exchanges an external token for an AP session with auto-provisioned user/project. | embedded auth, external token | Signing Key, Platform |
...
| Secret Manager | An external vault integration (AWS Secrets Manager, HashiCorp Vault, CyberArk Conjur, 1Password) for storing connection secrets outside Intellisper. | vault, credential store | App Connection |
...
| AI Provider | A configured LLM backend (OpenAI, Anthropic, Google, Azure, OpenRouter, Cloudflare, Custom, Intellisper) with encrypted credentials. | model provider, LLM config | AI Credits, Agent |
...
| Badge | A gamification award (9 types) given to users for milestones like first build, webhook usage, or AI block adoption. | achievement, reward | User |
| MCP Server | A per-project Model Context Protocol endpoint that exposes Intellisper tools to AI clients (Claude Desktop, Cursor, etc.). | — | MCP, Agent |
