---
title: "Changelog"
description: "A log of all notable changes to Intellisper"
---
...
Intellisper now ships with a built-in **network proxy that protects your automations from network attacks**.

- **One switch to turn it on** — set `IB_NETWORK_MODE=STRICT` and the protection is active. It's **opt-in today**, but in an upcoming release it will become **opt-out** (on by default).
- **Allow list for trusted internal services** — if a flow legitimately needs to reach an internal API or database, add its address (or a whole subnet like `10.10.0.0/24`) to `IB_SSRF_ALLOW_LIST`.
- **Works with your corporate proxy** — if you already use `HTTP_PROXY` / `HTTPS_PROXY`, Intellisper respects them automatically.
...
![Project Settings — Max Concurrent Jobs](/img/screenshots/manage-concurrency.png)
...
- **[Smoke tests](https://github.com/Kurvant/Intellisper/blob/main/.github/workflows/smoke-test.yml) in GitHub Actions** — validates health checks and webhook flow execution on AMD64 and ARM64
- **[Benchmark tests](https://github.com/Kurvant/Intellisper/blob/main/.github/workflows/benchmark.yml) in GitHub Actions** — load testing across 6 app/worker configurations measuring throughput, mean latency, P50, P99 ([see results](/install/architecture/benchmark))
...
- **Connections** — Blocks and Owner filters, flow usage count, inline edit and refresh actions
...
[Read more about our release cycle](https://intellisper.com/release)

</Update>

For previous releases, see the [GitHub Releases](https://github.com/Kurvant/Intellisper/releases) page.
