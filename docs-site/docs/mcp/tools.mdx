...
description: "Complete catalog of all MCP tools available in Intellisper"
---
...
Read the full source code, package.json, and input mappings of a CODE step. Returns untruncated content — unlike `ib_flow_structure` which truncates code to 300 characters.
...
<Tip>
Use this tool when you need to read or edit a CODE step's full source. `ib_flow_structure` truncates code for overview purposes, so always call `ib_read_step_code` before modifying a code step.
</Tip>
...
Research available blocks. Use `blockNames` for bulk exact lookup (always returns actions and triggers). Use `searchQuery` for fuzzy discovery.

| Input | Type | Required | Description |
|-------|------|----------|-------------|
| `blockNames` | string[] | No | Exact block names to look up (always returns actions and triggers) |
| `searchQuery` | string | No | Filter blocks by name |
...
Get the detailed input property schema for a specific block action or trigger. Returns field names, types, required/optional, descriptions, default values, and dropdown options. When auth is required but not provided, automatically lists available connections. Use this before `ib_update_step` or `ib_update_trigger` to know exactly which fields to set.

| Input | Type | Required | Description |
|-------|------|----------|-------------|
| `blockName` | string | Yes | Block name (e.g., `@intelblocks/block-slack`) |
...
Resolve dropdown options for a single block property. Returns available choices with labels and internal IDs. Use this to discover valid values for dynamic dropdown fields (e.g., Slack channels, Google Sheets, email labels) before configuring a step.
...
| Input | Type | Required | Description |
|-------|------|----------|-------------|
| `blockName` | string | Yes | Block name (e.g., `@intelblocks/block-google-sheets`) |
...
<Tip>
Use this instead of multiple `ib_resolve_property_options` calls when fields depend on each other. For example, to resolve Google Sheets columns: pass `spreadsheetId` and `sheetId` with their `selectedValue`, and `values` without — the tool resolves the full chain and returns the column fields in one call.
</Tip>
...
| Input | Type | Required | Description |
|-------|------|----------|-------------|
| `stepType` | string | Yes | `BLOCK_ACTION`, `BLOCK_TRIGGER`, `CODE`, `LOOP_ON_ITEMS`, or `ROUTER` |
| `blockName` | string | No | For BLOCK types: block name (short names like `slack` accepted) |
| `actionName` | string | No | For BLOCK_ACTION: action name |
| `triggerName` | string | No | For BLOCK_TRIGGER: trigger name |
| `input` | object | No | For BLOCK types: input config to validate |
| `auth` | string | No | For BLOCK types: any non-empty string indicates auth is provided |
...
| Input | Type | Required | Description |
|-------|------|----------|-------------|
| `blockName` | string | No | Filter by block name. Short names like `slack` or `google-sheets` are auto-expanded. |
...
| Input | Type | Required | Description |
|-------|------|----------|-------------|
| `provider` | string | No | Filter by provider (`openai`, `anthropic`, `google`, `azure`, `openrouter`, `intellisper`, `cloudflare-gateway`, `custom`) |
...
| `blockName` | string | No | For connections: which block needs auth |

### ib_set_project_context
...
<Tip>
After duplicating, use `ib_flow_structure` on the new flow to check configuration status. Steps that reference connections will need to be re-configured with `ib_update_step`.
</Tip>
...
Create a complete flow in one call — trigger plus any number of steps. Steps are added sequentially (trigger → step_1 → step_2 → ...). All steps are validated on creation. Use granular tools (`ib_add_step`, `ib_update_step`) to modify existing flows or add nested structures (loop contents, router branches).
...
| `trigger` | object | Yes | `{blockName, triggerName, input?, auth?}` |
| `steps` | array | Yes | Array of step specs, each with `type`, `displayName`, and type-specific fields |

**Step types in the array:**
- **BLOCK**: `blockName`, `actionName`, `input`, `auth`, `continueOnFailure`, `retryOnFailure`
...
|-------|------|----------|-------------|
| `flowId` | string | Yes | The flow ID |
| `blockName` | string | Yes | Block name (e.g., `@intelblocks/block-gmail`) |
| `triggerName` | string | Yes | Trigger name from the block |
...
| `stepType` | string | Yes | `CODE`, `BLOCK`, `LOOP_ON_ITEMS`, or `ROUTER` |
| `displayName` | string | Yes | Step display name |
| `blockName` | string | No | For BLOCK steps |
| `actionName` | string | No | For BLOCK steps |
...
| `continueOnFailure` | boolean | No | For CODE/BLOCK steps: continue flow if this step fails (default: false) |
| `retryOnFailure` | boolean | No | For CODE/BLOCK steps: retry this step on failure (default: false) |
...
| `continueOnFailure` | boolean | No | For CODE/BLOCK steps: continue flow if this step fails |
| `retryOnFailure` | boolean | No | For CODE/BLOCK steps: retry this step on failure |
...
Manage conditional branches in router steps. Use `ib_flow_structure` to see existing branch conditions and indices.
...
| `conditions` | array | No | New conditions (same format as `ib_add_branch`). Replaces existing conditions entirely. |
...
Execute a single block action once without building or saving a flow. Designed for one-shot tasks like *"check my inbox"* or *"send one Slack message"* where building a full automation would be overkill. Under the hood the tool creates a disposable flow, runs the action, returns the output, and cleans the flow up — the user never sees it in their flow list.

| Input | Type | Required | Description |
|-------|------|----------|-------------|
| `blockName` | string | Yes | Block name (e.g. `slack` or `@intelblocks/block-slack`). Use `ib_research_blocks` to discover. |
| `actionName` | string | Yes | Action to run (e.g. `send_channel_message`). Use `ib_get_block_props` for the input shape. |
| `input` | object | No | Fully-resolved input for the action. Keys must match the block action's props. Pass raw values — do **not** wrap them in `{{…}}`. Omit entirely if the action has no props. |
| `connectionExternalId` | string | No | `externalId` from `ib_list_connections`. Required if the block needs auth. Auto-wrapped server-side as `{{connections['externalId']}}`. Must be a plain ID — special characters are rejected. |

<Info>
When to pick this vs. `ib_build_flow`:
- **`ib_run_action`** — one-off, throwaway, returns a result now.
- **`ib_build_flow`** — persistent automation that should repeat, run on a schedule, or trigger on external events.
</Info>

Prefer calling `ib_list_connections` and `ib_get_block_props` first so you know the exact `actionName`, the expected props, and the right `connectionExternalId` before invoking. Missing required inputs or unknown actions produce a friendly error before dispatch — no run is created and nothing is billed.
