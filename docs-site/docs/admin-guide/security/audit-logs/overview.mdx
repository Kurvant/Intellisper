...
Intellisper records an audit event for every meaningful action on the platform — a flow gets created, a run finishes, a user signs in, a connection is saved, a signing key is rotated, and so on. Each event captures what happened, who did it, when, and the project or platform it belongs to.
...
![Audit Logs](/img/screenshots/audit-logs.png)
...
The full list of documented events. We add new ones as the platform grows; the source of truth for every event we emit is the [event schema in the codebase](https://github.com/Kurvant/Intellisper/blob/main/packages/shared/src/lib/ee/audit-events/index.ts).
