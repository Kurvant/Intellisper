...
description: "Configure SCIM provisioning from Okta to Intellisper."
---
...
- Generated an API key from `/platform/security/api-keys` route in the Intellisper app

## Configure SCIM Connection in Okta

<Steps>
  <Step title="Open your Okta app">
    In Okta Admin Console, open your Intellisper application (created in first [SSO step](/admin-guide/guides/sso#saml-with-okta)).

  </Step>
  <Step title="Enable provisioning">
    In the app's **General** tab, enable **SCIM Provisioning**.
    ![Okta User Provisioning](/img/screenshots/scim/okta/enable-provisioning.png)
  </Step>
  <Step title="Add SCIM connection details">
    **Provisioning** will be visible, go to it and set:
    - **SCIM base URL** to `https://your-intellisper-domain/api/v1/scim/v2`
...
![Okta SCIM Connection Image](/img/screenshots/scim/okta/provisioning.png)
...
<Steps>
  ...
  <Step>
    ...
    | `External namespace` | `urn:ietf:params:scim:schemas:intellisper:1.0:CustomUserAttributes` |
    | `Enum` | `enabled` |
    ...
    ![Okta add attribute image](/img/screenshots/scim/okta/add-attribute.png)
  ...
  <Step>
    ...
    <Warning>
      This step assumes that you already have a field in the Okta user profile that you can map to platformRole in your Intellisper user profile. If you don't have one, you can create a new field in **Directory -> Profile Editor -> User (default)**.
    </Warning>

     - Back to your Intellisper application page in **Provisioning -> To App -> Attribute Mappings**
    ...
    ![Okta role expression example](/img/screenshots/scim/okta/role-expression.png)
...
In the Intellisper application page, go to **Provisioning -> To App** and enable the actions you want to be applied to Intellisper when changes occur in Okta.

![Okta role expression example](/img/screenshots/scim/okta/provision-actions.png)

Now in the **Assignments** tab you can:
- Choose to provision individual users or groups. Note that groups in Okta will be projects in Intellisper.

- In case you don't have groups and you want to provision your Okta users at once, you can assign the `Everyone` group.

- When editing/creating users in an assigned group (including `Everyone`), they should be updated in Intellisper.

- To push groups to Intellisper, go to the **Push groups** tab and click on the push button, find the group and save.

<Info>
  Default role for users in projects will be `Editor` role, right now there is no way to link the project role with Okta
</Info>

![Okta role expression example](/img/screenshots/scim/okta/push-group.png)



<Tip>
  Created users in Intellisper will receive a welcome email. When clicked, they will be redirected to sign in with `SAML`.
</Tip>


### Deprovision

Users' state switches to `INACTIVE` in Intellisper only when they are deactivated in Okta. Suspension or deletion in Okta does not reflect in Intellisper because of Okta's design.
...
<Warning>
  Deleting a group will delete the whole project in Intellisper with its flows and connections. Users linked to that group won't be affected.
</Warning>
