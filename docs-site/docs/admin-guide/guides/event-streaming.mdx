...
Event Streaming forwards platform [audit events](/admin-guide/security/audit-logs/overview) to a webhook URL of your choice. The most common pattern is to point that webhook at a flow inside Intellisper — your flow then routes each event to the channel you care about (Slack, Gmail, Microsoft Teams, custom HTTP, etc.).
...
![Create event destination](/img/screenshots/event-destination.png)
...
![Event destination handler flow](/img/screenshots/event-destination-flow.png)
4. Customize the branches with whatever blocks you like — Slack, Gmail, Microsoft Teams, custom HTTP, and so on.
