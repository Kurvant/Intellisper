...
description: "Configure Single Sign-On (SSO) to enable secure, centralized authentication for your Intellisper platform"
---
...
Single Sign-On (SSO) allows your team to authenticate using your organization's existing identity provider, eliminating the need for separate Intellisper credentials. This improves security, simplifies user management, and provides a seamless login experience.

## Prerequisites

Before configuring SSO, ensure you have:

- **Admin access** to your Intellisper platform
- **Admin access** to your identity provider (Google, GitHub, Okta, or JumpCloud)
- The **redirect URL** from your Intellisper SSO configuration screen

## Accessing SSO Configuration

Navigate to **Platform Settings** → **SSO** in your Intellisper admin dashboard to access the SSO configuration screen.

![SSO Configuration](/img/screenshots/sso.png)
...
Intellisper supports multiple SSO providers to integrate with your existing identity management system.
...
<Steps>
    ...
    <Step title="Configure Redirect URI">
        Copy the **Redirect URL** from the Intellisper SSO configuration screen and add it to the **Authorized redirect URIs** in Google Cloud Console.
    </Step>
    <Step title="Copy Credentials to Intellisper">
        Copy the **Client ID** and **Client Secret** from Google and paste them into the corresponding fields in Intellisper.
    </Step>
...
<Steps>
    ...
    <Step title="Register New Application">
        ...
        - **Application name**: Choose a recognizable name (e.g., "Intellisper SSO")
        - **Homepage URL**: Enter your Intellisper instance URL
    </Step>
    <Step title="Configure Authorization Callback">
        Copy the **Redirect URL** from the Intellisper SSO configuration screen and paste it into the **Authorization callback URL** field.
    </Step>
    ...
    <Step title="Copy Credentials to Intellisper">
        Copy the **Client ID** and **Client Secret** and paste them into the corresponding fields in Intellisper.
    </Step>
    <Step title="Save Configuration">
        Click **Finish** to complete the setup.
    </Step>
...
<Steps>
    ...
    <Step title="Configure General Settings">
        Enter an **App name** (e.g., "Intellisper") and optionally upload a logo. Click **Next**.
    </Step>
    <Step title="Configure SAML Settings">
        - **Single sign-on URL**: Copy the SSO URL from the Intellisper configuration screen
        - **Audience URI (SP Entity ID)**: Enter `Intellisper`
    ...
    <Step title="Configure Intellisper">
...
<Steps>
    <Step title="Create an Enterprise Application">
        ...
        Name it (e.g., "Intellisper") and select **Integrate any other application you don't find in the gallery (Non-gallery)**.
    </Step>
    ...
    <Step title="Set Identifier and Reply URL">
        ...
        - **Identifier (Entity ID)**: `Intellisper`
        - **Reply URL (Assertion Consumer Service URL)**: paste the SSO URL from the Intellisper configuration screen
    </Step>
    ...
    <Step title="Copy the Federation Metadata">
        ...
        You can paste this URL directly into the **IdP Metadata** field in Intellisper — Intellisper will fetch the metadata XML automatically. Alternatively, open the URL in a browser, save the XML, and paste its contents.
    </Step>
    <Step title="Copy the Signing Certificate">
        Download the **Certificate (Base64)** from the **SAML Certificates** section. Open the file and copy its contents (including the `-----BEGIN CERTIFICATE-----` / `-----END CERTIFICATE-----` markers) into the **Signing Key** field in Intellisper.
    </Step>
    <Step title="Assign Users">
        Go to **Users and groups** in the application and assign the users or groups that should be allowed to sign in.
    </Step>
    <Step title="Save Configuration">
        Click **Save** in Intellisper to complete the setup.
    </Step>
</Steps>
...
<Steps>
    ...
    <Step title="Configure ACS URL">
        Copy the **ACS URL** from the Intellisper configuration screen and paste it into the **ACS URLs** field in JumpCloud.
        
        ![JumpCloud ACS URL](/img/screenshots/jumpcloud/acl-url.png)
    </Step>
    <Step title="Configure SP Entity ID">
        Set the **SP Entity ID** (Audience URI) to `Intellisper`.
    </Step>
    <Step title="Add User Attributes">
        ...
        ![JumpCloud User Attributes](/img/screenshots/jumpcloud/user-attribute.png)
    </Step>
    <Step title="Enable HTTP-Redirect Binding">
        ...
        ![JumpCloud Redirect Binding](/img/screenshots/jumpcloud/declare-login.png)
    ...
    <Step title="Export Metadata">
        ...
        ![JumpCloud Export Metadata](/img/screenshots/jumpcloud/export-metadata.png)
    ...
    <Step title="Configure IdP Metadata in Intellisper">
        Paste the exported metadata XML into the **IdP Metadata** field in Intellisper.
    </Step>
    ...
    <Step title="Assign Users to Application">
        ...
        ![JumpCloud Assign App](/img/screenshots/jumpcloud/user-groups.png)
    </Step>
    <Step title="Save Configuration">
        Click **Finish** to complete the setup.
    </Step>
...
<AccordionGroup>
    ...
    <Accordion title="SAML authentication fails">
        ...
        - If you pasted a metadata URL, make sure it is publicly reachable (Intellisper fetches it server-side)
...
If you encounter issues during SSO setup, please contact our enterprise support or [sales team](https://intellisper.com/sales).
