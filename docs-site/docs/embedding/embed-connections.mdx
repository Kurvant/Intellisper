...
<Info>
**Requirements:**
* Intellisper version 0.34.5 or higher
...
<Warning>
**Breaking Change:** <br/><br/> If your Intellisper instance version is &lt; 0.45.0 and (you are using the connect method from the embed sdk, and need the connection externalId to be returned after the user creates it OR if you want to reconnect a specific connection with an externalId), you must upgrade your instance to >= 0.45.0
</Warning>

- You can use the embedded SDK in your SaaS to allow your users to create connections and store them in Intellisper.
...
<Steps>
    ...
    <Step title="Call Connect Method">
        After initializing the SDK, you will have access to a property called `intellisper` inside your `window` object. Call its `connect` method to open a new connection dialog as follows.

        ```html
        <script> 
        intellisper.connect({blockName:'@intelblocks/block-google-sheets'});
        </script>
        ```
        ...
        **Connect Parameters:**
        ...
        | -------------- | -------- | ------ | ------------------------------------------- |
        | blockName      | ✅      | string | The name of the block you want to create a connection for.
        ...
        **Connect Result**
        ...
        <Tip>
        You can use the `connections` block in the builder to retrieve the created connection using its name.
        ![Connections in Builder](/img/screenshots/connections-piece.png)
        ![Connections in Builder](/img/screenshots/connections-piece-usage.png)
        </Tip>
    </Step>
</Steps>
