...
Use predefined connections to allow users to access your block in the embedded app without re-entering authentication credentials.
...
- (Optional) Hide the connections dropdown in the block settings.

### Prerequisites

- [Run the Enterprise Edition](/handbook/engineering/playbooks/run-ee)
- [Create your block](/build-blocks/building-blocks/overview). Later we will customize the block logic to use predefined connections.
...
<Steps>
  <Step title="Create an API Key">
    ...
    ![Create API Key](/img/screenshots/create-api-key.png)
  </Step>
...
const instanceUrl = 'https://cloud.intellisper.com';

// The name of the user / organization in your SAAS
const externalProjectId = 'org_1234';
const blockName = '@intelblocks/block-gelato';
// This will depend on what your block auth type is, can be one of this ['PLATFORM_OAUTH2','SECRET_TEXT','BASIC_AUTH','CUSTOM_AUTH']
const pieceAuthType = "CUSTOM_AUTH"
const connectionProps = {
    // Fill in the props required by your block's auth
}
...
<Steps>
    <Step title="Modify Trigger / Action Definition">
       Wherever you call `createTrigger` or `createAction` set `requireAuth` to `false`, this will hide the connections dropdown in the block settings in the builder,
...
const fetchConnection = async (
...
): Promise<BlockPropValueSchema<typeof auth>> => {
	...
	return connection as BlockPropValueSchema<typeof auth>;
};
