...
|---|---|---|---|
| `combine` | `combine(text1; text2; separator)` | Joins two or more values into one block of text. | `combine(John; Smith; " ")` → `John Smith` |
...
| `join_list` | `join_list(list; separator)` | Turns a list into a single block of text with a separator. | `join_list([bug;urgent;backend]; ", ")` → `bug, urgent, backend` |
