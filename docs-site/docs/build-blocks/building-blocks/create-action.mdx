...
1. `Block Folder Name`: This is the name associated with the folder where the action resides. It helps organize and categorize actions within the block.
...
This will create a new TypeScript file named `get-icecream-flavor.ts` in the `packages/blocks/community/gelato/src/lib/actions` directory.
...
export const getIcecreamFlavor = createAction({
  name: 'get_icecream_flavor', // Must be a unique across the block, this shouldn't be changed.
  auth: gelatoAuth,
  ...
  async run(context) {
    const res = await httpClient.sendRequest<string[]>({
      ...
      url: 'https://api.intellisper.com/v1/webhooks/RGjv57ex3RAHOgs0YK6Ja/sync',
      headers: {
...
To make the action readable by Intellisper, add it to the array of actions in the block definition.
...
<Tip>
If the build fails, try debugging by running `npx turbo run build --filter=@intelblocks/block-gelato`.
It will display any errors in your code.
</Tip>

To test the action, use the flow builder in Intellisper. It should function as shown in the screenshot.

![Gelato Action](/img/screenshots/gelato-action.png)
