...
This tutorial will guide you through the process of creating trigger for a Gelato block that fetches new icecream flavor.
...
1. `Block Folder Name`: This is the name associated with the folder where the trigger resides. It helps organize and categorize triggers within the block.
...
4. `Trigger Technique`: Specifies the trigger type - either [polling](../block-reference/triggers/polling-trigger) or [webhook](../block-reference/triggers/webhook-trigger).
...
? Enter the block folder name : gelato
...
This will create a new TypeScript file at `packages/blocks/community/gelato/src/lib/triggers` named `new-flavor-created.ts`.
...
> = {
  ...
  items: async ({ auth, propsValue, lastFetchEpochMS }) => {
    const request: HttpRequest = {
      ...
      url: 'https://api.intellisper.com/v1/webhooks/aHlEaNLc6vcF1nY2XJ2ed/sync',
      headers: {
        authorization: auth,
      },
    };
...
To make the trigger readable by Intellisper, add it to the array of triggers in the block definition.
...
By default, the development setup only builds specific components. Open the file `.env.dev` and include "gelato" in the `IB_DEV_BLOCKS`.
...
Once you edit the environment variable, restart the backend. The block will be rebuilt. After this process, you'll need to **refresh** the frontend to see the changes.

To test the trigger, use the load sample data from flow builder in Intellisper. It should function as shown in the screenshot.
...
3. Copy the tunnel domain, i.e wozcsvaint.loclx.io, and replace the `IB_FRONTEND_URL` environment variable in `.env.dev` with the exposed url, i.e https://wozcsvaint.loclx.io
...
![Gelato Action](/img/screenshots/gelato-trigger.png)