...
**1. Clone Intellisper repository.**

Use the command line to clone Intellisper repository:
...
```bash
docker compose -p intellisper up
```
...
ngrok http 8080
```
3. Replace `IB_FRONTEND_URL` environment variable in `.env` with the ngrok url.
...
To upgrade to new versions, which are installed using docker compose, perform the following steps. First, open a terminal in the intellisper repository directory and run the following commands.
