---
title: 'Helm'
description: 'Deploy Intellisper on Kubernetes using Helm'
---

This guide walks you through deploying Intellisper on Kubernetes using the official Helm chart.
...
```bash
git clone https://github.com/Kurvant/Intellisper.git
cd intellisper
```
...
Create a `my-values.yaml` file with your configuration. You can use the [example values file](https://github.com/Kurvant/Intellisper/blob/main/deploy/intellisper-helm/values.yaml) as a reference.
...
### 4. Install Intellisper

```bash
helm install intellisper deploy/intellisper-helm -f my-values.yaml
```
...
# Upgrade release
helm upgrade intellisper deploy/intellisper-helm -f my-values.yaml

# Check upgrade status
kubectl rollout status deployment/intellisper
```
...
1. **Pod won't start**: Check logs with `kubectl logs deployment/intellisper`
...
```bash
# View logs
kubectl logs deployment/intellisper -f

# Port forward for testing
kubectl port-forward svc/intellisper 4200:80 --namespace default
...
Intellisper supports three editions:
...
- **`cloud`**: For Intellisper Cloud deployments
...
For Enterprise Edition features and licensing, visit [intellisper.com](https://intellisper.com/docs/admin-console/overview).

## Environment Variables

For a complete list of configuration options, see the [Environment Variables](/install/configuration/environment-variables) documentation. Most environment variables can be configured through the Helm values file under the `intellisper` section.
...
```bash
helm uninstall intellisper

# Clean up persistent volumes (optional)
kubectl delete pvc -l app.kubernetes.io/instance=intellisper
```
