---
title: "Railway"
description: "Deploy Intellisper to the cloud in minutes using Railway's one-click template"
---

Railway simplifies your infrastructure stack from servers to observability with a single, scalable, easy-to-use platform. With Railway's one-click deployment, you can get Intellisper up and running in minutes without managing servers, databases, or infrastructure.
...
The Railway template deploys Intellisper with the following components:

- **Intellisper Application**: The main Intellisper container running the latest version from [Docker Hub](https://hub.docker.com/r/activepieces/activepieces)
...
Once deployed, Railway will provide you with a public URL where your Intellisper instance is accessible.

## Configuration

### Environment Variables

Railway allows you to configure Intellisper through environment variables. You can set these in the Railway dashboard under your project's **Variables** tab.
...
- `IB_TELEMETRY_ENABLED`: Enable/disable telemetry (default: `false`)
...
4. Update `IB_FRONTEND_URL` environment variable to match your custom domain
...
- [ ] Configure `IB_WORKER_CONCURRENCY` based on your workload and hardware resources
- [ ] Ensure PostgreSQL backups are configured in Railway
- [ ] Consider database scaling options in Railway

## Observability

Railway provides built-in observability features for Intellisper. You can view logs and metrics in the Railway dashboard.

## Upgrading

To upgrade to a new version of Intellisper on Railway:

1. Go to your Railway project dashboard
2. Navigate to **Deployments**
3. Click **Redeploy** on the latest deployment
4. Railway will pull the latest Intellisper image and redeploy
...
After deploying Intellisper on Railway:
...
4. **Explore blocks** - browse available integrations in the block library
...
- [Architecture Overview](/install/architecture/overview): Understand Intellisper architecture
