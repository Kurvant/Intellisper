...
description: "How to rollback Intellisper to a previous version"
---

## Overview

Intellisper ships a rollback command that reverses database migrations when you need to downgrade to a previous version. Most releases are fully rollback-safe — the release notes will let you know if one isn't.
...
Copy the `pglite` folder inside your configured `IB_CONFIG_PATH`:
...
docker run --rm --env-file .env --entrypoint npm \
  ghcr.io/kurvant/intellisper:0.78.0 \
...
services:
  intellisper:
    image: ghcr.io/kurvant/intellisper:0.77.0
```

### Step 4: Start Intellisper
...
```bash
dropdb intellisper
createdb intellisper
pg_restore -d intellisper backup-pre-upgrade.dump
```

Then switch your `docker-compose.yml` to the previous image version and start Intellisper.
