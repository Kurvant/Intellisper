...
<Steps>
  <Step title="Create Worker Token">
    To create a worker token, use the local CLI command to generate the JWT and sign it with your `IB_JWT_SECRET` used for the app server. Follow these steps:
    1. Open your terminal and navigate to the root of the repository.
    2. Run the command: `npm run workers token`.
    3. When prompted, enter the JWT secret (this should be the same as the `IB_JWT_SECRET` used for the app server).
    4. The generated token will be displayed in your terminal, copy it and use it in the next step.
    ![Workers Token](/img/worker-token.png)
  </Step>
  <Step title="Configure Environment Variables">
    Define the following environment variables in the `.env` file on the worker machine:
    - Set `IB_CONTAINER_TYPE` to `WORKER`
    - Specify `IB_FRONTEND_URL`
    - Provide `IB_WORKER_TOKEN`
  </Step>
  <Step title="Configure Persistent Volume">
    Configure a persistent volume for the worker to cache flows and blocks. This is important as first uncached execution of blocks and flows are very slow. Having a persistent volume significantly improves execution speed.
  ...
  <Step title="Verify Worker Operation">
    ...
    ![Workers Infrastructure](/img/workers.png)
  </Step>
  <Step title="Configure App Container Type">
    On the APP machine, set `IB_CONTAINER_TYPE` to `APP`.
  </Step>
</Steps>
