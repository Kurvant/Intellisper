...
description: "Checklist to follow after deploying Intellisper"
---
...
<Tip>
For licensing inquiries regarding the self-hosted enterprise edition, please reach out to `sales@intellisper.com`, as the code and Docker image are not covered by the MIT license.
</Tip>

<Note>You can request a trial key from within the app or in the cloud by filling out the form. Alternatively, you can contact sales at [https://intellisper.com/sales](https://intellisper.com/sales).<br></br>Please know that when your trial runs out, all enterprise [features](https://intellisper.com/pricing) will be shut down meaning any user other than the platform admin will be deactivated, and your private blocks will be deleted, which could result in flows using them to fail.</Note>

<Warning>
Before version 0.73.0, you cannot switch from CE to EE directly We suggest upgrading to 0.73.0 with the same edition first, then switch `IB_EDITION`.
</Warning>
...
1. Set the `IB_EDITION` environment variable to `ee`.
2. Set the `IB_EXECUTION_MODE` to anything other than `UNSANDBOXED`, check the above section.
3. Once your instance is up, activate the license key by going to **Platform Admin -> Setup -> License Keys**.

![Activation License Key](/img/screenshots/activation-license-key-settings.png)
