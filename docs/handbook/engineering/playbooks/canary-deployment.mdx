...
This removes the need to keep `IB_CANARY_PLATFORM_IDS` in sync across services and allows runtime changes without a redeploy.
...
| Env var | Value |
|---|---|
| `IB_FRONTEND_URL` | canary app's public URL |
...
| Env var | Value |
|---|---|
| `IB_WORKER_GROUP_ID` | `canary` |

Workers use `IB_FRONTEND_URL` for their Socket.IO RPC channel and for posting engine results back to the app. Canary workers are registered with the canary app, so the RPC path is fully isolated.
...
| Env var | Description |
|---|---|
| `IB_CANARY_APP_URL` | Internal URL of the canary app |
...
Required GitHub secrets: `IB_API_KEY` (the primary app's `IB_API_KEY` value).
