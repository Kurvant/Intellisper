...
description: "Guide for creating database migrations in Intellisper"
icon: "database"
---

Intellisper uses TypeORM as its database driver in Node.js. We support two database types across different editions of our platform.
...
- Developers creating blocks who want a quick setup
...
<Steps>
  <Step title="Setup IB_DB_TYPE">
    Set the `IB_DB_TYPE` environment variable to `POSTGRES` after making sure have latest state by running Intellisper first.
  </Step>
