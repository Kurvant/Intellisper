...
description: "Connect CyberArk Conjur to Intellisper for centralized secret management"
---

CyberArk Conjur is a secrets management solution that provides secure storage and access to credentials. Integration with Intellisper uses **host/API key authentication**: Intellisper authenticates as a Conjur host, receives a short-lived token, and uses it to retrieve secrets for which that host has `read` and `execute` permissions.
...
- A Conjur policy that defines a host for Intellisper and grants it access to the variables you want to use

## Conjur host configuration for Intellisper

To allow Intellisper to read secrets, configure a Conjur policy that declares a group, variables, a host, a layer, and the right permissions. The steps below describe how to create that policy file.

### Example policy (Intellisper)

The following policy defines a policy `intellisper` with a group, two variables, a host, a layer, and the grants so the host can read the variables.

```yaml
- !policy
  id: intellisper
  body:
    - !group intellisper-secrets
    - &variables
        - !variable
            id: key-1
            kind: password
    ...
    - !layer intellisper
    - !grant
        role: !layer intellisper
        members:
            - !host intellisper
    - !grant
        role: !group intellisper-secrets
        member: !layer intellisper
```

### Policy steps (summary)

1. **Declare a group** at the root of the policy (e.g. `intellisper-secrets`). This group will be allowed to read (and optionally execute) the variables.
...
3. **Declare the host** that Intellisper will use (e.g. `intellisper`) and a **layer** (e.g. `intellisper`), and add the host to the layer:
...
5. **Load the policy** into Conjur. Conjur will create the host and return an **API key** for that host. You will use this API key and the host identity when connecting Intellisper.
...
Store the **api_key** securely; you will enter it in Intellisper as the **API Key**.
...
Intellisper will authenticate as the configured host and retrieve the secret from Conjur when the flow runs.
