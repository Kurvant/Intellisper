...
Intellisper supports both standard OpenTelemetry environment variables and vendor-specific configuration for observability and tracing.

## Environment Variables

| Variable | Description | Default Value | Example |
|----------|-------------|---------------|---------|
| `IB_OTEL_ENABLED` | Enable OpenTelemetry tracing | `false` | `true` |
...
<Note>
Both `IB_OTEL_ENABLED` and `OTEL_EXPORTER_OTLP_ENDPOINT` must be set for OpenTelemetry to be enabled.
</Note>
