...
This page focuses on describing the main components of Intellisper and focus mainly on workflow executions.
...
**Third Party**:
- **Postgres**: The main database for Intellisper.
...
- **Webhooks**:
  All webhooks are sent to the Intellisper app, which performs basic validation and adds them to the queue. In case of a spike, webhooks will be added to the queue.
...
To scale Intellisper, you typically need to increase the replicas of either workers, the app, or the Postgres database. A small Redis instance is sufficient as it can handle thousands of jobs per second and rarely acts as a bottleneck.
...
- `blocks`: This package contains the implementation of triggers and actions for third-party apps.
