...
title: 'Flow Control'
icon: 'Joystick'
description: 'Learn how to control flow execution from inside a block'
---
...
`resumeDateTime` is capped by the server's `IB_PAUSED_FLOW_TIMEOUT_DAYS` setting; the engine throws `PausedFlowTimeoutError` if you ask for a longer delay.
...
<Warning>
  **Deprecated:** older blocks use `ctx.run.pause({ pauseMetadata: { type: PauseType.WEBHOOK | PauseType.DELAY, ... } })` together with `ctx.generateResumeUrl(...)`. That V0 API is kept for backwards compatibility with in-flight paused runs and will be removed. New actions must use `ctx.run.createWaitpoint` + `ctx.run.waitForWaitpoint`.
</Warning>
